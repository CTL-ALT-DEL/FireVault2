...
let quickPhotoDraft0950={dataUrl:"",name:"",accountId:"",category:DEFAULT_PHOTO_CATEGORY_0956,useOverlay:true,includeReport:false};
...
function accountNotesTab0735(s,ctx){
  ...
  return `<div class="accountTabPanel0735">
    ...
    <section class="accountQuickBar0735 accountWorkActions0735 moduleActions0955">${moduleEnabled0955("optional.tasks")?`<button id="qaAddTask544"><span>□</span><strong>${esc(appTerm("task",1))}</strong></button>`:""}${moduleEnabled0955("optional.deficiencies")?`<button id="qaAddDef544"><span>!</span><strong>${esc(appTerm("deficiency",1))}</strong></button>`:""}${moduleEnabled0955("core.photos")?`<button id="qaAddPhoto544"><span>▣</span><strong>Photo</strong></button>`:""}${moduleEnabled0955("optional.reports")?`<button id="qaReport544"><span>▤</span><strong>Report</strong></button>`:""}</section>
...
function siteDetail(){
  ...
  html(`<div class="screen accountDetail0871 accountDetail0952">
    <section class="accountDetailHeader0952 category-${category}">
      ...
      <section class="accountActionGrid0871 accountActionGrid0952 moduleActions0955" aria-label="${esc(appLabel("recordActions"))}">
        <button id="detailCall0871" ${phone?"":"disabled"}>${fvIcon073("call","accountActionIcon0871")}<span>Call</span></button>
        ${moduleEnabled0955("core.nearby")?`<button id="detailRoute0871" ${hasGps(s)?"":"disabled"}>${fvIcon073("route","accountActionIcon0871")}<span>Route</span></button>`:""}
        ${moduleEnabled0955("core.notes")?`<button id="detailNote0871">${fvIcon073("note","accountActionIcon0871")}<span>Add ${esc(appTerm("note",1))}</span></button>`:""}
        ${moduleEnabled0955("core.photos")?`<button id="detailPhoto0952">${fvIcon073("photo","accountActionIcon0871")}<span>Photo</span></button>`:""}
      </section>
...
function quickPhotoStart0950(accountId=""){
  ...
  const rememberedCategory=quickPhotoReadSafe0950(QUICK_PHOTO_LAST_CATEGORY_KEY0950,DEFAULT_PHOTO_CATEGORY_0956)||DEFAULT_PHOTO_CATEGORY_0956;
  quickPhotoDraft0950={dataUrl:"",name:"",accountId:account.id,category:PHOTO_CATEGORIES_524.includes(rememberedCategory)?rememberedCategory:DEFAULT_PHOTO_CATEGORY_0956,useOverlay:true,includeReport:false};
...
function quickPhotoShowReview0950(){
  ...
  document.getElementById("quickPhotoCategory0950").onchange=event=>{quickPhotoDraft0950.category=event.target.value||DEFAULT_PHOTO_CATEGORY_0956;quickPhotoWriteSafe0950(QUICK_PHOTO_LAST_CATEGORY_KEY0950,quickPhotoDraft0950.category);};
  ...
  return `<div class="settingsStack settingsStack540 architectureStack0953">
    ${settingsSection540("Reusable foundation","App Profile",`${APP_PROFILE.name} remains optimized for ${APP_PROFILE.audience.toLowerCase()}, while shared behavior is defined outside the fire-specific layer.`,`
      <div class="architectureProfile0953">
        ...
        <p>The profile supplies terminology, enabled modules, record fields, detail sections, and photo categories to the shared workflows. FireVault keeps every current fire-alarm module and field enabled.</p>
      </div>
    `,"red")}
...
function showChangelog(){
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.95.6 makes the reusable data model profile-driven while preserving every current fire-alarm field and workflow.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
