...
FireVault Build 1.03.27 uses `src/app-profile.js` as the active configuration for identity, terminology, modules, record data, workflow presets, branding, content packs, storage policy, AppForge blueprint participation, product recipes, factory manifests, and generated PWA packages.
...
Build 1.03.27 keeps AppForge out of the normal technician-facing Settings interface. Existing factory and generator tools are available only when FireVault is opened with the explicit `?appforge=1` developer query.
