...
function moduleAccountTabs0955(){
  return ACCOUNT_TAB_BINDINGS.filter(item=>moduleRequirementsMet0955(item.modules)).map(item=>[item.key,item.term?appTerm(item.term,2):item.label,item.modules]);
}
  ...
  return `<div class="settingsStack settingsStack540 architectureStack0953">
    ${settingsSection540("Reusable foundation","App Profile",`${APP_PROFILE.name} remains optimized for ${APP_PROFILE.audience.toLowerCase()}, while shared behavior is defined outside the fire-specific layer.`,`
      <div class="architectureProfile0953">
        ...
        <p>The profile supplies terminology and enabled modules to the Directory, Nearby, Account Detail, forms, Quick Photo, navigation, account tabs, actions, Settings, appearance, and photo categories. FireVault keeps every current fire-alarm module enabled.</p>
      </div>
    `,"red")}
...
function showChangelog(){
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.95.5 makes the live interface respond to the reusable module registry while preserving every current fire-alarm workflow.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
