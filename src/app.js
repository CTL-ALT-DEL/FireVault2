import { BUILD, KEY, loadData, saveData, ensureSite, fullAddress, esc, uid, downloadBlob, syncSummary, syncQueue, syncConflicts, syncActivity, createSyncPackage, importSyncPackage, resolveSyncConflict, notePackageExport, deviceIdentity, recordSyncActivity, autoBackupInfo, latestAutoBackup, restoreAutoBackup, isDemoMode, setDemoMode, resetDemoData, securityFoundationSummary, securityAudit, recycleBinInfo, restoreRecycleRecord, purgeRecycleBin, recordSecurityEvent, validateVaultIntegrity } from "./storage.js?v=1.03.20";
import { backendAdapterSummary, runBackendAdapterDiagnostics, backendAdapterManifest, PROVIDER_CONTRACT_VERSION, FILE_STORAGE_CATALOG, fileStoragePlanSummary, cloudFileStorageManifest, MICROSOFT_STORAGE_TYPES, microsoftStorageAccounts, saveMicrosoftStorageAccounts, createMicrosoftStorageAccount, microsoftStorageAccountById, microsoftAppRegistration, saveMicrosoftAppRegistration, microsoftStorageSummary, microsoftStorageManifest } from "./providers.js?v=1.03.20";
import { encodePlusCode, isValidFullPlusCode, normalizePlusCode, plusCodePrecisionLabel } from "./open-location-code.js?v=1.03.20";
import { hydrateVaultMedia, stageVaultMedia, prepareVaultWithMedia, mediaStorageSummary, requestPersistentMediaStorage, pruneOrphanedMedia, flushMediaWrites } from "./media-store.js?v=1.03.20";
import { APP_PROFILE, APP_PROFILE_SCHEMA_VERSION, appTerm, appLabel, appNavigationLabel, appProfileExport } from "./app-profile.js?v=1.03.20";
import { MODULE_REGISTRY, MODULE_REGISTRY_VERSION, MODULE_CLASSIFICATIONS, FUTURE_APP_COLUMNS, moduleRegistrySummary, moduleMatrixRows, moduleRegistryExport } from "./module-registry.js?v=1.03.20";
import { MODULE_BINDINGS_VERSION, NAV_MODULE_REQUIREMENTS as NAV_MODULE_REQUIREMENTS_0955, ROUTE_MODULE_REQUIREMENTS as ROUTE_MODULE_REQUIREMENTS_0955, SETTINGS_MODULE_REQUIREMENTS as SETTINGS_MODULE_REQUIREMENTS_0955, ACCOUNT_TAB_BINDINGS, requirementsMet as moduleRequirementsMetFromBindings0955, moduleBindingsExport } from "./module-bindings.js?v=1.03.20";
import { RECORD_SCHEMA, RECORD_SCHEMA_VERSION, recordFieldById, activeRecordFields, recordFieldEnabled, recordFieldRequired, activeDetailSections, recordDetailSectionEnabled, recordPhotoCategories, recordSchemaSummary, recordSchemaExport } from "./record-schema.js?v=1.03.20";
import { WORKFLOW_SCHEMA, WORKFLOW_SCHEMA_VERSION, activeWorkflowActions, workflowActionEnabled, quickPhotoWorkflow, workflowSchemaSummary, workflowSchemaExport } from "./workflow-schema.js?v=1.03.20";
import { THEME_PROFILE_SCHEMA_VERSION, resolveThemeProfile, applyThemeProfile, themeBrandAsset, themeWordmarkMarkup, themeProfileSummary, themeProfileExport } from "./theme-profile.js?v=1.03.20";
import { CONTENT_PACK_SCHEMA_VERSION, CONTENT_SOURCE_TYPES, CONTENT_SOURCES, CONTENT_PACKS, resolveContentPackProfile, activeContentSources, activeContentPacks, contentPackLibraryFolders, contentPackSummary, contentPackRegistryExport } from "./content-pack-registry.js?v=1.03.20";
import { SYNC_STORAGE_PROFILE_SCHEMA_VERSION, STORAGE_PROVIDER_TYPES, STORAGE_PROVIDERS, STORAGE_ROLES, resolveSyncStorageProfile, activeStorageProviders, storageProviderEnabled, storageRoleProviders, syncStorageSettingsTabEnabled, syncStorageSummary, syncStorageProfileExport } from "./sync-storage-profile.js?v=1.03.20";
import { APP_FORGE_BLUEPRINT_SCHEMA_VERSION, appForgeReadinessSummary, validateAppForgeProfile, appForgeBlueprintExport } from "./app-forge-blueprint.js?v=1.03.20";
import { APP_FORGE_RECIPE_SCHEMA_VERSION, appForgeRecipes, appForgeRecipeSummary, appForgeRecipeBlueprintExport, appForgeRecipeCatalogExport } from "./app-forge-recipes.js?v=1.03.20";
import { APP_FORGE_FACTORY_SCHEMA_VERSION, createAppForgeGenerationRequest, appForgeFactoryManifest, appForgeFactorySummary, appForgeFactorySchemaExport } from "./app-forge-factory.js?v=1.03.20";
import { APP_FORGE_GENERATOR_SCHEMA_VERSION, appForgeGeneratorPlan, appForgeGeneratorSummary, generateAppForgePwaPackage, appForgeGeneratorSchemaExport } from "./app-forge-generator.js?v=1.03.20";
...
function showDemoAccountChoice10317(){
  ...
  overlay.querySelector("#useRealAccount10317").onclick=()=>{
    ...
    data=loadData({allowEmptyReal:true});applyTheme();close();firstAccountGuide10317=true;startAccountCreation10317();
  };
}
...
function captureFirstAccountAddress10317(){
  if(firstAccountLookupActive10317)return;
  if(!navigator.geolocation){setFirstAccountAssistStatus10317("GPS is unavailable here. Enter the account address manually.","error");document.getElementById("street")?.focus();return;}
  ...
  navigator.geolocation.getCurrentPosition(async position=>{
    const coords={lat:Number(position.coords.latitude.toFixed(6)),lng:Number(position.coords.longitude.toFixed(6)),accuracy:Math.round(position.coords.accuracy||0),capturedAt:new Date().toISOString()};
  ...
  },err=>{firstAccountLookupActive10317=false;setButtonBusy0781(button,false);setFirstAccountAssistStatus10317(`GPS could not be used: ${err.message||"permission denied"}. Enter the address manually.`,"error");document.getElementById("street")?.focus();},gpsOptions());
}
...
function readAccountAddressSearchCache10318(query){return accountAddressSearchCache10318()[String(query).trim().toLowerCase()]||null;}
function writeAccountAddressSearchCache10318(query,results){try{const cache=accountAddressSearchCache10318(),keys=Object.keys(cache);if(keys.length>12)delete cache[keys[0]];cache[String(query).trim().toLowerCase()]=results;sessionStorage.setItem(ACCOUNT_ADDRESS_SEARCH_CACHE_KEY_10318,JSON.stringify(cache));}catch{}}
...
async function searchAccountAddress10318(){
  ...
  try{
    ...
    const results=(await response.json()).map(normalizeAccountAddressCandidate10318).filter(result=>result.displayAddress||result.street);writeAccountAddressSearchCache10318(query,results);renderAccountAddressResults10318(results);setAccountAddressStatus10318("review",results.length?"Select a result below, then confirm it before the address is added.":"No exact match was returned. Refine the search or enter the address manually.");
...
function arrivalGuideMap1020(s,points=[]){
  ...
  return `<div class="arrivalGuideMap1020"><iframe src="${esc(src)}" title="On-site map for ${esc(s.name||"account")}" loading="eager" tabindex="-1"></iframe><div class="arrivalGuideMapGrid1020"></div><div class="arrivalGuideMarkers1020">${model.rows.map(row=>{if(row.kind==="user")return `<span class="arrivalGuideUser1020" style="left:${row.x.toFixed(2)}%;top:${row.y.toFixed(2)}%" title="Your GPS location"></span>`;if(row.kind==="site")return `<span class="arrivalGuideSite1020" style="left:${row.x.toFixed(2)}%;top:${row.y.toFixed(2)}%" title="Account GPS"></span>`;const index=points.findIndex(p=>p.id===row.id)+1,p=points[index-1];return `<button style="left:${row.x.toFixed(2)}%;top:${row.y.toFixed(2)}%" data-arrival-map-point1020="${esc(row.id)}" aria-label="${esc(p?.label||"Saved location")}"><span>${index}</span></button>`;}).join("")}</div><div class="arrivalGuideMapLegend1020"><span><i class="you"></i>You</span><span><i class="site"></i>Account GPS</span></div></div>`;
}
function openArrivalGuide1020(siteId=selectedSiteId,source="manual"){
  ...
  overlay.querySelectorAll("[data-arrival-route1020]").forEach(button=>button.onclick=()=>{const p=points.find(row=>row.id===button.dataset.arrivalRoute1020);if(p)window.open(`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(p.plusCode||`${p.lat},${p.lng}`)}`,'_blank','noopener');});
...
function accountDirectoryRow0759(s,addressCount=1){
  ...
  return `<article class="accountCard0871 accountRow0951 category-${category}" data-account-card0759 data-id="${esc(s.id)}" data-search="${esc(siteSearchBlob(s))}" data-letter0763="${esc(initial)}" role="button" tabindex="0" aria-expanded="false" aria-label="Open ${esc(s.name||recordLower)}, ${esc(address)}">
    ...
    <div class="accountRowActions0951 workflowActions0957" style="${workflowActionStyle0957("directory")}" aria-label="${esc(appLabel("recordActions"))}" aria-hidden="true">
...
function setTopAccountCard10319(list,card){
  if(!list)return null;
  const cards=visibleAccountCards0876(list);
  const target=card&&!card.hidden&&list.contains(card)?card:(cards[0]||null);
  cards.forEach(item=>{
    const active=item===target;
    item.classList.toggle("isTopAccount10319",active);
    item.setAttribute("aria-expanded",String(active));
    const actions=item.querySelector(".accountRowActions0951");
    if(actions)actions.setAttribute("aria-hidden",String(!active));
    actions?.querySelectorAll("button").forEach(button=>{
      if(active)button.removeAttribute("tabindex");
      else button.tabIndex=-1;
    });
  });
  list.dataset.topAccountId10319=target?.dataset.id||"";
  return target;
}
function syncTopAccountCard10319(list){
  return setTopAccountCard10319(list,closestVisibleAccountCard0796(list));
}
...
function settleAccountsList0796(list){
  ...
  if(!card)return;
  // Keep every card compact while the list is moving. Mark the destination
  // now, then reveal its actions only after it is settled at the top.
  setTopAccountCard10319(list,card);
...
function manualHome058(){
  ...
  return `<div class="academyHome067">
    ...
    ${pinnedLearning059()}
    <p class="academyReviewNote067">Manual revision 1.03.20 · Reviewed July 2026. Documentation must be checked with every release.</p>
  </div>`;
}
...
function manualSimplePage058(type){
 const pages={
  quick:["🚀","Quick Start Guide","Get FireVault ready for a normal field day.",[["1. Verify the build","Confirm the green build badge shows 1.03.20 before entering production information."],["2. Complete Technician Profile","Enter your name, company, phone, email, and license or employee identification."],["3. Review permissions","Allow location and photo access only when FireVault requests them and the feature is needed."],["4. Create or open a site","Add the customer name, full address, panel details, contacts, access notes, and GPS location."],["5. Document the visit","Record notes, photos, tasks, deficiencies, equipment changes, and a service visit."],["6. Finish and protect the data","Review the report, send or copy the required summary, then export a current backup."]]],
  new:["🆕","What’s New in 1.03.20","Nearby Accounts is now a calmer, more readable map-and-list workspace without new features.",[["Clearer controls","Map/List and category filtering use larger, structured controls while keeping their existing behavior."],["Selected account","The map overlay keeps the account name, address, Account ID, category, and distance together."],["Ready actions","Route and Call remain available for the selected account without waiting for delayed street zoom."],["Stable scrolling","Nearby cards settle without a smooth-scroll feedback loop and retain the correct selected account."],["Phone and iPad","Phones stay compact while iPad widths present the map and account list side by side."]]],
...
function showChangelog(){
  const notes = [
    ...
    "Map/List and category-filter controls are larger and selected-account context is easier to scan.",
    "Route and Call remain immediately available for the selected account, and missing phone numbers disable Call safely.",
    "Account cards show a larger bottom-right distance, open by tap or keyboard, and keep visual and accessible selection aligned.",
    "Nearby list settling no longer uses a smooth-scroll feedback loop, and iPad widths use a side-by-side map/list layout.",
    "Build 1.03.19 keeps Account Directory rows compact, repairs multi-word search, and protects bottom-navigation labels.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 1.03.20 makes Nearby Accounts calmer and easier to operate on phones and iPads.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
