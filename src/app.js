...
function openLocationEditor07912(pointId=""){
  ...
  const close=(restoreWorkspace=true)=>{overlay.remove();if(restoreWorkspace)resumeNativeFieldWorkspace10329(s);};
  ...
  document.getElementById("buildingLocationForm07912").onsubmit=e=>{
    ...
    save();close(false);accountDetailTab0735="locations";rememberAccountTab0751("locations");toast(`${target.label} saved.`,"success");siteDetail();
  };
}
...
function resumeNativeFieldWorkspace10329(account=site()){
  if(nativeFieldWorkspaceAvailable10329()&&view==="siteDetail"&&account){
    requestAnimationFrame(()=>presentNativeFieldWorkspace10329(account));
  }
}
function nativeWorkspaceDate10329(value=""){
  if(!value)return "";
  const date=new Date(value);
  if(Number.isNaN(date.getTime()))return String(value);
  return date.toLocaleDateString([], {month:"short",day:"numeric"});
}
function nativeFieldWorkspacePayload10329(s){
  const docs=Array.isArray(s.docs)?s.docs:[];
  const equipment=Array.isArray(s.equipment)?s.equipment:[];
  const notes=siteNoteEntries506(s);
  ...
  const primary=primaryContact477(s);
  const recent=[
    ...accountRecentActivity0735(s,10),
    ...notes.map((note,index)=>({
      t:new Date(note.at||0).getTime(),kind:"note",title:"Field note",
      detail:String(note.text||"").replaceAll("\n"," ").slice(0,120),
      routeName:"jobMode",id:String(note.id||`note-${index}`)
  ...
  return {
    ...
    latitude:hasGps(s)?Number(s.gps.lat):null,
    longitude:hasGps(s)?Number(s.gps.lng):null,
    tags:accountTags0737(s).slice(0,4).map(tag=>String(tag.name||"")).filter(Boolean),
    notes:notes.slice(0,30).map((note,index)=>({
      id:String(note.id||`note-${index}`),
      title:String(note.label||"Saved note"),
      text:String(note.text||""),
      date:nativeWorkspaceDate10329(note.at)
    })),
    documents:docs.slice(0,50).map((doc,index)=>({
      id:String(doc.id||`document-${index}`),
      title:String(doc.title||doc.imageName||doc.type||"Saved file"),
      subtitle:docIsScan0800(doc)?`${Number(doc.scanPageCount||doc.scanPages?.length||0)}-page scan`:docHasPhoto512(doc)?String(photoCategory524(doc)||"Photo"):String(doc.type||doc.ref||"Document"),
      kind:docIsScan0800(doc)?"scan":docHasPhoto512(doc)?"photo":"file",
      date:nativeWorkspaceDate10329(doc.updatedAt||doc.createdAt||doc.date)
    })),
    equipment:equipment.slice(0,50).map((item,index)=>({
      id:String(item.id||`equipment-${index}`),
      title:String(equipmentTitle(item)),
      subtitle:String(equipmentMeta(item)),
      status:String(item.status||"Active")
    })),
    locations:locations.slice(0,50).map((point,index)=>({
      id:String(point.id||`location-${index}`),
      label:String(point.label||point.type||"Saved location"),
      subtitle:String([point.type,locationMeta07912(point),point.notes].filter(Boolean).join(" • ")),
      type:String(point.type||"Other"),
      plusCode:String(point.plusCode||""),
      latitude:point.lat!==null&&point.lat!==""&&Number.isFinite(Number(point.lat))?Number(point.lat):null,
      longitude:point.lng!==null&&point.lng!==""&&Number.isFinite(Number(point.lng))?Number(point.lng):null
    })),
    recent:recent.map((item,index)=>({
      id:String(item.id||`recent-${index}-${item.t}`),
      title:String(item.title||"Saved field activity"),
      subtitle:String(item.detail||""),
      kind:String(item.kind||"activity"),
      date:nativeWorkspaceDate10329(item.t)
    }))
  };
}
function presentNativeFieldWorkspace10329(s){
  if(!nativeFieldWorkspaceAvailable10329()||!s)return false;
  try{
    window.webkit.messageHandlers.fireVaultWorkspace.postMessage({
      action:"present",
      requestId:`workspace-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      account:nativeFieldWorkspacePayload10329(s)
    });
    return true;
  }catch(error){
    console.warn("Native Field Workspace unavailable",error);
    return false;
  }
}
window.fireVaultNativeWorkspaceAction=function(message={}){
  const action=String(message.action||"");
  const target=(data.sites||[]).find(item=>item.id===String(message.accountId||""));
  if(target)selectedSiteId=target.id;
  const s=site();
  if(!s)return;
  switch(action){
    case "back":{
      const destination=accountDetailReturn0952||"sites";
      route(destination==="home"?"home":destination==="nearbySites"?"nearbySites":"sites");
      break;
    }
    case "favorite": toggleSitePinned566(); break;
    case "edit": mode="edit";route("siteForm");break;
    case "call":{
      const contact=primaryContact477(s),phone=formatPhone0758(contact?.phone||s.sitePhone)||"";
      if(phone)location.href=`tel:${phone.replace(/[^+\d]/g,"")}`;
      else toast("No phone number is saved for this account.");
      break;
    }
    case "scan": startNativeDocumentScan10325(s,{returnRoute:"siteDetail"});break;
    case "note": addSiteNotePrompt();break;
    case "photo": quickPhotoStart0950(s.id);break;
    case "route": routeSite10323(s);break;
    case "nearby": mode=null;restoreAppChrome572();route("home");break;
    case "search": mode=null;restoreAppChrome572();route("sites");break;
    case "settings": mode=null;restoreAppChrome572();openSettingsHome572();break;
    case "addFile": mode=null;route("siteDocForm");break;
    case "openFile":{
      const doc=(s.docs||[]).find(item=>item.id===String(message.id||""));
      if(!doc){toast("That saved file could not be found.");break;}
      if(docIsScan0800(doc))scannedDocumentModal0800(doc);
      else if(docHasPhoto512(doc))photoPreviewModal524(doc);
      else{mode=doc.id;route("siteDocForm");}
      break;
    }
    case "addEquipment": mode=null;route("equipmentForm");break;
    case "openEquipment": mode=String(message.id||"")||null;route(mode?"equipmentForm":"equipmentList");break;
    case "addLocation": addLocationPoint071();break;
    case "routeLocation": routeLocationPoint071(String(message.id||""));break;
    default: break;
  }
};
function siteDetail(){
  ...
  if(nativeFieldWorkspaceAvailable10329())requestAnimationFrame(()=>presentNativeFieldWorkspace10329(s));
...
function photoPreviewModal524(d){
  ...
  document.getElementById("modalEditPhoto524").onclick=()=>{ close(false); mode=d.id; route("siteDocForm"); };
}
...
async function startNativeDocumentScan10325(account=site(),options={}){
  ...
  try{
    ...
    resumeNativeFieldWorkspace10329(account);
  }
}
function showNativeScanReview10325(account,pages,source,options={}){
  ...
  const close=(restoreWorkspace=true)=>{overlay.remove();document.body.classList.remove("nativeScanReviewOpen10325");if(restoreWorkspace)resumeNativeFieldWorkspace10329(account);};
  ...
  document.getElementById("saveNativeScan10325").onclick=async()=>{
    ...
    try{
      ...
      save();close(false);toast(`${validPages.length}-page scan saved to ${account.name||"account"}.`,"success");route(options.returnRoute==="siteDetail"?"siteDetail":"siteDocs");
    }catch(error){
...
function scannedDocumentModal0800(d){
  ...
  const close=(restoreWorkspace=true)=>{overlay.remove();if(restoreWorkspace)resumeNativeFieldWorkspace10329();};overlay.onclick=e=>{if(e.target===overlay)close();};document.getElementById("scanModalClose0800").onclick=close;document.getElementById("scanModalPdf0800").onclick=()=>scannerDownloadPdf0800(d);document.getElementById("scanModalShare0800").onclick=()=>scannerSharePdf0800(d);document.getElementById("scanModalDelete0800").onclick=()=>{if(confirm(`Delete “${d.title||"this scanned document"}”?`)){const s=site();s.docs=s.docs.filter(x=>x.id!==d.id);save();close(false);toast("Scanned document deleted.");if(view==="siteDetail")render();else siteDocs();}};
}
