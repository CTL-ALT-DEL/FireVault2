import { BUILD, KEY, loadData, saveData, ensureSite, fullAddress, esc, uid, downloadBlob, syncSummary, syncQueue, syncConflicts, syncActivity, createSyncPackage, importSyncPackage, resolveSyncConflict, notePackageExport, deviceIdentity, recordSyncActivity, autoBackupInfo, latestAutoBackup, restoreAutoBackup, isDemoMode, setDemoMode, resetDemoData, securityFoundationSummary, securityAudit, recycleBinInfo, restoreRecycleRecord, purgeRecycleBin, recordSecurityEvent, validateVaultIntegrity } from "./storage.js?v=1.03.25";
import { backendAdapterSummary, runBackendAdapterDiagnostics, backendAdapterManifest, PROVIDER_CONTRACT_VERSION, FILE_STORAGE_CATALOG, fileStoragePlanSummary, cloudFileStorageManifest, MICROSOFT_STORAGE_TYPES, microsoftStorageAccounts, saveMicrosoftStorageAccounts, createMicrosoftStorageAccount, microsoftStorageAccountById, microsoftAppRegistration, saveMicrosoftAppRegistration, microsoftStorageSummary, microsoftStorageManifest } from "./providers.js?v=1.03.25";
import { encodePlusCode, isValidFullPlusCode, normalizePlusCode, plusCodePrecisionLabel } from "./open-location-code.js?v=1.03.25";
import { hydrateVaultMedia, stageVaultMedia, prepareVaultWithMedia, mediaStorageSummary, requestPersistentMediaStorage, pruneOrphanedMedia, flushMediaWrites } from "./media-store.js?v=1.03.25";
import { APP_PROFILE, APP_PROFILE_SCHEMA_VERSION, appTerm, appLabel, appNavigationLabel, appProfileExport } from "./app-profile.js?v=1.03.25";
import { MODULE_REGISTRY, MODULE_REGISTRY_VERSION, MODULE_CLASSIFICATIONS, FUTURE_APP_COLUMNS, moduleRegistrySummary, moduleMatrixRows, moduleRegistryExport } from "./module-registry.js?v=1.03.25";
import { MODULE_BINDINGS_VERSION, NAV_MODULE_REQUIREMENTS as NAV_MODULE_REQUIREMENTS_0955, ROUTE_MODULE_REQUIREMENTS as ROUTE_MODULE_REQUIREMENTS_0955, SETTINGS_MODULE_REQUIREMENTS as SETTINGS_MODULE_REQUIREMENTS_0955, ACCOUNT_TAB_BINDINGS, requirementsMet as moduleRequirementsMetFromBindings0955, moduleBindingsExport } from "./module-bindings.js?v=1.03.25";
import { RECORD_SCHEMA, RECORD_SCHEMA_VERSION, recordFieldById, activeRecordFields, recordFieldEnabled, recordFieldRequired, activeDetailSections, recordDetailSectionEnabled, recordPhotoCategories, recordSchemaSummary, recordSchemaExport } from "./record-schema.js?v=1.03.25";
import { WORKFLOW_SCHEMA, WORKFLOW_SCHEMA_VERSION, activeWorkflowActions, workflowActionEnabled, quickPhotoWorkflow, workflowSchemaSummary, workflowSchemaExport } from "./workflow-schema.js?v=1.03.25";
import { THEME_PROFILE_SCHEMA_VERSION, resolveThemeProfile, applyThemeProfile, themeBrandAsset, themeWordmarkMarkup, themeProfileSummary, themeProfileExport } from "./theme-profile.js?v=1.03.25";
import { CONTENT_PACK_SCHEMA_VERSION, CONTENT_SOURCE_TYPES, CONTENT_SOURCES, CONTENT_PACKS, resolveContentPackProfile, activeContentSources, activeContentPacks, contentPackLibraryFolders, contentPackSummary, contentPackRegistryExport } from "./content-pack-registry.js?v=1.03.25";
import { SYNC_STORAGE_PROFILE_SCHEMA_VERSION, STORAGE_PROVIDER_TYPES, STORAGE_PROVIDERS, STORAGE_ROLES, resolveSyncStorageProfile, activeStorageProviders, storageProviderEnabled, storageRoleProviders, syncStorageSettingsTabEnabled, syncStorageSummary, syncStorageProfileExport } from "./sync-storage-profile.js?v=1.03.25";
import { APP_FORGE_BLUEPRINT_SCHEMA_VERSION, appForgeReadinessSummary, validateAppForgeProfile, appForgeBlueprintExport } from "./app-forge-blueprint.js?v=1.03.25";
import { APP_FORGE_RECIPE_SCHEMA_VERSION, appForgeRecipes, appForgeRecipeSummary, appForgeRecipeBlueprintExport, appForgeRecipeCatalogExport } from "./app-forge-recipes.js?v=1.03.25";
import { APP_FORGE_FACTORY_SCHEMA_VERSION, createAppForgeGenerationRequest, appForgeFactoryManifest, appForgeFactorySummary, appForgeFactorySchemaExport } from "./app-forge-factory.js?v=1.03.25";
import { APP_FORGE_GENERATOR_SCHEMA_VERSION, appForgeGeneratorPlan, appForgeGeneratorSummary, generateAppForgePwaPackage, appForgeGeneratorSchemaExport } from "./app-forge-generator.js?v=1.03.25";
...
async function startNativeDocumentScan10325(account=site()){
  ...
  try{
    ...
    showNativeScanReview10325(account,result.pages||[],result.source||"Apple VisionKit Document Camera");
  }catch(error){
    setButtonBusy0781(button,false);
    if(!error?.cancelled)toast(error?.message||"Document scanning failed.","error");
  }
}
function showNativeScanReview10325(account,pages,source){
  ...
  document.getElementById("saveNativeScan10325").onclick=async()=>{
    ...
    try{
      ...
      save();close();toast(`${validPages.length}-page scan saved to ${account.name||"account"}.`,"success");route("siteDocs");
    }catch(error){
...
const FIREVAULT_MANUAL_058 = [
  ...
  {id:"photos",title:"Photos & Photo Overlay",icon:"▧",status:"Current",summary:"Capture, label, organize, and preserve site photographs.",topics:[
    ...
    ["Scan a document on iPhone","In the installed native iPhone app, open the correct account, choose Documents / Photos, and tap Scan Document. Apple’s scanner finds the page edges, corrects perspective, and lets you capture, review, retake, or add more pages. Tap Done, enter the document title and type, then tap Save Scan. The browser PWA intentionally does not show this native-only button."],
...
function manualHome058(){
  ...
  return `<div class="academyHome067">
    ...
    ${pinnedLearning059()}
    <p class="academyReviewNote067">Manual revision 1.03.25 · Reviewed July 2026. Documentation must be checked with every release.</p>
  </div>`;
}
...
function manualSimplePage058(type){
 ...
 pages.quick[3][0][1]="Confirm the green build badge shows 1.03.25 before entering production information.";
 pages.new=["🆕","What’s New in 1.03.25","Apple-powered document scanning is restored in the native iPhone app.",[
  ["Native Scan Document","Open an account’s Documents / Photos workspace and tap Scan Document in the installed Xcode app."],
  ["Apple camera quality","Apple VisionKit finds document edges, corrects perspective, supports retakes, and captures multiple pages."],
  ["Simple save review","Review the page order, enter a title and document type, add optional notes, then save to the account."],
  ["Offline account storage","Scanned pages stay in protected on-device media storage and appear in the account timeline."],
  ["PDF tools retained","Reopen, preview, download, and share saved scans using the existing PDF tools."]
 ]];
 if(Array.isArray(pages.revisions?.[3]?.[0]))pages.revisions[3][0].unshift(["1.03.25","Restores account document scanning through Apple VisionKit with automatic edge correction, multi-page capture, compact save review, IndexedDB storage, and retained PDF tools."]);
...
const ACADEMY_TUTORIALS_059=[
 ...
 {id:"scanner",icon:"▤",title:"Scan a Site Document",time:"3 min",level:"Beginner",summary:"Use Apple’s document camera to attach a clean multi-page scan to an account.",steps:["In the installed iPhone app, open the correct account and choose Documents / Photos.","Tap Scan Document and hold the phone over the first page. Apple finds and corrects the page edges.","Review or retake the page, then capture any additional pages and tap Done.","Confirm the page order, enter a title and document type, and add optional notes.","Tap Save Scan, reopen it from the account, and use Download PDF or Share when needed."]},
...
function showChangelog(){
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 1.03.25 restores native Apple document scanning without changing existing account data.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
