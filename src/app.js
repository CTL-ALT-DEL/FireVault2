...
function showChangelog(){
  const notes = [
    "Build 0.94.10 fixes the Account Detail crash when opening an account from Nearby Accounts or Search and preserves the Tech Info overlay improvements from Build 0.94.9.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.94.10 fixes Account Detail opening from Nearby Accounts and Search while preserving the Photo Overlay improvements from Build 0.94.9.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
