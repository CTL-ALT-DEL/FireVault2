...
function settings(){
  ...
  document.getElementById("settingsDoneBtn")?.addEventListener("click",()=>{mode=dashboardItem.tabs?.length>1?"settingsGroup0860":null;render();});
...
function showChangelog(){
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.91.1 corrects the three Settings status shortcuts so Location, Backup, and Demo Mode align cleanly across iPhone and iPad.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
