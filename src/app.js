...
function settingsQuickIcon0911(name){
  const paths={
    location:'<path d="M12 21s7-6.1 7-12A7 7 0 1 0 5 9c0 5.9 7 12 7 12Z"/><circle cx="12" cy="9" r="2.4"/>',
    backup:'<path d="M7.2 18.5H6a4 4 0 0 1-.5-8A6.5 6.5 0 0 1 18 8.8a4.4 4.4 0 0 1 .1 8.8H17"/><path d="M12 11v10m0-10-3 3m3-3 3 3"/>',
    demo:'<rect x="3" y="4" width="18" height="14" rx="2.5"/><path d="m10 8.5 5 2.5-5 2.5Z"/><path d="M9 21h6m-3-3v3"/>'
  };
  return `<svg class="settingsQuickIcon0911" viewBox="0 0 24 24" aria-hidden="true">${paths[name]||paths.demo}</svg>`;
}
function settingsOverview0880(){
  const backupInfo=autoBackupInfo();
  const webdav=data.settings?.webdav||{};
  const items=[
    {tab:"gps",icon:"location",label:"Location",value:data.settings?.gps?.enabled===false?"Off":settingsTabStatus0880("gps")[0],tone:"green"},
    {tab:webdav.enabled?"webdav":"backup",icon:"backup",label:"Backup",value:webdav.lastUpload?settingsShortDate0880(webdav.lastUpload):(backupInfo.count?`${backupInfo.count} local`:webdav.enabled?"WebDAV on":"Not created"),tone:"amber"},
    {tab:"demo",icon:"demo",label:"Demo Mode",value:isDemoMode()?"Active":"Off",tone:isDemoMode()?"gold":"slate"}
  ];
  return `<div class="settingsStatusRail0880" aria-label="Settings status shortcuts">${items.map(item=>`<button class="settingsStatusCard0880 tone-${item.tone}" data-settings-quick0880="${esc(item.tab)}" aria-label="Open ${esc(item.label)} settings. Current status: ${esc(item.value)}"><span class="settingsStatusIcon0911">${settingsQuickIcon0911(item.icon)}</span><span class="settingsStatusText0911"><small>${esc(item.label)}</small><strong>${esc(item.value)}</strong></span><b class="settingsStatusChevron0911" aria-hidden="true">›</b></button>`).join("")}</div>`;
}
...
function showChangelog(){
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.93.0 focuses on dependable field interaction: visible launch branding, safer forms, accurate navigation states, keyboard-safe editing, and protection from accidental duplicate actions.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
