...
function applyTechnicianInfoTemplate0946(){
  ...
  const groupAlign=technicianOverlaySavedAlignment0948();
  ...
  const inserted=template.map((item,index)=>({...item,align:groupAlign,noWrap:true,breakBefore:index===0?remaining.length>0:Boolean(item.breakBefore)}));
...
function overlayRenderedLines0945(set,siteData){
  ...
  layout.forEach((item,index)=>{
    if(index===0||item.breakBefore)logical.push({left:[],center:[],right:[],noWrap:false});
  ...
  return `<div class="overlayStudio0890 overlayStudio0942 overlayStudio0943">
    ...
    <div class="overlayControlsColumn0942 overlayControlsColumn0943">
      <section class="overlaySection0890 overlayContent0890 overlayFieldBuilder0944">
        ...
        <div class="overlayBuilderGroup0944"><div class="overlayBuilderTitle0944"><div><strong>Fields shown on photo</strong><small>Reorder, start a new line, or align fields left, center, or right.</small></div><button type="button" class="overlayQuickAlign0945" id="overlayTechnicianInfo0946"><span>＋</span><strong>TECHNICIAN INFO</strong><small>Use Profile template</small></button></div><div class="overlayActiveFields0944" id="overlayActiveFields0944">${overlayActiveFieldsMarkup0944()}</div></div>
...
function showChangelog(){
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.94.8 improves Technician Overlay Template fitting and adds group-level left, center, and right alignment.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
