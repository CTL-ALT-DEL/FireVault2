import { BUILD, KEY, loadData, saveData, ensureSite, fullAddress, esc, uid, downloadBlob, syncSummary, syncQueue, syncConflicts, syncActivity, createSyncPackage, importSyncPackage, resolveSyncConflict, notePackageExport, deviceIdentity, recordSyncActivity, autoBackupInfo, latestAutoBackup, restoreAutoBackup, isDemoMode, setDemoMode, resetDemoData, securityFoundationSummary, securityAudit, recycleBinInfo, restoreRecycleRecord, purgeRecycleBin, recordSecurityEvent, validateVaultIntegrity } from "./storage.js?v=1.03.17";
import { backendAdapterSummary, runBackendAdapterDiagnostics, backendAdapterManifest, PROVIDER_CONTRACT_VERSION, FILE_STORAGE_CATALOG, fileStoragePlanSummary, cloudFileStorageManifest, MICROSOFT_STORAGE_TYPES, microsoftStorageAccounts, saveMicrosoftStorageAccounts, createMicrosoftStorageAccount, microsoftStorageAccountById, microsoftAppRegistration, saveMicrosoftAppRegistration, microsoftStorageSummary, microsoftStorageManifest } from "./providers.js?v=1.03.17";
import { encodePlusCode, isValidFullPlusCode, normalizePlusCode, plusCodePrecisionLabel } from "./open-location-code.js?v=1.03.17";
import { hydrateVaultMedia, stageVaultMedia, prepareVaultWithMedia, mediaStorageSummary, requestPersistentMediaStorage, pruneOrphanedMedia, flushMediaWrites } from "./media-store.js?v=1.03.17";
import { APP_PROFILE, APP_PROFILE_SCHEMA_VERSION, appTerm, appLabel, appNavigationLabel, appProfileExport } from "./app-profile.js?v=1.03.17";
import { MODULE_REGISTRY, MODULE_REGISTRY_VERSION, MODULE_CLASSIFICATIONS, FUTURE_APP_COLUMNS, moduleRegistrySummary, moduleMatrixRows, moduleRegistryExport } from "./module-registry.js?v=1.03.17";
import { MODULE_BINDINGS_VERSION, NAV_MODULE_REQUIREMENTS as NAV_MODULE_REQUIREMENTS_0955, ROUTE_MODULE_REQUIREMENTS as ROUTE_MODULE_REQUIREMENTS_0955, SETTINGS_MODULE_REQUIREMENTS as SETTINGS_MODULE_REQUIREMENTS_0955, ACCOUNT_TAB_BINDINGS, requirementsMet as moduleRequirementsMetFromBindings0955, moduleBindingsExport } from "./module-bindings.js?v=1.03.17";
import { RECORD_SCHEMA, RECORD_SCHEMA_VERSION, recordFieldById, activeRecordFields, recordFieldEnabled, recordFieldRequired, activeDetailSections, recordDetailSectionEnabled, recordPhotoCategories, recordSchemaSummary, recordSchemaExport } from "./record-schema.js?v=1.03.17";
import { WORKFLOW_SCHEMA, WORKFLOW_SCHEMA_VERSION, activeWorkflowActions, workflowActionEnabled, quickPhotoWorkflow, workflowSchemaSummary, workflowSchemaExport } from "./workflow-schema.js?v=1.03.17";
import { THEME_PROFILE_SCHEMA_VERSION, resolveThemeProfile, applyThemeProfile, themeBrandAsset, themeWordmarkMarkup, themeProfileSummary, themeProfileExport } from "./theme-profile.js?v=1.03.17";
import { CONTENT_PACK_SCHEMA_VERSION, CONTENT_SOURCE_TYPES, CONTENT_SOURCES, CONTENT_PACKS, resolveContentPackProfile, activeContentSources, activeContentPacks, contentPackLibraryFolders, contentPackSummary, contentPackRegistryExport } from "./content-pack-registry.js?v=1.03.17";
import { SYNC_STORAGE_PROFILE_SCHEMA_VERSION, STORAGE_PROVIDER_TYPES, STORAGE_PROVIDERS, STORAGE_ROLES, resolveSyncStorageProfile, activeStorageProviders, storageProviderEnabled, storageRoleProviders, syncStorageSettingsTabEnabled, syncStorageSummary, syncStorageProfileExport } from "./sync-storage-profile.js?v=1.03.17";
import { APP_FORGE_BLUEPRINT_SCHEMA_VERSION, appForgeReadinessSummary, validateAppForgeProfile, appForgeBlueprintExport } from "./app-forge-blueprint.js?v=1.03.17";
import { APP_FORGE_RECIPE_SCHEMA_VERSION, appForgeRecipes, appForgeRecipeSummary, appForgeRecipeBlueprintExport, appForgeRecipeCatalogExport } from "./app-forge-recipes.js?v=1.03.17";
import { APP_FORGE_FACTORY_SCHEMA_VERSION, createAppForgeGenerationRequest, appForgeFactoryManifest, appForgeFactorySummary, appForgeFactorySchemaExport } from "./app-forge-factory.js?v=1.03.17";
import { APP_FORGE_GENERATOR_SCHEMA_VERSION, appForgeGeneratorPlan, appForgeGeneratorSummary, generateAppForgePwaPackage, appForgeGeneratorSchemaExport } from "./app-forge-generator.js?v=1.03.17";
...
function applyDetectedAddress10317(result){
  ...
  applyGpsFields10317(result);firstAccountDetected10317=result;
...
function firstAccountAssistantMarkup10317(){return `<section class="card firstAccountAssistant10317"><div class="firstAccountAssistHead10317"><span>1</span><div><em>FIRST ACCOUNT GUIDE</em><h2>Start with your current location</h2><p>FireVault can find the nearest address. You will confirm it before any fields are filled.</p></div></div><button type="button" class="primary" id="firstAccountGps10317">⌖ Find Current Address</button><button type="button" class="ghost" id="firstAccountManual10317">Enter Address Manually</button><p id="firstAccountAssistStatus10317" aria-live="polite">GPS sends your coordinates for one address lookup only after you tap the button.</p></section>`;}
...
function siteForm(){
  ...
  html(`<div class="screen accountFormScreen0760">
    <section class="accountFormHeader0760 accountFormTop0760"><button class="back ghost" id="backBtn" aria-label="Cancel and go back">←</button><div><span>${editing?`${esc(recordSingular.toUpperCase())} MAINTENANCE`:`NEW ${esc(recordSingular.toUpperCase())}`}</span><h1>${esc(editing?appLabel("editRecord"):appLabel("addRecord"))}</h1><p>${editing?`Update this ${esc(recordLower)} without changing its history.`:`Create the core ${esc(recordLower)} now. Details can be added after saving.`}</p></div></section>
    <div class="form grow accountForm0760">
      <div class="accountFormError0760" id="accountFormError0760" hidden></div>
      ${guidedNewAccount10317?firstAccountAssistantMarkup10317():""}
  ...
  document.getElementById("firstAccountManual10317")?.addEventListener("click",()=>{setFirstAccountAssistStatus10317("Manual entry selected. Add the account name and address below.","ready");document.getElementById("name")?.focus();});
  ...
  document.getElementById("saveBtn")?.addEventListener("click",()=>{
    ...
    if(fieldOn("gps")){
      ...
      if(Number.isFinite(gpsLat)&&Number.isFinite(gpsLng)){
        obj.gps={lat:gpsLat,lng:gpsLng,accuracy:Number(val("gpsAccuracy"))||0,capturedAt:val("gpsCapturedAt")||new Date().toISOString(),source:firstAccountDetected10317?"Browser GPS + confirmed address":"Browser GPS"};
    ...
    save();completeFirstRun10317();firstAccountGuide10317=false;firstAccountDetected10317=null;toast(editing?`${recordSingular} updated.`:guidedNewAccount10317?`First ${recordLower} created. Add contacts, photos, and exact locations from Account Detail.`:`${recordSingular} created.`);route("siteDetail");
  });
...
const FIREVAULT_MANUAL_058 = [
  {id:"start",title:"Getting Started",icon:"▶",status:"Current",summary:"Install FireVault, understand local storage, and learn the main navigation.",topics:[
    ...
    ["Create your first real account","Tap Add Account when you are ready. Choose Yes — Use Real Vault to leave Demo Mode, then tap Find Current Address. FireVault asks for location permission, suggests the nearby address and any available commercial name, and waits for you to confirm before filling the form. Choose the manual option whenever the suggestion is unavailable or incorrect."],
...
function manualHome058(){
  ...
  return `<div class="academyHome067">
    ...
    ${pinnedLearning059()}
    <p class="academyReviewNote067">Manual revision 1.03.17 · Reviewed July 2026. Documentation must be checked with every release.</p>
  </div>`;
}
...
function manualSimplePage058(type){
 const pages={
  quick:["🚀","Quick Start Guide","Get FireVault ready for a normal field day.",[["1. Verify the build","Confirm the green build badge shows 1.03.17 before entering production information."],["2. Complete Technician Profile","Enter your name, company, phone, email, and license or employee identification."],["3. Review permissions","Allow location and photo access only when FireVault requests them and the feature is needed."],["4. Create or open a site","Add the customer name, full address, panel details, contacts, access notes, and GPS location."],["5. Document the visit","Record notes, photos, tasks, deficiencies, equipment changes, and a service visit."],["6. Finish and protect the data","Review the report, send or copy the required summary, then export a current backup."]]],
  new:["🆕","What’s New in 1.03.17","New users now get a short path from Demo Mode to a confirmed GPS-assisted first account.",[["Three quick cards","First-open guidance introduces Demo Mode, Search, Nearby Accounts, Account Detail, and adding real data."],["Protected transition","Add Account asks before leaving Demo Mode so fictional and real customer information remain separate."],["Address assistance","GPS finds the nearest address and any available commercial location name, then waits for confirmation."],["Manual fallback","Permission denial, a wrong address, or an unavailable lookup never blocks manual account entry."],["Private by design","Location lookup starts only after a tap, uses one request, and nothing is saved until the account form is completed."]]],
...
const ACADEMY_TUTORIALS_059=[
 ...
 {id:"first-site",icon:"▦",title:"Create Your First Customer",time:"4 min",level:"Beginner",summary:"Move safely from Demo Mode into a confirmed first real account.",steps:["Explore the fictional demo accounts, then tap Add Account when you are ready.","Choose Yes — Use Real Vault for real customer information, or choose No to keep practicing in Demo Mode.","Tap Find Current Address and approve location access. Confirm the suggested address and commercial name, try again, or enter the address manually.","Complete the required account information, panel, contact, access notes, and any other known details, then save.","Open Account Detail and review Site Brief and Important Site Info for accuracy."]},
...
function showChangelog(){
  const notes = [
    "Build 1.03.17 guides new users from Demo Mode into a confirmed GPS-assisted first account.",
    "Three short first-open cards introduce Demo Mode, Search, Nearby Accounts, Account Detail, and the move to real data.",
    "Add Account asks whether to use the real vault or continue with a temporary practice account.",
    "The first-account guide requests GPS only after a tap and performs one reverse-address lookup.",
    "A confirmation popup shows the address and any available commercial location name before filling the form.",
    "Manual entry remains available after permission denial, lookup failure, no match, or a wrong suggested address.",
    "Confirmed GPS continues through the existing offline Google Plus Code generator and unchanged account save path.",
    "A dedicated onboarding contract protects Demo Mode isolation, lookup privacy, confirmation, fallbacks, and responsive geometry.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 1.03.17 gives new users a safe, guided path to their first real account.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
