...
function showChangelog(){
  const notes = [
    "Build 0.95.1 redesigns Account Directory as a compact dark two-line list with faster visual scanning and preserved account actions.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.95.1 introduces the compact dark Account Directory design selected from Concept 6.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
