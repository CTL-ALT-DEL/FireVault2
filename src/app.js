...
function deficiencyCloseoutStatus545(d={}){
  const closed=deficiencyClosed(d);
  if(closed && d.resolutionNotes) return "Closeout documented";
  if(closed) return "Closed — needs closeout note";
  if(d.closeoutStep) return d.closeoutStep;
  return "Needs review";
}
function deficiencyCustomerLine545(d={}){
  const parts=[];
  parts.push(d.title || "Deficiency");
  if(d.priority) parts.push(d.priority);
  if(d.status) parts.push(d.status);
  if(d.resolutionNotes) parts.push(`Resolution: ${String(d.resolutionNotes).replaceAll("
"," / ")}`);
  else if(d.closeoutStep) parts.push(`Next step: ${String(d.closeoutStep).replaceAll("
"," / ")}`);
  return parts.join(" — ");
}
function deficiencyCloseoutText545(s={}, d={}){
  const photos=deficiencyPhotos525(s,d);
  const lines=[];
  lines.push(`DEFICIENCY CLOSEOUT - ${s.name || "Site"}`);
  lines.push(fullAddress(s));
  lines.push("");
  lines.push(`Issue: ${d.title || "Deficiency"}`);
  lines.push(`Priority: ${d.priority || "Normal"}`);
  lines.push(`Status: ${d.status || "Open"}`);
  if(d.createdAt) lines.push(`Created: ${new Date(d.createdAt).toLocaleString()}`);
  if(d.resolvedAt) lines.push(`Closed: ${new Date(d.resolvedAt).toLocaleString()}`);
  if(d.notes) lines.push(`Field notes: ${String(d.notes).replaceAll("
"," / ")}`);
  if(d.closeoutStep) lines.push(`Next action: ${String(d.closeoutStep).replaceAll("
"," / ")}`);
  if(d.resolutionNotes) lines.push(`Resolution notes: ${String(d.resolutionNotes).replaceAll("
"," / ")}`);
  if(d.customerNote) lines.push(`Customer note: ${String(d.customerNote).replaceAll("
"," / ")}`);
  lines.push(`Linked photos: ${photos.length}`);
  photos.slice(0,20).forEach((p,i)=>lines.push(`${i+1}. ${p.title || p.imageName || "Deficiency Photo"}${p.customerCaption?` — ${p.customerCaption}`:""}`));
  lines.push("");
  lines.push(`Generated by FireVault Build ${BUILD}`);
  return lines.join("
");
}
async function copyDeficiencyCloseout545(siteId, defId){
  const s=ensureSite(data.sites.find(x=>x.id===siteId));
  const d=(s.deficiencies||[]).find(x=>x.id===defId);
  if(!s || !d) return toast("Deficiency not found.");
  try{ await navigator.clipboard.writeText(deficiencyCloseoutText545(s,d)); toast("Deficiency closeout copied."); }
  catch{ toast("Clipboard unavailable."); }
}
function markDeficiencyClosed545(siteId, defId){
  const s=ensureSite(data.sites.find(x=>x.id===siteId));
  const d=(s.deficiencies||[]).find(x=>x.id===defId);
  if(!s || !d) return;
  d.status="Closed";
  d.resolvedAt=d.resolvedAt || new Date().toISOString();
  d.updatedAt=new Date().toISOString();
  if(!d.resolutionNotes) d.resolutionNotes="Resolved / corrected. Add details here if needed.";
  save(); toast("Deficiency closed with closeout note."); deficiencies();
}
...
function resolveDeficiency(siteId, defId){
  ...
  const d=(s.deficiencies||[]).find(x=>x.id===defId);
  if(!d) return;
  d.status="Closed"; d.resolvedAt=new Date().toISOString(); d.updatedAt=new Date().toISOString();
  if(!d.resolutionNotes) d.resolutionNotes="Closed in field. Add resolution details if needed.";
...
function reopenDeficiency(siteId, defId){
  ...
  const d=(s.deficiencies||[]).find(x=>x.id===defId);
  if(!d) return;
  d.status="Open"; delete d.resolvedAt; d.updatedAt=new Date().toISOString();
...
function saveDeficiencyFromForm525(openPhotoAfter=false){
  ...
  const obj={title:val("title")||"Untitled Deficiency",priority:val("priority"),status:val("status"),notes:raw("notes"),closeoutStep:raw("closeoutStep545"),resolutionNotes:raw("resolutionNotes545"),customerNote:raw("customerNote545"),updatedAt:new Date().toISOString()};
  let savedDef=null;
...
function deficiencyCard(r){
  ...
  return `<div class="card siteItem deficiencyCard439 deficiencyCard525 ${closed?"defClosed":""} def-${esc(sev)}" data-site="${esc(r.s.id)}" data-id="${esc(d.id)}">
    ...
    <div class="defCloseoutLine545"><strong>${esc(deficiencyCloseoutStatus545(d))}</strong>${d.customerNote?`<span>${esc(d.customerNote)}</span>`:""}</div>
    ...
    <div class="defQuickActions"><button class="ghost smallBtn defSiteBtn" data-site="${esc(r.s.id)}">Site</button><button class="ghost smallBtn defAddPhotoBtn525" data-site="${esc(r.s.id)}" data-id="${esc(d.id)}">＋ Photo</button><button class="ghost smallBtn defCopyCloseout545" data-site="${esc(r.s.id)}" data-id="${esc(d.id)}">Copy Closeout</button><button class="${closed?"ghost":"primary"} smallBtn defResolveBtn" data-site="${esc(r.s.id)}" data-id="${esc(d.id)}">${closed?"Reopen":"Close"}</button></div>
  </div>`;
}
function deficiencies(){
  ...
  html(`<div class="screen deficiencyScreen439"><div class="row defTopRow"><button class="back ghost" id="backBtn">←</button><div><h1>Deficiency Center</h1><p>${selectedSiteId ? esc(site()?.name || "Site deficiencies") : "Open issues across all sites"}</p></div><button class="primary" id="addBtn">＋</button></div>
    <div class="card defHero439 defHero545"><div><strong>${counts.open}</strong><span>Open</span></div><div><strong>${counts.critical}</strong><span>Critical</span></div><div><strong>${counts.checklist}</strong><span>Checklist</span></div><div><strong>${counts.closed}</strong><span>Closed</span></div><div><strong>${scoped.filter(r=>deficiencyClosed(r.d)&&!r.d.resolutionNotes).length}</strong><span>Need closeout</span></div></div>
  ...
  document.querySelectorAll(".defCopyCloseout545").forEach(b=>b.onclick=e=>{e.stopPropagation(); copyDeficiencyCloseout545(b.dataset.site,b.dataset.id);});
...
function deficiencyForm(){
  ...
  html(`<div class="screen deficiencyForm525"><div class="row"><button class="back ghost" id="backBtn">←</button><h1>${mode?"Edit":"Add"} Deficiency</h1></div><div class="form grow"><div class="card deficiencyFormCard525"><label>Site</label><select id="sitePick">${data.sites.map(x=>`<option value="${x.id}" ${x.id===s.id?"selected":""}>${esc(x.name)}</option>`).join("")}</select><label>Title</label><input id="title" value="${esc(d.title||"")}" placeholder="Smoke detector trouble, NAC open, battery date, ground fault..."><div class="compactField"><div><label>Priority</label><select id="priority">${["Normal","High","Critical"].map(x=>`<option ${d.priority===x?"selected":""}>${x}</option>`).join("")}</select></div><div><label>Status</label><select id="status"><option ${d.status!=="Closed"?"selected":""}>Open</option><option ${d.status==="Closed"?"selected":""}>Closed</option></select></div></div><label>Field Notes</label><textarea id="notes" placeholder="Problem found, device location, circuit, parts needed, customer impact...">${esc(d.notes||"")}</textarea><div class="defCloseoutEditor545"><label>Next Action / Parts Needed</label><textarea id="closeoutStep545" placeholder="Need lift access, replacement smoke, NAC module, return trip, customer approval...">${esc(d.closeoutStep||"")}</textarea><label>Resolution Notes</label><textarea id="resolutionNotes545" placeholder="What corrected the deficiency? Leave blank until resolved.">${esc(d.resolutionNotes||"")}</textarea><label>Customer Note</label><textarea id="customerNote545" placeholder="Short customer-facing wording for reports or closeout emails.">${esc(d.customerNote||"")}</textarea></div>${!mode?`<label><input type="checkbox" id="makeTask" checked> Create matching follow-up task</label>`:""}</div>
    ...
    <button class="primary" id="saveBtn">Save Deficiency</button><button class="ghost" id="saveAddPhotoBtn525">Save + Add Photo</button>${mode?`<button class="ghost" id="copyDefCloseout545">Copy Closeout</button><button class="ghost" id="closeWithNote545">Close With Note</button><button class="danger" id="delBtn">Delete Deficiency</button>`:""}</div></div>`);
  ...
  const copyCloseout=document.getElementById("copyDefCloseout545"); if(copyCloseout) copyCloseout.onclick=()=>copyDeficiencyCloseout545(s.id,d.id);
  const closeWithNote=document.getElementById("closeWithNote545"); if(closeWithNote) closeWithNote.onclick=()=>{ saveDeficiencyFromForm525(false); markDeficiencyClosed545(s.id,d.id); };
...
function showChangelog(){
  const notes = [
    "Advanced to Build 0.50.45 from the stable 0.50.44 baseline.",
    "Added Deficiency Closeout Polish with next-action, resolution-note, and customer-note fields.",
    "Added Copy Closeout on deficiency cards and the deficiency edit screen.",
    "Added Close With Note support and a Need Closeout count for closed deficiencies missing resolution notes.",
    "Preserved fixed splash/header behavior, Home spacing polish, Startup Health diagnostics, the 5-second splash screen, Site Quick Actions, Photo Vault tools, Customer Report Photo workflow, iPad autosizing, simple Home screen, Search Bar Concept #6, and excluded job-status workflow controls."
  ];
  const overlay=document.createElement("div");
  overlay.className="releaseOverlay";
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    <div class="releaseHead"><div><strong>FireVault</strong><span>Build ${BUILD}</span></div><button class="ghost iconBtn" id="closeRelease" aria-label="Close release notes">×</button></div>
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">Added deficiency closeout fields and copy actions for cleaner issue documentation.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
