import { BUILD, KEY, loadData, saveData, ensureSite, fullAddress, esc, uid, downloadBlob, syncSummary, syncQueue, syncConflicts, syncActivity, createSyncPackage, importSyncPackage, resolveSyncConflict, notePackageExport, deviceIdentity, recordSyncActivity, autoBackupInfo, latestAutoBackup, restoreAutoBackup, isDemoMode, setDemoMode, resetDemoData, securityFoundationSummary, securityAudit, recycleBinInfo, restoreRecycleRecord, purgeRecycleBin, recordSecurityEvent, validateVaultIntegrity } from "./storage.js?v=1.03.28";
import { backendAdapterSummary, runBackendAdapterDiagnostics, backendAdapterManifest, PROVIDER_CONTRACT_VERSION, FILE_STORAGE_CATALOG, fileStoragePlanSummary, cloudFileStorageManifest, MICROSOFT_STORAGE_TYPES, microsoftStorageAccounts, saveMicrosoftStorageAccounts, createMicrosoftStorageAccount, microsoftStorageAccountById, microsoftAppRegistration, saveMicrosoftAppRegistration, microsoftStorageSummary, microsoftStorageManifest } from "./providers.js?v=1.03.28";
import { encodePlusCode, isValidFullPlusCode, normalizePlusCode, plusCodePrecisionLabel } from "./open-location-code.js?v=1.03.28";
import { hydrateVaultMedia, stageVaultMedia, prepareVaultWithMedia, mediaStorageSummary, requestPersistentMediaStorage, pruneOrphanedMedia, flushMediaWrites } from "./media-store.js?v=1.03.28";
import { APP_PROFILE, APP_PROFILE_SCHEMA_VERSION, appTerm, appLabel, appNavigationLabel, appProfileExport } from "./app-profile.js?v=1.03.28";
import { MODULE_REGISTRY, MODULE_REGISTRY_VERSION, MODULE_CLASSIFICATIONS, FUTURE_APP_COLUMNS, moduleRegistrySummary, moduleMatrixRows, moduleRegistryExport } from "./module-registry.js?v=1.03.28";
import { MODULE_BINDINGS_VERSION, NAV_MODULE_REQUIREMENTS as NAV_MODULE_REQUIREMENTS_0955, ROUTE_MODULE_REQUIREMENTS as ROUTE_MODULE_REQUIREMENTS_0955, SETTINGS_MODULE_REQUIREMENTS as SETTINGS_MODULE_REQUIREMENTS_0955, ACCOUNT_TAB_BINDINGS, requirementsMet as moduleRequirementsMetFromBindings0955, moduleBindingsExport } from "./module-bindings.js?v=1.03.28";
import { RECORD_SCHEMA, RECORD_SCHEMA_VERSION, recordFieldById, activeRecordFields, recordFieldEnabled, recordFieldRequired, activeDetailSections, recordDetailSectionEnabled, recordPhotoCategories, recordSchemaSummary, recordSchemaExport } from "./record-schema.js?v=1.03.28";
import { WORKFLOW_SCHEMA, WORKFLOW_SCHEMA_VERSION, activeWorkflowActions, workflowActionEnabled, quickPhotoWorkflow, workflowSchemaSummary, workflowSchemaExport } from "./workflow-schema.js?v=1.03.28";
import { THEME_PROFILE_SCHEMA_VERSION, resolveThemeProfile, applyThemeProfile, themeBrandAsset, themeWordmarkMarkup, themeProfileSummary, themeProfileExport } from "./theme-profile.js?v=1.03.28";
import { CONTENT_PACK_SCHEMA_VERSION, CONTENT_SOURCE_TYPES, CONTENT_SOURCES, CONTENT_PACKS, resolveContentPackProfile, activeContentSources, activeContentPacks, contentPackLibraryFolders, contentPackSummary, contentPackRegistryExport } from "./content-pack-registry.js?v=1.03.28";
import { SYNC_STORAGE_PROFILE_SCHEMA_VERSION, STORAGE_PROVIDER_TYPES, STORAGE_PROVIDERS, STORAGE_ROLES, resolveSyncStorageProfile, activeStorageProviders, storageProviderEnabled, storageRoleProviders, syncStorageSettingsTabEnabled, syncStorageSummary, syncStorageProfileExport } from "./sync-storage-profile.js?v=1.03.28";
import { APP_FORGE_BLUEPRINT_SCHEMA_VERSION, appForgeReadinessSummary, validateAppForgeProfile, appForgeBlueprintExport } from "./app-forge-blueprint.js?v=1.03.28";
import { APP_FORGE_RECIPE_SCHEMA_VERSION, appForgeRecipes, appForgeRecipeSummary, appForgeRecipeBlueprintExport, appForgeRecipeCatalogExport } from "./app-forge-recipes.js?v=1.03.28";
import { APP_FORGE_FACTORY_SCHEMA_VERSION, createAppForgeGenerationRequest, appForgeFactoryManifest, appForgeFactorySummary, appForgeFactorySchemaExport } from "./app-forge-factory.js?v=1.03.28";
import { APP_FORGE_GENERATOR_SCHEMA_VERSION, appForgeGeneratorPlan, appForgeGeneratorSummary, generateAppForgePwaPackage, appForgeGeneratorSchemaExport } from "./app-forge-generator.js?v=1.03.28";
...
function settingsTabEnabled0955(id){
  if(id==="overlay")return moduleEnabled0955("core.photoOverlay")||moduleEnabled0955("core.photos");
  return moduleRequirementsMet0955(SETTINGS_MODULE_REQUIREMENTS_0955[id]||[])&&syncStorageSettingsTabEnabled(APP_PROFILE,id);
}
...
/* Build 1.03.24 — keep the dynamic navigation dock intact through iOS search. */
function restoreDirectoryNavigation10324(){
...
/* Build 1.03.17 — minimal first-run guide and confirmed GPS address assistance. */
const FIRST_RUN_KEY_10317="firevault_first_run_complete_10317";
const FIRST_ADDRESS_CACHE_KEY_10317="firevault_first_address_cache_10317";
let firstAccountGuide10317=false;
let demoAccountChoiceResolved10317=false;
let firstAccountLookupActive10317=false;
let firstAccountDetected10317=null;

/* Build 1.03.23 — native Apple Maps bridge with browser-safe fallback. */
const NATIVE_MAP_TIMEOUT_10323=18000;
const nativeMapPending10323=new Map();
function nativeAppleMapsAvailable10323(){return Boolean(window.webkit?.messageHandlers?.fireVaultMaps?.postMessage);}
function nativeMapRequest10323(action,payload={},timeout=NATIVE_MAP_TIMEOUT_10323){
  if(!nativeAppleMapsAvailable10323())return Promise.reject(new Error("Native Apple Maps is unavailable."));
  const requestId=`map-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`;
  return new Promise((resolve,reject)=>{
    const timer=setTimeout(()=>{nativeMapPending10323.delete(requestId);reject(new Error("Apple Maps did not respond in time."));},timeout);
    nativeMapPending10323.set(requestId,{resolve,reject,timer});
    try{window.webkit.messageHandlers.fireVaultMaps.postMessage({action,requestId,...payload});}
    catch(error){clearTimeout(timer);nativeMapPending10323.delete(requestId);reject(error);}
  });
}
window.fireVaultNativeMapsResolve=function(response={}){
  const pending=nativeMapPending10323.get(String(response.requestId||""));if(!pending)return;
  nativeMapPending10323.delete(String(response.requestId||""));clearTimeout(pending.timer);
  if(response.success)pending.resolve(response.payload||{});else pending.reject(new Error(response.error||"Apple Maps request failed."));
};
function nativeMapCredit10323(result={}){return String(result.source||"").startsWith("Apple")?"Address lookup · Apple Maps":"Address lookup © OpenStreetMap contributors";}
async function nativeMapSnapshot10323(element,options={}){
  if(!nativeAppleMapsAvailable10323()||!element)return false;
  const payload={action:"snapshot",lat:Number(options.lat),lng:Number(options.lng),latDelta:Number(options.latDelta)||.012,lngDelta:Number(options.lngDelta)||.012,width:Math.round(Number(options.width)||element.clientWidth||420),height:Math.round(Number(options.height)||element.clientHeight||260)};
  const key=JSON.stringify(payload);if(element.dataset.appleMapKey10323===key)return true;element.dataset.appleMapKey10323=key;
  try{const result=await nativeMapRequest10323("snapshot",payload,22000);if(element.dataset.appleMapKey10323!==key)return false;element.src=result.imageDataUrl;element.dataset.mapProvider="apple";return true;}
  catch{if(element.dataset.appleMapKey10323===key)element.removeAttribute("data-apple-map-key10323");return false;}
}
function hydrateNativeMapElements10323(root=document){
  if(!nativeAppleMapsAvailable10323())return;
  root.querySelectorAll?.("[data-apple-map10323]").forEach(element=>nativeMapSnapshot10323(element,{lat:element.dataset.mapLat,lng:element.dataset.mapLng,latDelta:element.dataset.mapLatDelta,lngDelta:element.dataset.mapLngDelta,width:element.clientWidth,height:element.clientHeight}));
}
function nativeRoute10323(lat,lng,name="Location",fallbackUrl=""){
  const latitude=Number(lat),longitude=Number(lng);
  if(nativeAppleMapsAvailable10323()&&Number.isFinite(latitude)&&Number.isFinite(longitude)){
    nativeMapRequest10323("route",{lat:latitude,lng:longitude,name:String(name||"Location")}).catch(()=>{if(fallbackUrl)window.open(fallbackUrl,"_blank","noopener");});return;
  }
  if(fallbackUrl)window.open(fallbackUrl,"_blank","noopener");
}

/* Build 1.03.25 — native Apple VisionKit document scanner bridge. */
const NATIVE_SCANNER_TIMEOUT_10325=15*60*1000;
const nativeScannerPending10325=new Map();
function nativeDocumentScannerAvailable10325(){return Boolean(window.webkit?.messageHandlers?.fireVaultScanner?.postMessage);}
function nativeDocumentScanRequest10325(){
  if(!nativeDocumentScannerAvailable10325())return Promise.reject(new Error("Apple Document Scanner is unavailable."));
  const requestId=`scan-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,8)}`;
  return new Promise((resolve,reject)=>{
    const timer=setTimeout(()=>{nativeScannerPending10325.delete(requestId);reject(new Error("The document scanner session timed out."));},NATIVE_SCANNER_TIMEOUT_10325);
    nativeScannerPending10325.set(requestId,{resolve,reject,timer,pages:[]});
    try{window.webkit.messageHandlers.fireVaultScanner.postMessage({action:"scan",requestId});}
    catch(error){clearTimeout(timer);nativeScannerPending10325.delete(requestId);reject(error);}
  });
}
window.fireVaultNativeScannerPage=function(response={}){
  const pending=nativeScannerPending10325.get(String(response.requestId||""));if(!pending)return;
  const index=Number(response.index),count=Number(response.count),page=response.page||{};
  if(!Number.isInteger(index)||index<0||!String(page.imageDataUrl||"").startsWith("data:image/jpeg;base64,"))return;
  if(Number.isInteger(count)&&count>0)pending.expectedPageCount=count;
  pending.pages[index]={imageData:page.imageDataUrl,width:Number(page.width)||0,height:Number(page.height)||0,mediaMime:page.mime||"image/jpeg"};
};
window.fireVaultNativeScannerResolve=function(response={}){
  const requestId=String(response.requestId||""),pending=nativeScannerPending10325.get(requestId);if(!pending)return;
  nativeScannerPending10325.delete(requestId);clearTimeout(pending.timer);
  if(response.success){
    const expected=Number(response.payload?.pageCount)||Number(pending.expectedPageCount)||0,pages=pending.pages.filter(Boolean);
    if(!pages.length||expected!==pages.length){pending.reject(new Error("Apple Document Scanner did not return every captured page. Please scan the document again."));return;}
    pending.resolve({...response.payload,pages});return;
  }
  const error=new Error(response.error||"Apple Document Scanner could not finish.");error.cancelled=response.cancelled===true;pending.reject(error);
};

function firstRunComplete10317(){try{return localStorage.getItem(FIRST_RUN_KEY_10317)==="1";}catch{return false;}}
function completeFirstRun10317(){try{localStorage.setItem(FIRST_RUN_KEY_10317,"1");}catch{}}
function realVaultCount10317(){
  try{const value=JSON.parse(localStorage.getItem(KEY)||"null");return Array.isArray(value?.sites)?value.sites.length:0;}catch{return 0;}
}
function shouldShowFirstRun10317(){return isDemoMode()&&!firstRunComplete10317()&&realVaultCount10317()===0;}
function closeFirstRunSurface10317(surface){surface?.remove();document.body.classList.remove("fvFirstRunOpen10317");}
function firstRunSlides10317(){return [
  {icon:"D",eyebrow:"SAFE START",title:"Explore with demo accounts",body:"FireVault opens with fictional accounts so you can tap around safely. Nothing in Demo Mode changes your real customer vault.",points:["Try Search and Nearby Accounts","Open an account and review its field tools"]},
  {icon:"⌕",eyebrow:"FIND FAST",title:"Everything starts with an account",body:"Search by customer, address, Account ID, panel, or phone. Account Detail keeps notes, photos, equipment, deficiencies, and exact locations together.",points:["Use Nearby when you are in the field","Use Photo after confirming the selected account"]},
  {icon:"＋",eyebrow:"WHEN READY",title:"Add your first real account",body:"Tap Add Account. FireVault will ask before leaving Demo Mode, then GPS can suggest the current address for you to confirm.",points:["Real information never saves into the demo vault","You can always enter the address manually"]}
];}
function showFirstRunOnboarding10317(){
  if(!shouldShowFirstRun10317()||document.querySelector(".firstRunOverlay10317"))return;
  let index=0;
  const slides=firstRunSlides10317();
  const overlay=document.createElement("div");
  overlay.className="firstRunOverlay10317";
  overlay.innerHTML=`<section class="firstRunDialog10317" role="dialog" aria-modal="true" aria-labelledby="firstRunTitle10317"><div id="firstRunBody10317"></div></section>`;
  document.body.appendChild(overlay);document.body.classList.add("fvFirstRunOpen10317");
  const draw=()=>{
    const slide=slides[index],body=overlay.querySelector("#firstRunBody10317");
    body.innerHTML=`<header class="firstRunHead10317"><span>${esc(slide.icon)}</span><div><em>${esc(slide.eyebrow)}</em><h2 id="firstRunTitle10317">${esc(slide.title)}</h2></div><button class="ghost" id="skipFirstRun10317">Skip</button></header><p class="firstRunCopy10317">${esc(slide.body)}</p><div class="firstRunPoints10317">${slide.points.map(point=>`<p><i>✓</i>${esc(point)}</p>`).join("")}</div><div class="firstRunProgress10317" aria-label="Step ${index+1} of ${slides.length}">${slides.map((_,step)=>`<i class="${step===index?"active":""}"></i>`).join("")}</div><footer class="firstRunActions10317">${index?`<button class="ghost" id="firstRunBack10317">Back</button>`:"<span></span>"}<button class="primary" id="firstRunNext10317">${index===slides.length-1?"Explore Demo":"Next"}</button></footer>`;
    body.querySelector("#skipFirstRun10317").onclick=()=>{completeFirstRun10317();closeFirstRunSurface10317(overlay);};
    body.querySelector("#firstRunBack10317")?.addEventListener("click",()=>{index--;draw();});
    body.querySelector("#firstRunNext10317").onclick=()=>{if(index<slides.length-1){index++;draw();return;}completeFirstRun10317();closeFirstRunSurface10317(overlay);};
  };
  draw();
}
function scheduleFirstRunOnboarding10317(attempt=0){
  setTimeout(()=>{
    if(!shouldShowFirstRun10317())return;
    if(document.querySelector("#fvUpdateBanner072,.privacyLockOverlay0791,.releaseOverlay")){if(attempt<8)scheduleFirstRunOnboarding10317(attempt+1);return;}
    showFirstRunOnboarding10317();
  },attempt?900:650);
}
function startAccountCreation10317(){
  selectedSiteId=null;mode=null;firstAccountDetected10317=null;
  if(isDemoMode()&&!demoAccountChoiceResolved10317){showDemoAccountChoice10317();return;}
  firstAccountGuide10317=firstAccountGuide10317||!isDemoMode()&&!(data.sites||[]).length;
  route("siteForm");
}
function showDemoAccountChoice10317(){
  document.querySelector(".demoAccountChoice10317")?.remove();
  completeFirstRun10317();
  const overlay=document.createElement("div");
...
const US_STATE_CODES_10317={"alabama":"AL","alaska":"AK","arizona":"AZ","arkansas":"AR","california":"CA","colorado":"CO","connecticut":"CT","delaware":"DE","florida":"FL","georgia":"GA","hawaii":"HI","idaho":"ID","illinois":"IL","indiana":"IN","iowa":"IA","kansas":"KS","kentucky":"KY","louisiana":"LA","maine":"ME","maryland":"MD","massachusetts":"MA","michigan":"MI","minnesota":"MN","mississippi":"MS","missouri":"MO","montana":"MT","nebraska":"NE","nevada":"NV","new hampshire":"NH","new jersey":"NJ","new mexico":"NM","new york":"NY","north carolina":"NC","north dakota":"ND","ohio":"OH","oklahoma":"OK","oregon":"OR","pennsylvania":"PA","rhode island":"RI","south carolina":"SC","south dakota":"SD","tennessee":"TN","texas":"TX","utah":"UT","vermont":"VT","virginia":"VA","washington":"WA","west virginia":"WV","wisconsin":"WI","wyoming":"WY","district of columbia":"DC"};
function reverseStateCode10317(address={}){
  const iso=Object.entries(address).find(([key])=>key.startsWith("ISO3166-2-lvl"))?.[1]||"";
  const isoCode=String(iso).split("-").pop();
  if(/^[A-Z]{2}$/.test(isoCode))return isoCode;
  return US_STATE_CODES_10317[String(address.state||"").toLowerCase()]||String(address.state_code||"").toUpperCase().slice(0,2);
}
function reverseBusinessName10317(payload={},address={}){
  const category=String(payload.category||payload.class||"").toLowerCase();
  const allowed=new Set(["amenity","shop","office","tourism","leisure","craft","industrial","commercial","healthcare","historic"]);
  const addressName=address.amenity||address.shop||address.office||address.company||address.tourism||address.leisure||"";
  const candidate=String(addressName||payload.namedetails?.name||payload.name||"").trim();
  if(!candidate||!allowed.has(category)&&!addressName)return "";
  const road=String(address.road||address.pedestrian||address.highway||"").trim().toLowerCase();
  return candidate.toLowerCase()===road?"":candidate;
}
function normalizeReverseAddress10317(payload={},coords={}){
  ...
  const state=reverseStateCode10317(address);
  const zip=String(address.postcode||"").split(";")[0].trim();
  const businessName=reverseBusinessName10317(payload,address);
  const displayAddress=[street,city,state,zip].filter(Boolean).join(", ")||String(payload.display_name||"");
  return {lat:Number(coords.lat),lng:Number(coords.lng),accuracy:Number(coords.accuracy)||0,capturedAt:coords.capturedAt||new Date().toISOString(),street,city,state,zip,businessName,displayAddress,source:"OpenStreetMap Nominatim",lookedUpAt:new Date().toISOString()};
}
function firstAddressCacheKey10317(lat,lng){return `${Number(lat).toFixed(4)},${Number(lng).toFixed(4)}`;}
function readFirstAddressCache10317(lat,lng){try{const all=JSON.parse(sessionStorage.getItem(FIRST_ADDRESS_CACHE_KEY_10317)||"{}");return all[firstAddressCacheKey10317(lat,lng)]||null;}catch{return null;}}
function writeFirstAddressCache10317(result){try{const all=JSON.parse(sessionStorage.getItem(FIRST_ADDRESS_CACHE_KEY_10317)||"{}");all[firstAddressCacheKey10317(result.lat,result.lng)]=result;sessionStorage.setItem(FIRST_ADDRESS_CACHE_KEY_10317,JSON.stringify(all));}catch{}}
async function reverseCurrentAddress10317(coords){
  const cached=readFirstAddressCache10317(coords.lat,coords.lng);if(cached)return cached;
  if(nativeAppleMapsAvailable10323()){
    const result=await nativeMapRequest10323("reverse",coords);writeFirstAddressCache10317(result);return result;
  }
  const endpoint=String(data.settings?.gps?.reverseGeocodeEndpoint||"https://nominatim.openstreetmap.org/reverse");
  const url=new URL(endpoint);url.searchParams.set("format","jsonv2");url.searchParams.set("lat",coords.lat);url.searchParams.set("lon",coords.lng);url.searchParams.set("zoom","18");url.searchParams.set("addressdetails","1");url.searchParams.set("namedetails","1");url.searchParams.set("accept-language","en");
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),14000);
  try{
    const response=await fetch(url.toString(),{headers:{Accept:"application/json"},referrerPolicy:"strict-origin-when-cross-origin",signal:controller.signal});
    if(!response.ok)throw new Error(`Address service returned ${response.status}.`);
    const result=normalizeReverseAddress10317(await response.json(),coords);writeFirstAddressCache10317(result);return result;
  }finally{clearTimeout(timer);}
}
function setFirstAccountAssistStatus10317(message,state=""){
  const box=document.getElementById("firstAccountAssistStatus10317");if(!box)return;box.textContent=message;box.dataset.state=state;
}
function applyGpsFields10317(result){
  const values={gpsLat:Number(result.lat).toFixed(6),gpsLng:Number(result.lng).toFixed(6),gpsAccuracy:Math.round(result.accuracy||0),gpsCapturedAt:result.capturedAt||new Date().toISOString()};
  Object.entries(values).forEach(([id,value])=>{const input=document.getElementById(id);if(input){input.value=value;input.dispatchEvent(new Event("input",{bubbles:true}));}});
  updateAccountPlusPreview0794();
}
function applyDetectedAddress10317(result){
  const values={street:result.street,city:result.city,state:result.state,zip:result.zip};
  Object.entries(values).forEach(([id,value])=>{const input=document.getElementById(id);if(input&&value){input.value=value;input.dispatchEvent(new Event("input",{bubbles:true}));}});
  const name=document.getElementById("name");if(name&&result.businessName&&!name.value.trim()){name.value=result.businessName;name.dispatchEvent(new Event("input",{bubbles:true}));}
  applyGpsFields10317(result);firstAccountDetected10317=result;markAccountAddressConfirmed10318(result,"Current location");
  setFirstAccountAssistStatus10317(`${result.businessName?`${result.businessName} · `:""}${result.displayAddress} added. Review the fields, then complete the required account information.`,"success");
  (document.getElementById("externalAccountId0760")||document.getElementById("name"))?.focus({preventScroll:true});
}
function showDetectedAddress10317(result){
  document.querySelector(".detectedAddressOverlay10317")?.remove();
  const overlay=document.createElement("div");overlay.className="firstRunOverlay10317 detectedAddressOverlay10317";
  overlay.innerHTML=`<section class="firstRunDialog10317 detectedAddressDialog10317" role="dialog" aria-modal="true" aria-labelledby="detectedAddressTitle10317"><header><span>⌖</span><div><em>GPS ADDRESS ASSISTANT</em><h2 id="detectedAddressTitle10317">I see you are at this address</h2></div></header>${result.businessName?`<div class="detectedBusiness10317"><small>COMMERCIAL LOCATION FOUND</small><strong>${esc(result.businessName)}</strong></div>`:""}<address>${esc(result.displayAddress||"Address details were not available")}</address><p>Is that correct? Would you like to add this address?</p><div class="detectedAccuracy10317">GPS accuracy: about ${Math.max(1,Math.round(result.accuracy||0))} m · Plus Code will be generated locally</div><footer><button class="ghost" id="manualDetectedAddress10317">Enter Manually</button><button class="ghost" id="retryDetectedAddress10317">No — Try Again</button><button class="primary" id="useDetectedAddress10317">Yes — Add Address</button></footer><small class="osmCredit10317">${esc(nativeMapCredit10323(result))}</small></section>`;
  document.body.appendChild(overlay);document.body.classList.add("fvFirstRunOpen10317");
  const close=()=>closeFirstRunSurface10317(overlay);
  overlay.querySelector("#manualDetectedAddress10317").onclick=()=>{applyGpsFields10317(result);setFirstAccountAssistStatus10317("GPS is ready. Enter or correct the address manually before saving.","ready");close();document.getElementById("street")?.focus();};
  overlay.querySelector("#retryDetectedAddress10317").onclick=()=>{close();captureFirstAccountAddress10317();};
  overlay.querySelector("#useDetectedAddress10317").onclick=()=>{applyDetectedAddress10317(result);close();};
}
async function captureFirstAccountAddress10317(){
  if(firstAccountLookupActive10317)return;
  if(!nativeAppleMapsAvailable10323()&&!navigator.geolocation){setFirstAccountAssistStatus10317("GPS is unavailable here. Enter the account address manually.","error");document.getElementById("street")?.focus();return;}
  const button=document.getElementById("firstAccountGps10317");firstAccountLookupActive10317=true;setButtonBusy0781(button,true,"Finding address…");setFirstAccountAssistStatus10317("Getting your current GPS location…","working");
  try{
    const nativeCoords=nativeAppleMapsAvailable10323()?await nativeMapRequest10323("currentLocation"):await new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,gpsOptions()));
    const raw=nativeCoords?.coords||nativeCoords;
    const coords={lat:Number(Number(raw.latitude??raw.lat).toFixed(6)),lng:Number(Number(raw.longitude??raw.lng).toFixed(6)),accuracy:Math.round(raw.accuracy||0),capturedAt:raw.capturedAt||new Date().toISOString()};
    if(data.settings?.gps?.addressAssist===false){applyGpsFields10317(coords);setFirstAccountAssistStatus10317("GPS is ready. Enter the address manually, then finish the account.","ready");firstAccountLookupActive10317=false;setButtonBusy0781(button,false);return;}
    setFirstAccountAssistStatus10317("GPS found. Looking up the nearest address…","working");
    try{const result=await reverseCurrentAddress10317(coords);if(!result.street&&!result.displayAddress)throw new Error("No street address was returned.");showDetectedAddress10317(result);setFirstAccountAssistStatus10317("Address found. Confirm it in the popup.","ready");}
    catch(err){applyGpsFields10317(coords);setFirstAccountAssistStatus10317(`GPS is ready, but the address lookup was unavailable. Enter the address manually.`,"ready");document.getElementById("street")?.focus();}
  }catch(err){setFirstAccountAssistStatus10317(`GPS could not be used: ${err.message||"permission denied"}. Enter the address manually.`,"error");document.getElementById("street")?.focus();}
  finally{firstAccountLookupActive10317=false;setButtonBusy0781(button,false);}
}
/* Build 1.03.18 — unified Add Account address search, review, and confirmation. */
const ACCOUNT_ADDRESS_SEARCH_CACHE_KEY_10318="firevault_account_address_search_10318";
let accountAddressConfirmed10318=false;
let accountAddressSignature10318="";
let accountAddressResults10318=[];
let accountAddressSearchActive10318=false;
function accountAddressSignatureFromForm10318(){return ["street","city","state","zip"].map(id=>String(document.getElementById(id)?.value||"").trim().toLowerCase()).join("|");}
function setAccountAddressStatus10318(state="review",message="Review and confirm this address before creating the account."){
  const status=document.getElementById("accountAddressStatus10318"),copy=document.getElementById("accountAddressStatusCopy10318");
  if(status){status.dataset.state=state;status.querySelector("strong").textContent=state==="confirmed"?"Address confirmed":state==="working"?"Looking up address":state==="error"?"Lookup unavailable":"Confirmation needed";}
  if(copy)copy.textContent=message;
}
function updateAccountCreateSummary10318(){
  const name=String(document.getElementById("name")?.value||"").trim(),address=accountAddressSignatureFromForm10318().replaceAll("|","").trim(),id=String(document.getElementById("externalAccountId0760")?.value||"").trim();
  const ready=[Boolean(name),Boolean(address)&&accountAddressConfirmed10318].filter(Boolean).length;
  const count=document.getElementById("accountCreateReadyCount10318");if(count)count.textContent=`${ready}/2 ready`;
  const nameCheck=document.getElementById("accountCreateNameCheck10318");if(nameCheck){nameCheck.dataset.ready=String(Boolean(name));nameCheck.querySelector("span").textContent=name||"Account name needed";}
  const addressCheck=document.getElementById("accountCreateAddressCheck10318");if(addressCheck){addressCheck.dataset.ready=String(Boolean(address)&&accountAddressConfirmed10318);addressCheck.querySelector("span").textContent=address?(accountAddressConfirmed10318?"Address confirmed":"Address needs confirmation"):"Address needed";}
  const idCheck=document.getElementById("accountCreateIdCheck10318");if(idCheck)idCheck.querySelector("span").textContent=id||"Account ID can be added now or later";
  const footer=document.getElementById("accountCreateFooterStatus10318");if(footer)footer.textContent=ready===2?"Ready to create":"Complete the name and confirm the address";
}
function invalidateAccountAddress10318(){
  const signature=accountAddressSignatureFromForm10318();
  if(accountAddressConfirmed10318&&signature!==accountAddressSignature10318){accountAddressConfirmed10318=false;firstAccountDetected10317=null;setAccountAddressStatus10318("review","The address changed. Review it again before creating the account.");}
  updateAccountCreateSummary10318();
}
function markAccountAddressConfirmed10318(result={},label="Address lookup"){
  accountAddressConfirmed10318=true;accountAddressSignature10318=accountAddressSignatureFromForm10318();firstAccountDetected10317=result;
  const detail=[label,result.businessName,result.displayAddress].filter(Boolean).join(" · ");
  setAccountAddressStatus10318("confirmed",`${detail||"Address"} confirmed. You can still correct the fields; changes will require another review.`);
  updateAccountCreateSummary10318();
}
function accountSearchEndpoint10318(){
  const configured=String(data.settings?.gps?.addressSearchEndpoint||"").trim();if(configured)return configured;
  return String(data.settings?.gps?.reverseGeocodeEndpoint||"https://nominatim.openstreetmap.org/reverse").replace(/\/reverse(?:\?.*)?$/,"/search");
}
function accountAddressSearchCache10318(){try{return JSON.parse(sessionStorage.getItem(ACCOUNT_ADDRESS_SEARCH_CACHE_KEY_10318)||"{}")||{};}catch{return {};}}
function accountAddressSearchCacheKey10323(query){return `${nativeAppleMapsAvailable10323()?"apple":"web"}:${String(query).trim().toLowerCase()}`;}
function readAccountAddressSearchCache10318(query){return accountAddressSearchCache10318()[accountAddressSearchCacheKey10323(query)]||null;}
function writeAccountAddressSearchCache10318(query,results){try{const cache=accountAddressSearchCache10318(),keys=Object.keys(cache);if(keys.length>12)delete cache[keys[0]];cache[accountAddressSearchCacheKey10323(query)]=results;sessionStorage.setItem(ACCOUNT_ADDRESS_SEARCH_CACHE_KEY_10318,JSON.stringify(cache));}catch{}}
function normalizeAccountAddressCandidate10318(payload={}){
  const lat=Number(payload.lat),lng=Number(payload.lon),result=normalizeReverseAddress10317(payload,{lat,lng,accuracy:0,capturedAt:new Date().toISOString()});
  result.source="OpenStreetMap Nominatim address search";result.lookedUpAt=new Date().toISOString();result.placeType=String(payload.type||payload.category||"");return result;
}
function renderAccountAddressResults10318(results=[]){
  accountAddressResults10318=results;const box=document.getElementById("accountAddressResults10318");if(!box)return;
  box.hidden=false;box.innerHTML=results.length?`<div class="accountAddressResultsHead10318"><strong>${results.length} match${results.length===1?"":"es"}</strong><span>Choose one to review</span></div>${results.map((result,index)=>`<button type="button" class="accountAddressResult10318" data-address-result10318="${index}"><i>⌖</i><span>${result.businessName?`<em>${esc(result.businessName)}</em>`:""}<strong>${esc(result.displayAddress||[result.street,result.city,result.state,result.zip].filter(Boolean).join(", "))}</strong><small>${result.placeType?`${esc(result.placeType)} · `:""}Tap to confirm</small></span><b>Review</b></button>`).join("")}`:`<div class="accountAddressEmpty10318"><strong>No exact address found</strong><span>Add a city and state, try the business name, use current location, or enter it manually.</span></div>`;
  box.querySelectorAll("[data-address-result10318]").forEach(button=>button.onclick=()=>showAccountAddressConfirmation10318(results[Number(button.dataset.addressResult10318)]));
}
async function searchAccountAddress10318(){
  if(accountAddressSearchActive10318)return;const input=document.getElementById("accountAddressSearch10318"),button=document.getElementById("accountAddressSearchBtn10318"),query=String(input?.value||"").trim();
  if(data.settings?.gps?.addressAssist===false){setAccountAddressStatus10318("error","Address lookup is turned off in Maps & GPS settings. Enter and confirm the address manually.");document.getElementById("street")?.focus();return;}
  if(query.length<5){setAccountAddressStatus10318("error","Enter a complete street address, business name, city, or ZIP before searching.");input?.focus();return;}
  const cached=readAccountAddressSearchCache10318(query);if(cached){renderAccountAddressResults10318(cached);setAccountAddressStatus10318("review","Select a result below, then confirm it before the address is added.");return;}
  accountAddressSearchActive10318=true;setButtonBusy0781(button,true,"Searching…");setAccountAddressStatus10318("working",`Looking for “${query}”…`);
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),14000);
  try{
    let results=[];
    if(nativeAppleMapsAvailable10323()){
      const response=await nativeMapRequest10323("search",{query});results=(response.results||[]).filter(result=>result.displayAddress||result.street);
    }else{
      const url=new URL(accountSearchEndpoint10318());url.searchParams.set("format","jsonv2");url.searchParams.set("q",query);url.searchParams.set("addressdetails","1");url.searchParams.set("namedetails","1");url.searchParams.set("limit","5");url.searchParams.set("accept-language","en");
      const response=await fetch(url.toString(),{headers:{Accept:"application/json"},referrerPolicy:"strict-origin-when-cross-origin",signal:controller.signal});if(!response.ok)throw new Error(`Address service returned ${response.status}.`);
      results=(await response.json()).map(normalizeAccountAddressCandidate10318).filter(result=>result.displayAddress||result.street);
    }
    writeAccountAddressSearchCache10318(query,results);renderAccountAddressResults10318(results);setAccountAddressStatus10318("review",results.length?"Select a result below, then confirm it before the address is added.":"No exact match was returned. Refine the search or enter the address manually.");
  }catch(err){renderAccountAddressResults10318([]);setAccountAddressStatus10318("error",err?.name==="AbortError"?"Address search timed out. Try again or enter the address manually.":"Address search is unavailable. You can still enter and confirm the address manually.");}
  finally{clearTimeout(timer);accountAddressSearchActive10318=false;setButtonBusy0781(button,false);}
}
function closeAccountAddressDialog10318(overlay){overlay?.remove();document.body.classList.remove("fvFirstRunOpen10317");}
function showAccountAddressConfirmation10318(result){
  if(!result)return;document.querySelector(".accountAddressConfirmOverlay10318")?.remove();const overlay=document.createElement("div");overlay.className="firstRunOverlay10317 accountAddressConfirmOverlay10318";
  overlay.innerHTML=`<section class="firstRunDialog10317 accountAddressConfirmDialog10318" role="dialog" aria-modal="true" aria-labelledby="accountAddressConfirmTitle10318"><header><span>✓</span><div><em>ADDRESS REVIEW</em><h2 id="accountAddressConfirmTitle10318">Confirm this account address</h2></div><button type="button" class="ghost" id="closeAccountAddressConfirm10318" aria-label="Close">×</button></header>${result.businessName?`<div class="detectedBusiness10317"><small>COMMERCIAL LOCATION FOUND</small><strong>${esc(result.businessName)}</strong></div>`:""}<address>${esc(result.displayAddress||[result.street,result.city,result.state,result.zip].filter(Boolean).join(", "))}</address><div class="accountAddressConfirmFacts10318"><span><small>Street</small><strong>${esc(result.street||"Not returned")}</strong></span><span><small>City / State / ZIP</small><strong>${esc([result.city,result.state,result.zip].filter(Boolean).join(" ")||"Not returned")}</strong></span><span><small>GPS</small><strong>${Number.isFinite(result.lat)&&Number.isFinite(result.lng)?`${result.lat.toFixed(6)}, ${result.lng.toFixed(6)}`:"Not returned"}</strong></span></div><p>Use this location for the new account?</p><footer><button type="button" class="ghost" id="manualAccountAddress10318">Enter Manually</button><button type="button" class="ghost" id="backAccountAddress10318">Back to Results</button><button type="button" class="primary" id="confirmAccountAddress10318">Confirm Address</button></footer><small class="osmCredit10317">${esc(nativeMapCredit10323(result))}</small></section>`;
  document.body.appendChild(overlay);document.body.classList.add("fvFirstRunOpen10317");const close=()=>closeAccountAddressDialog10318(overlay);
  overlay.querySelector("#closeAccountAddressConfirm10318").onclick=close;overlay.querySelector("#backAccountAddress10318").onclick=close;overlay.querySelector("#manualAccountAddress10318").onclick=()=>{close();document.getElementById("street")?.focus();};
  overlay.querySelector("#confirmAccountAddress10318").onclick=()=>{applyDetectedAddress10317(result);markAccountAddressConfirmed10318(result,"Address search");close();document.getElementById("name")?.focus({preventScroll:true});};
}
function confirmManualAccountAddress10318(onConfirm){
  const address=[val("street"),val("city"),val("state").toUpperCase(),val("zip")].filter(Boolean).join(", ");document.querySelector(".accountManualConfirmOverlay10318")?.remove();const overlay=document.createElement("div");overlay.className="firstRunOverlay10317 accountManualConfirmOverlay10318";
  overlay.innerHTML=`<section class="firstRunDialog10317 accountManualConfirmDialog10318" role="dialog" aria-modal="true" aria-labelledby="manualAddressConfirmTitle10318"><header><span>⌖</span><div><em>FINAL LOCATION CHECK</em><h2 id="manualAddressConfirmTitle10318">Confirm account location</h2></div></header><address>${esc(address||"No address entered")}</address><p>This address was entered or changed manually. Is it correct for this account?</p><footer><button type="button" class="ghost" id="editManualAddress10318">Back to Edit</button><button type="button" class="primary" id="confirmManualAddress10318">Confirm & Create</button></footer></section>`;
  document.body.appendChild(overlay);document.body.classList.add("fvFirstRunOpen10317");const close=()=>closeAccountAddressDialog10318(overlay);overlay.querySelector("#editManualAddress10318").onclick=()=>{close();document.getElementById("street")?.focus();};overlay.querySelector("#confirmManualAddress10318").onclick=()=>{accountAddressConfirmed10318=true;accountAddressSignature10318=accountAddressSignatureFromForm10318();setAccountAddressStatus10318("confirmed","Manual address confirmed for this account.");updateAccountCreateSummary10318();close();onConfirm?.();};
}
...
let quickPhotoDraft0950={dataUrl:"",name:"",accountId:"",category:DEFAULT_PHOTO_CATEGORY_0956,useOverlay:QUICK_PHOTO_WORKFLOW_0957.defaultUseOverlay,useTechnicianOverlay:false,includeReport:QUICK_PHOTO_WORKFLOW_0957.defaultIncludeReport};
...
function fvIcon073(name, extraClass=""){
  const icons={
    ...
    scan:'<path d="M5 3h10l4 4v14H5z"/><path d="M15 3v5h4M8 12h8M8 16h8"/>',
...
function requestCurrentLocation0652(onSuccess,onFailure,options){
  if(nativeAppleMapsAvailable10323()){
    nativeMapRequest10323("currentLocation").then(position=>{
      const lat=finiteGpsNumber0652(position.lat,-90,90),lng=finiteGpsNumber0652(position.lng,-180,180);
      if(lat===null||lng===null){onFailure({code:2,message:"Invalid coordinates were returned."});return;}
      onSuccess({lat:Number(lat.toFixed(6)),lng:Number(lng.toFixed(6)),accuracy:Math.round(Number(position.accuracy||0)),at:position.capturedAt||new Date().toISOString(),source:"Apple Core Location"});
    }).catch(error=>onFailure({code:2,message:error.message||"Apple Core Location failed."}));
    return;
  }
...
function runNearbyScan0652(destination="nearbySites"){
  if(isDemoMode()){
    ...
    requestAnimationFrame(()=>maybeOfferArrivalGuide1020(state));
    return;
  }
  ...
  if(!nativeAppleMapsAvailable10323()&&!navigator.geolocation){
...
function mapRouteUrl071(s){
  const destination=hasGps(s)?`${s.gps.lat},${s.gps.lng}`:fullAddress(s);
  return `https://maps.apple.com/?daddr=${encodeURIComponent(destination)}&dirflg=d`;
}
function routeSite10323(s){if(!s||!hasGps(s))return;nativeRoute10323(Number(s.gps.lat),Number(s.gps.lng),s.name||"FireVault Account",mapRouteUrl071(s));}
...
function openLocationEditor07912(pointId=""){
  ...
  document.getElementById("captureLocationGps07912").onclick=async()=>{if(!nativeAppleMapsAvailable10323()&&!navigator.geolocation){toast("GPS is not available.","error");return;}toast("Capturing precise location…");try{const nativePosition=nativeAppleMapsAvailable10323()?await nativeMapRequest10323("currentLocation"):await new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,{enableHighAccuracy:true,timeout:20000,maximumAge:0}));const coords=nativePosition.coords||nativePosition;document.getElementById("locationLat07912").value=Number(Number(coords.latitude??coords.lat).toFixed(7));document.getElementById("locationLng07912").value=Number(Number(coords.longitude??coords.lng).toFixed(7));p.accuracy=Math.round(coords.accuracy||0);updatePreview();toast("Location captured.","success");}catch(err){toast(`GPS failed: ${err.message||"permission denied"}`,"error");}};
...
function routeLocationPoint071(id){ const s=site();const p=locationPoints071(s).find(x=>x.id===id);if(p)nativeRoute10323(p.lat,p.lng,p.label||p.type||"FireVault Location",`https://maps.apple.com/?daddr=${encodeURIComponent(`${p.lat},${p.lng}`)}&dirflg=d`); }
...
function accountLocationsTab07912(s){
  ...
  return `<div class="accountTabPanel0735 buildingNavigator07912">
    <section class="accountPanel0735 buildingNavigatorHero07912"><div class="accountPanelHead0735"><div><span>BUILDING NAVIGATOR</span><h2>${points.length} Saved Location${points.length===1?"":"s"}</h2><p>Find the exact entrance, panel, riser, FDC, pump, or access point.</p></div><div class="buildingNavigatorHeadActions1020"><button class="ghost" id="openArrivalGuide1020" ${preciseLocationPoints1020(s).length?"":"disabled"}>On-Site Guide</button><button class="primary" id="addLocationPoint071">＋ Add Location</button></div></div><div class="buildingNavigatorStats07912"><span><strong>${verified}</strong> Verified</span><span><strong>${needs}</strong> Review</span><span><strong>${preferred?esc(preferred.label):"Account"}</strong> Route target</span></div>${locationPlot07912(s,points)}</section>
...
async function captureGpsForSite(){
  const s=site();
  ...
  if(!nativeAppleMapsAvailable10323()&&!navigator.geolocation){ toast("GPS is not available in this browser."); return; }
  toast("Requesting GPS location...");
  try{const nativePosition=nativeAppleMapsAvailable10323()?await nativeMapRequest10323("currentLocation"):await new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,gpsOptions()));const pos=nativePosition.coords||nativePosition;
    s.gps={lat:Number(Number(pos.latitude??pos.lat).toFixed(6)),lng:Number(Number(pos.longitude??pos.lng).toFixed(6)),accuracy:Math.round(pos.accuracy || 0),capturedAt:pos.capturedAt||new Date().toISOString()};
    save(); toast("GPS saved to site."); render();
  }catch(err){toast("GPS failed: " + (err.message || "permission denied"));}
}
function updateAccountPlusPreview0794(){
  const box=document.getElementById("accountPlusPreview0794");if(!box)return;
  const latRaw=String(document.getElementById("gpsLat")?.value||"").trim(),lngRaw=String(document.getElementById("gpsLng")?.value||"").trim(),lat=Number(latRaw),lng=Number(lngRaw);
  const cfg=plusCodeSettings0794();
  const code=latRaw&&lngRaw&&Number.isFinite(lat)&&Number.isFinite(lng)?encodePlusCode071(lat,lng,cfg.accountLength):"";
...
async function captureGpsIntoForm(){
  if(!nativeAppleMapsAvailable10323()&&!navigator.geolocation){ toast("GPS is not available in this browser."); return; }
  toast("Requesting GPS location...");
  try{const nativePosition=nativeAppleMapsAvailable10323()?await nativeMapRequest10323("currentLocation"):await new Promise((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,gpsOptions()));const pos=nativePosition.coords||nativePosition;
    ...
    if(lat) lat.value=Number(Number(pos.latitude??pos.lat).toFixed(6));
    if(lng) lng.value=Number(Number(pos.longitude??pos.lng).toFixed(6));
    if(acc) acc.value=Math.round(pos.accuracy || 0);
    if(at) at.value=pos.capturedAt||new Date().toISOString();
    updateAccountPlusPreview0794();
...
/* Build 1.02.0 — foreground arrival detection and reusable on-site location guide. */
const ARRIVAL_GUIDE_SEEN_KEY_1020="firevault_arrival_guide_seen_1020";
const ARRIVAL_GUIDE_MAX_ACCURACY_1020=125;
const ARRIVAL_GUIDE_ACCESS_TYPES_1020=new Set(["Parking Area","Main Entrance","Secondary Entrance"]);
const ARRIVAL_GUIDE_PRIORITY_1020={"Parking Area":0,"Main Entrance":1,"Secondary Entrance":2,"Knox Box":3,"Key Box":4,"Fire Alarm Control Panel":5,"Annunciator":6,"Sprinkler Riser":7,"Fire Department Connection":8,"Fire Pump":9};
function preciseLocationPoints1020(s){return locationPoints071(s).filter(p=>Number.isFinite(p.lat)&&Number.isFinite(p.lng));}
function arrivalAccessPoints1020(s){return preciseLocationPoints1020(s).filter(p=>ARRIVAL_GUIDE_ACCESS_TYPES_1020.has(p.type));}
function arrivalGuidePoints1020(s){return preciseLocationPoints1020(s).sort((a,b)=>(ARRIVAL_GUIDE_PRIORITY_1020[a.type]??50)-(ARRIVAL_GUIDE_PRIORITY_1020[b.type]??50)||(a.label||"").localeCompare(b.label||""));}
function arrivalSeenSites1020(){try{const value=JSON.parse(sessionStorage.getItem(ARRIVAL_GUIDE_SEEN_KEY_1020)||"[]");return new Set(Array.isArray(value)?value.map(String):[]);}catch{return new Set();}}
function markArrivalSeen1020(siteIds=[]){try{const seen=arrivalSeenSites1020();siteIds.forEach(id=>seen.add(String(id||"")));sessionStorage.setItem(ARRIVAL_GUIDE_SEEN_KEY_1020,JSON.stringify([...seen].filter(Boolean)));}catch{}}
function arrivalThresholdMeters1020(accuracy=0){return Math.max(120,Math.min(180,Math.max(0,Number(accuracy)||0)+75));}
function arrivalCandidates1020(state){
  const lat=Number(state?.lat),lng=Number(state?.lng),accuracy=Math.max(0,Number(state?.accuracy)||0);
  if(!Number.isFinite(lat)||!Number.isFinite(lng)||accuracy>ARRIVAL_GUIDE_MAX_ACCURACY_1020)return [];
  const seen=arrivalSeenSites1020(),threshold=arrivalThresholdMeters1020(accuracy);
  return (data.sites||[]).filter(s=>hasGps(s)&&arrivalAccessPoints1020(s).length&&!seen.has(String(s.id))).map(s=>({s,meters:distanceMeters(lat,lng,Number(s.gps.lat),Number(s.gps.lng)),access:arrivalAccessPoints1020(s)})).filter(row=>row.meters<=threshold).sort((a,b)=>a.meters-b.meters||(a.s.name||"").localeCompare(b.s.name||""));
}
function closeArrivalPrompt1020(){document.getElementById("arrivalPrompt1020")?.remove();}
function arrivalAccessSummary1020(points=[]){
  const parking=points.filter(p=>p.type==="Parking Area").length,entrances=points.filter(p=>p.type==="Main Entrance"||p.type==="Secondary Entrance").length;
  return [parking?`${parking} parking point${parking===1?"":"s"}`:"",entrances?`${entrances} entrance${entrances===1?"":"s"}`:""].filter(Boolean).join(" • ")||`${points.length} access point${points.length===1?"":"s"}`;
}
function showArrivalPrompt1020(candidates=[]){
  closeArrivalPrompt1020();
  if(!candidates.length)return;
  const multiple=candidates.length>1,siteIds=candidates.map(row=>row.s.id);
  const host=document.createElement("aside");host.id="arrivalPrompt1020";host.className="arrivalPromptHost1020";
  host.innerHTML=`<section class="arrivalPromptCard1020" role="dialog" aria-modal="false" aria-labelledby="arrivalPromptTitle1020">
    <div class="arrivalPromptIcon1020" aria-hidden="true">⌖</div>
    <div class="arrivalPromptMain1020"><span>ARRIVAL DETECTED</span><h2 id="arrivalPromptTitle1020">${multiple?"Multiple accounts share this site":`You’ve arrived at ${esc(candidates[0].s.name||"this site")}`}</h2><p>${multiple?"Choose the correct account before opening exact parking and entrance guidance.":`${esc(distanceLabel(candidates[0].meters))} away • ${esc(arrivalAccessSummary1020(candidates[0].access))}`}</p>
      ${multiple?`<div class="arrivalPromptChoices1020">${candidates.slice(0,4).map(row=>`<button data-arrival-choice1020="${esc(row.s.id)}"><strong>${esc(row.s.name||"Unnamed Account")}</strong><small>${esc(accountId069(row.s)||fullAddress(row.s)||distanceLabel(row.meters))}</small><b>›</b></button>`).join("")}</div>`:""}
    </div>
    <div class="arrivalPromptActions1020"><button class="ghost" id="arrivalLater1020">Not Now</button>${multiple?"":`<button class="primary" id="arrivalOpen1020">Open Site Guide</button>`}</div>
  </section>`;
  document.body.appendChild(host);
  const dismiss=()=>{markArrivalSeen1020(siteIds);closeArrivalPrompt1020();};
  document.getElementById("arrivalLater1020").onclick=dismiss;
  document.getElementById("arrivalOpen1020")?.addEventListener("click",()=>{markArrivalSeen1020(siteIds);openArrivalGuide1020(candidates[0].s.id,"arrival");});
  host.querySelectorAll("[data-arrival-choice1020]").forEach(button=>button.onclick=()=>{markArrivalSeen1020(siteIds);openArrivalGuide1020(button.dataset.arrivalChoice1020,"arrival");});
}
function maybeOfferArrivalGuide1020(state=nearbyState){
  if(document.visibilityState!=="visible"||document.getElementById("arrivalGuideOverlay1020")||document.querySelector('.releaseOverlay,.buildingLocationOverlay07912,#fvUpdateBanner072'))return;
  const candidates=arrivalCandidates1020(state);
  if(candidates.length)showArrivalPrompt1020(candidates);
}
function arrivalGuideMapModel1020(s,points=[]){
  const rows=points.map(p=>({kind:"location",id:p.id,lat:Number(p.lat),lng:Number(p.lng)}));
  if(hasGps(s))rows.push({kind:"site",id:"site",lat:Number(s.gps.lat),lng:Number(s.gps.lng)});
  if(nearbyState&&Number.isFinite(Number(nearbyState.lat))&&Number.isFinite(Number(nearbyState.lng))&&(!hasGps(s)||distanceMeters(Number(nearbyState.lat),Number(nearbyState.lng),Number(s.gps.lat),Number(s.gps.lng))<1200))rows.push({kind:"user",id:"user",lat:Number(nearbyState.lat),lng:Number(nearbyState.lng)});
  if(!rows.length)return null;
  let minLat=Math.min(...rows.map(row=>row.lat)),maxLat=Math.max(...rows.map(row=>row.lat)),minLng=Math.min(...rows.map(row=>row.lng)),maxLng=Math.max(...rows.map(row=>row.lng));
  const centerLat=(minLat+maxLat)/2,centerLng=(minLng+maxLng)/2,cos=Math.max(.25,Math.cos(centerLat*Math.PI/180));
  const minLatSpan=.0014,minLngSpan=.0017/cos;
  const latSpan=Math.max(minLatSpan,(maxLat-minLat)*1.55),lngSpan=Math.max(minLngSpan,(maxLng-minLng)*1.55);
  minLat=centerLat-latSpan/2;maxLat=centerLat+latSpan/2;minLng=centerLng-lngSpan/2;maxLng=centerLng+lngSpan/2;
  const point=row=>({x:((row.lng-minLng)/(maxLng-minLng))*100,y:((maxLat-row.lat)/(maxLat-minLat))*100});
  return {rows:rows.map(row=>({...row,...point(row)})),bounds:{minLat,maxLat,minLng,maxLng}};
}
function arrivalGuideMap1020(s,points=[]){
  const model=arrivalGuideMapModel1020(s,points);if(!model)return "";
  const b=model.bounds,bbox=[b.minLng,b.minLat,b.maxLng,b.maxLat].map(n=>n.toFixed(7)).join("%2C"),src=`https://www.openstreetmap.org/export/embed.html?bbox=${bbox}&layer=mapnik`;
  const centerLat=(b.minLat+b.maxLat)/2,centerLng=(b.minLng+b.maxLng)/2,latDelta=b.maxLat-b.minLat,lngDelta=b.maxLng-b.minLng;
  const base=nativeAppleMapsAvailable10323()?`<img data-apple-map10323 data-map-lat="${centerLat}" data-map-lng="${centerLng}" data-map-lat-delta="${latDelta}" data-map-lng-delta="${lngDelta}" alt="Apple Maps view for ${esc(s.name||"account")}">`:`<iframe src="${esc(src)}" title="On-site map for ${esc(s.name||"account")}" loading="eager" tabindex="-1"></iframe>`;
  return `<div class="arrivalGuideMap1020">${base}<div class="arrivalGuideMapGrid1020"></div><div class="arrivalGuideMarkers1020">${model.rows.map(row=>{if(row.kind==="user")return `<span class="arrivalGuideUser1020" style="left:${row.x.toFixed(2)}%;top:${row.y.toFixed(2)}%" title="Your GPS location"></span>`;if(row.kind==="site")return `<span class="arrivalGuideSite1020" style="left:${row.x.toFixed(2)}%;top:${row.y.toFixed(2)}%" title="Account GPS"></span>`;const index=points.findIndex(p=>p.id===row.id)+1,p=points[index-1];return `<button style="left:${row.x.toFixed(2)}%;top:${row.y.toFixed(2)}%" data-arrival-map-point1020="${esc(row.id)}" aria-label="${esc(p?.label||"Saved location")}"><span>${index}</span></button>`;}).join("")}</div><div class="arrivalGuideMapLegend1020"><span><i class="you"></i>You</span><span><i class="site"></i>Account GPS</span></div></div>`;
}
function openArrivalGuide1020(siteId=selectedSiteId,source="manual"){
  const s=(data.sites||[]).find(row=>String(row.id)===String(siteId||""));if(!s){toast("Account not found.","error");return;}
  const points=arrivalGuidePoints1020(s);if(!points.length){toast("Add GPS to a saved location before opening the Site Guide.","error");return;}
  selectedSiteId=s.id;closeArrivalPrompt1020();document.getElementById("arrivalGuideOverlay1020")?.remove();
  const current=nearbyState&&Number.isFinite(Number(nearbyState.lat))&&hasGps(s)?distanceMeters(Number(nearbyState.lat),Number(nearbyState.lng),Number(s.gps.lat),Number(s.gps.lng)):null;
  const access=points.filter(p=>ARRIVAL_GUIDE_ACCESS_TYPES_1020.has(p.type));
  const overlay=document.createElement("div");overlay.id="arrivalGuideOverlay1020";overlay.className="arrivalGuideOverlay1020";
  overlay.innerHTML=`<section class="arrivalGuideSheet1020" role="dialog" aria-modal="true" aria-labelledby="arrivalGuideTitle1020">
    <header class="arrivalGuideHead1020"><div><span>ON-SITE GUIDE</span><h2 id="arrivalGuideTitle1020">${esc(s.name||"Account")}</h2><p>${esc(fullAddress(s)||"Saved account location")}</p></div><button class="ghost" id="closeArrivalGuide1020" aria-label="Close On-Site Guide">×</button></header>
    <div class="arrivalGuideStatus1020"><div><strong>${current!==null&&current<=arrivalThresholdMeters1020(nearbyState?.accuracy)?"On site":"Property map"}</strong><small>${current!==null?`${esc(distanceLabel(current))} from account GPS${nearbyState?.accuracy?` • ±${Math.round(Number(nearbyState.accuracy))} m accuracy`:""}`:`Opened from ${source==="arrival"?"arrival prompt":"Account Detail"}`}</small></div><div><strong>${points.length}</strong><small>Exact location${points.length===1?"":"s"}</small></div></div>
    ${arrivalGuideMap1020(s,points)}
    <div class="arrivalGuideSequence1020"><div class="arrivalGuideSequenceHead1020"><div><span>FIELD SEQUENCE</span><h3>${access.length?"Parking to equipment":"Saved exact locations"}</h3></div><small>Tap a map pin or use an action below.</small></div>
      <div class="arrivalGuideList1020">${points.map((p,index)=>{const away=nearbyState&&Number.isFinite(Number(nearbyState.lat))?distanceMeters(Number(nearbyState.lat),Number(nearbyState.lng),p.lat,p.lng):null;return `<article id="arrivalGuidePoint1020-${esc(p.id)}" data-arrival-guide-point1020="${esc(p.id)}"><span class="arrivalGuideNumber1020">${index+1}</span><div class="arrivalGuidePointMain1020"><em>${esc(p.type)}</em><strong>${esc(p.label)}</strong><small>${esc([p.floor?`Floor ${p.floor}`:"",p.placement,away!==null?distanceLabel(away):""].filter(Boolean).join(" • ")||"Exact saved location")}</small>${p.notes||p.description?`<p>${esc([p.description,p.notes].filter(Boolean).join(" • "))}</p>`:""}${p.plusCode?`<code>${esc(p.plusCode)}</code>`:""}</div><div class="arrivalGuidePointActions1020"><button class="ghost" data-arrival-copy1020="${esc(p.id)}">Copy</button><button class="primary" data-arrival-route1020="${esc(p.id)}">Route</button></div></article>`;}).join("")}</div>
    </div>
  </section>`;
  document.body.appendChild(overlay);
  hydrateNativeMapElements10323(overlay);
  let escape=null;const close=()=>{overlay.remove();if(escape)document.removeEventListener("keydown",escape);};
  document.getElementById("closeArrivalGuide1020").onclick=close;overlay.addEventListener("click",event=>{if(event.target===overlay)close();});
  const select=id=>{overlay.querySelectorAll("[data-arrival-guide-point1020]").forEach(row=>row.classList.toggle("active",row.dataset.arrivalGuidePoint1020===id));overlay.querySelectorAll("[data-arrival-map-point1020]").forEach(pin=>pin.classList.toggle("active",pin.dataset.arrivalMapPoint1020===id));document.getElementById(`arrivalGuidePoint1020-${id}`)?.scrollIntoView({behavior:"smooth",block:"nearest"});};
  overlay.querySelectorAll("[data-arrival-map-point1020]").forEach(button=>button.onclick=()=>select(button.dataset.arrivalMapPoint1020));
  overlay.querySelectorAll("[data-arrival-guide-point1020]").forEach(row=>row.onclick=event=>{if(!event.target.closest("button"))select(row.dataset.arrivalGuidePoint1020);});
  overlay.querySelectorAll("[data-arrival-copy1020]").forEach(button=>button.onclick=()=>{const p=points.find(row=>row.id===button.dataset.arrivalCopy1020);if(p)copyText071([p.label,p.plusCode||"",`${p.lat}, ${p.lng}`].filter(Boolean).join("\n"),"Location copied.");});
  overlay.querySelectorAll("[data-arrival-route1020]").forEach(button=>button.onclick=()=>{const p=points.find(row=>row.id===button.dataset.arrivalRoute1020);if(p)nativeRoute10323(p.lat,p.lng,p.label||p.type||"FireVault Location",`https://maps.apple.com/?daddr=${encodeURIComponent(`${p.lat},${p.lng}`)}&dirflg=d`);});
  escape=event=>{if(event.key==="Escape")close();};document.addEventListener("keydown",escape);document.getElementById("closeArrivalGuide1020")?.focus();
}
...
function dashboard068(){
  ...
  document.getElementById('addSiteBtn').onclick=startAccountCreation10317;
...
function nearbyAccountCard069(r,index){
  ...
  const selected=homeNearbySelected069===s.id;
  const distance=distanceLabel(r.meters);
  return `<article class="nearbyAccount069 category-${category} ${selected?'selected':''}" data-nearby-card069="${esc(s.id)}" data-nearby-index069="${index}" data-nearby-category070="${category}" role="button" tabindex="0" aria-current="${selected?'location':'false'}" aria-label="Open ${esc(s.name||'Unnamed Account')}, ${esc(distance)} away">
    ...
    <span class="nearbyDistance069"><b>${esc(distance)}</b><small>away</small><i class="gpsDot069"></i></span>
  </article>`;
}
function homeNearbyMapShell069(){
  return `<div class="nearbyMapShell069 staticMapShell069">
    ${nativeAppleMapsAvailable10323()?`<img id="nearbyStaticBase069" class="nearbyStaticBase069" alt="Apple Maps view of nearby accounts" aria-hidden="true">`:`<iframe id="nearbyStaticBase069" class="nearbyStaticBase069" title="Nearby accounts map" loading="eager" tabindex="-1" aria-hidden="true"></iframe>`}
...
function home(){
  ...
  html(`<div class="screen nearbyHome069">
    ...
    <section class="nearbyWorkspace069 ${homeNearbyView069}">
      ...
      <div class="nearbyListHead069"><strong><b>${rows.length}</b> ${nearbyCategoryFilter070==='all'?'account':NEARBY_CATEGORY_META_070[nearbyCategoryFilter070].label+' account'}${rows.length===1?'':'s'} within ${NEARBY_LIST_MAX_MILES_069} miles</strong><span>Nearest first</span></div>
  ...
  if(mapRoute0711) mapRoute0711.onclick=e=>{e.stopPropagation();const row=mapRow069(homeNearbySelected069);if(row)routeSite10323(row.s);};
  ...
  const openNearbyAccount10320=id=>{
    clearNearbyStreetZoom0840();
  ...
  document.querySelectorAll('[data-nearby-card069]').forEach(c=>{
    c.onclick=e=>{if(!e.target.closest('button'))openNearbyAccount10320(c.dataset.nearbyCard069);};
    c.onkeydown=e=>{
      if(e.key!=="Enter"&&e.key!==" ")return;
      e.preventDefault();
      openNearbyAccount10320(c.dataset.nearbyCard069);
    };
  });
  ...
  if(list){
    ...
    list.addEventListener('touchstart',()=>{
      ...
      list.classList.add('isScrolling10320');
      clearTimeout(nearbySnapTimer069);
      clearNearbyStreetZoom0840();
      nearbyMapPopupSite069="";
    ...
    list.addEventListener('touchend',()=>{
      ...
      else list.classList.remove('isScrolling10320');
    },{passive:true});
    list.addEventListener('touchcancel',()=>{
      nearbyTouching069=false;
      nearbyScrollActivated069=false;
      list.classList.remove('isScrolling10320');
    },{passive:true});
...
function updateNearbyMapSelection069(){
  ...
  if(selectedOverlay0712){
    ...
    const id=accountId069(row?.s),category=accountCategory070(row?.s);
    const categoryLabel=NEARBY_CATEGORY_META_070[category]?.label||'Basic';
    selectedOverlay0712.innerHTML=row?`<small class="nearbySelectedKicker10320">Selected • ${esc(distanceLabel(row.meters))} away</small><strong>${esc(row.s.name||'Unnamed Account')}</strong><span>${esc(fullAddress(row.s)||'No address saved')}</span><em>${id?`${esc(id)} • `:''}${esc(categoryLabel)}</em>`:'';
  }
...
function selectNearby069(siteId,fromList=false,showPopup=false,streetFocus=false){
  ...
  document.querySelectorAll('[data-nearby-card069]').forEach(c=>{
    const selected=c.dataset.nearbyCard069===siteId;
    c.classList.toggle('selected',selected);
    c.setAttribute('aria-current',selected?'location':'false');
  });
  const row=mapRow069(siteId);
...
function drawStaticNearbyMap069(){
  try{
    ...
    if(nativeAppleMapsAvailable10323())nativeMapSnapshot10323(base,{lat:mapLat,lng:mapLng,latDelta:b.north-b.south,lngDelta:b.east-b.west,width:shell.clientWidth,height:shell.clientHeight});
    else if(base.dataset.mapSrc!==nextSrc){base.dataset.mapSrc=nextSrc;base.src=nextSrc;}
    ...
    overlay.querySelectorAll('[data-static-marker069]').forEach(m=>m.onclick=e=>{
      ...
      if(card){
        ...
        const list=document.getElementById('nearbyCards069');
        if(list){
          prepareNearbyScrollTail069(list);
          const target=Math.min(Math.max(0,list.scrollHeight-list.clientHeight),nearbyCardTop069(list,card));
          requestAnimationFrame(()=>{list.scrollTop=target;requestAnimationFrame(()=>nearbyScrollLock069=false);});
        }else nearbyScrollLock069=false;
      }
    });
    updateNearbyMapSelection069();
  }catch(err){
...
function accountsSortLabel0760(){
  return ({az:"A-Z",favorites:"Favorites",recent:"Recently opened",attention:"Priority"})[accountsSort0760]||"A-Z";
}
...
function settleAccountsList0796(list){
  ...
  if(!card)return;
  // Keep every card compact and make one native-position correction only
...
function sites(){
  ...
  const accountSearchIndex10319=new Map(accounts.map(account=>[account.id,siteSearchBlob(account)]));
  sitesFilter0736="all";
  ...
  const openAdd=startAccountCreation10317;
  ...
  let appliedSearchQuery10319=siteSearch.toLowerCase();
  const applySearch=()=>{
    siteSearch=(searchEl?.value||"").trim();
    const query=siteSearch.toLowerCase();
    const queryTerms=query.split(/\s+/).filter(Boolean);
    const queryChanged=query!==appliedSearchQuery10319;
    appliedSearchQuery10319=query;
    let shown=0;
    list?.querySelectorAll("[data-account-card0759]").forEach(card=>{
      const searchText=accountSearchIndex10319.get(card.dataset.id)||card.dataset.search||"";
      const visible=!queryTerms.length||queryTerms.every(term=>searchText.includes(term));
      card.hidden=!visible;
      if(visible)shown++;
    });
    ...
    restoreDirectoryNavigation10324();
    persistAccountsViewState0761();
  };
  // Filter synchronously so iOS search, dictation, paste, and hardware-keyboard
  // input all produce visible feedback without depending on a delayed timer.
  searchEl?.addEventListener("input",applySearch);
  searchEl?.addEventListener("change",applySearch);
  searchEl?.addEventListener("compositionend",applySearch);
  ...
  list?.addEventListener("pointerdown",()=>{if(document.activeElement===searchEl)searchEl.blur();},{passive:true});
...
function accountMapPreview0735(s={}){
  ...
  const base=nativeAppleMapsAvailable10323()?`<img data-apple-map10323 data-map-lat="${lat}" data-map-lng="${lng}" data-map-lat-delta="${d*2}" data-map-lng-delta="${d*2}" alt="Apple Maps preview for ${esc(s.name||"account")}">`:`<iframe src="${esc(src)}" title="Map preview for ${esc(s.name||"account")}" loading="lazy" tabindex="-1"></iframe>`;
  return `<div class="accountMapPreview0735">${base}<span class="accountMapPin0735" aria-hidden="true"></span><button id="accountMapRoute0735">Route</button></div>`;
}
...
function revealActiveAccountTab1014(){
  const rail=document.querySelector(".accountTabs0952");
  ...
  if(activeRect.left>=railRect.left+4&&activeRect.right<=railRect.right-4)return;
...
function siteDetail(){
  ...
  const tagMarkup=accountTagChips0737(s,3);
  const detailActionIds=new Set(workflowActions0957("detailPrimary").map(action=>action.id));
  const nativeScanReady10328=nativeDocumentScannerAvailable10325()&&moduleEnabled0955("core.files");
  const essentialCount10328=Math.max(1,(nativeScanReady10328?1:0)+(detailActionIds.has("call")?1:0)+(detailActionIds.has("route")?1:0));
  const tabIcons1011={overview:"home",details:"list",locations:"map",equipment:"tools",docs:"library",notes:"note"};
  const tabByKey10328=new Map(tabs.map(item=>[item[0],item]));
  const accordionOrder10328=["overview","locations","equipment","docs","notes","details"];
  const accordionLabels10328={overview:"Overview",locations:"Map & Locations",equipment:"Equipment",docs:"Files & Scans",notes:"Notes",details:"Account Info"};
  const accordionMeta10328={overview:"Contact and recent activity",locations:hasGps(s)?"GPS and arrival points":"Add a map location",equipment:`${equipment.length} saved`,docs:`${docs.length} saved`,notes:s.notes?"Notes available":"No notes yet",details:"System, access, and identifiers"};
  const accordionSections10328=accordionOrder10328.flatMap(key=>tabByKey10328.has(key)?[[key,accordionLabels10328[key]||tabByKey10328.get(key)[1]]]:[]);
  if(!accordionSections10328.some(([key])=>key===accountDetailTab0735))accountDetailTab0735="overview";
  const accordionMarkup10328=accordionSections10328.map(([key,label])=>{const active=accountDetailTab0735===key;return `<section class="accountAccordionItem10328 ${active?"active":""}"><button class="accountAccordionToggle10328" data-account-tab0735="${key}" aria-expanded="${active?"true":"false"}">${fvIcon073(tabIcons1011[key]||"list","accountAccordionIcon10328")}<span><strong>${esc(label)}</strong><small>${esc(accordionMeta10328[key]||"")}</small></span><b aria-hidden="true">${active?"⌃":"⌄"}</b></button>${active?`<div class="accountAccordionPanel10328">${panelMarkup}</div>`:""}</section>`;}).join("");
  html(`<div class="screen accountDetail0871 accountDetail0952 accountDetail1011">
    <section class="accountDetailHeader0952 accountCompactHeader10327 category-${category}">
      ...
      <div class="accountIdentity0871 accountIdentity0952 accountCompactIdentity10327">
      ...
      <section class="accountEssentialActions10328" style="--account-essential-count:${essentialCount10328}" aria-label="Account essentials">
        ${nativeScanReady10328?`<button class="primary" id="accountScanDocument10326" aria-label="Scan a document for ${esc(s.name||"this account")}">${fvIcon073("scan","accountEssentialIcon10328")}<strong>Scan</strong></button>`:""}
    ...
    <div class="accountDetailContent0871 accountDetailContent0952 accountAccordion10328">${accordionMarkup10328}</div>
  </div>`);
  hydrateNativeMapElements10323(document);
  ...
  document.getElementById("detailRoute0871")?.addEventListener("click",()=>routeSite10323(s));
  document.getElementById("accountScanDocument10326")?.addEventListener("click",()=>startNativeDocumentScan10325(s,{returnRoute:"siteDetail"}));
  ...
  document.getElementById("navigateBtn477")?.addEventListener("click",()=>routeSite10323(s));
  document.getElementById("accountMapRoute0735")?.addEventListener("click",()=>routeSite10323(s));
  ...
  document.getElementById("openPrimaryPlus0794")?.addEventListener("click",()=>{if(hasGps(s))nativeRoute10323(Number(s.gps.lat),Number(s.gps.lng),s.name||"FireVault Account",`https://maps.apple.com/?ll=${encodeURIComponent(`${s.gps.lat},${s.gps.lng}`)}&q=${encodeURIComponent(s.name||sitePlusCode071(s)||"FireVault Account")}`);});
  ...
  document.getElementById("openArrivalGuide1020")?.addEventListener("click",()=>openArrivalGuide1020(s.id,"manual"));
...
function photoOverlayCategory1034(d={}){return document.getElementById("docPhotoCategory524")?selectedPhotoCategory524():(photoCategory524(d)||"");}
function photoTechnicianOverlaySelected1034(d={}){const input=document.getElementById("docUseTechnicianOverlay1034");return input?input.checked:d.useTechnicianOverlayOnSave===true;}
function setPhotoCategory524(cat){
  ...
  if(document.getElementById("docOverlayCanvas0890"))showDocOverlayPreview513({photoCategory:cat,imageData:docPhotoDraftDataUrl512});
}
...
function docPhotoPreviewMarkup512(d={}){
  ...
  const useTechnicianOverlay = d.useTechnicianOverlayOnSave === true;
  return `<div class="docPhotoManager512 docPhotoManager513 docPhotoManager524">
    ...
    <div class="photoQuickLayout524">
      ...
      <div class="photoNotesPanel524">
        ...
        <label class="checkRow inlineCheck photoOverlayToggle524"><input type="checkbox" id="docUseOverlay524" ${useOverlay?"checked":""}> Use Photo Overlay for this photo</label>
        <label class="checkRow inlineCheck photoOverlayToggle524 technicianPhotoToggle1034"><input type="checkbox" id="docUseTechnicianOverlay1034" ${useTechnicianOverlay?"checked":""}> Add Technician Overlay</label>
        ...
        <p class="fieldNote">Technician Overlay uses the separately saved fields from Settings → Photo Overlay. It does not change the normal Photo Overlay layout.</p>
      </div>
    </div>
...
async function showDocOverlayPreview513(d={}){
  ...
  if(!canvas) return;
  try{
    const composed=await renderOverlayComposite0890(source,overlayCleanSetting510(data.settings.overlay||{}),site()||{},1200,{category:photoOverlayCategory1034(d),showTechnician:photoTechnicianOverlaySelected1034(d)});
...
async function downloadPhotoWithOverlay512(doc={}){
  ...
  try{
    ...
    const canvas=await renderOverlayComposite0890(source,set,s||{},1800,{category:photoOverlayCategory1034(doc),showTechnician:photoTechnicianOverlaySelected1034(doc)});
    canvas.toBlob(blob=>{
...
function wireDocPhotoControls512(d={}){
  ...
  document.getElementById("docUseTechnicianOverlay1034")?.addEventListener("change",()=>{if(document.getElementById("docOverlayCanvas0890"))showDocOverlayPreview513({photoCategory:selectedPhotoCategory524(),imageData:docPhotoDraftDataUrl512||d.imageData});});
...
function siteDocs(){
  ...
  html(`<div class="screen docsScreen docsScreen512 docsScreen516"><div class="row"><button class="back ghost" id="backBtn">←</button><div><h1>Documents / Photos</h1><p>${esc(s.name||"Site")}</p></div><button class="primary" id="addDocBtn">＋</button></div>
    <div class="card docsHero docsHero516 docsHero521"><h2>Account Documents / Photo Vault</h2><p>Keep customer-specific references, links, field photos, and scanned paperwork stored with this account.</p><div class="docHeroActions523 docHeroActions0800 ${nativeDocumentScannerAvailable10325()?"hasNativeScanner10325":""}">${nativeDocumentScannerAvailable10325()?`<button class="primary nativeScanButton10325" id="scanDocumentBtn10325">▤ Scan Document</button>`:""}<button class="${nativeDocumentScannerAvailable10325()?"ghost":"primary"}" id="addPhotoBtn523">＋ Add Photo</button><button class="ghost" id="addRegularDocBtn523">＋ Add Document / Link</button></div>${nativeDocumentScannerAvailable10325()?`<div class="nativeScanNote10325"><strong>Apple Document Scanner</strong><span>Automatic edges, crop correction, multi-page capture, and offline account storage.</span></div>`:""}<div class="docStats docStats0800"><span><strong>${docs.length}</strong>Total</span><span><strong>${scans}</strong>Scans</span><span><strong>${photos}</strong>Photos</span><span><strong>${linked}</strong>Links</span></div><details class="docOrganize10326"><summary><span>Find or organize saved items</span><small>Search · filters · sort</small></summary><div>${docVaultSearchBar521()}${docVaultSortControls522()}${docVaultFilterBar516(docs)}</div></details></div>
  ...
  document.getElementById("scanDocumentBtn10325")?.addEventListener("click",()=>startNativeDocumentScan10325(s));
...
function quickPhotoStart0950(accountId=""){
  ...
  quickPhotoDraft0950={dataUrl:"",name:"",accountId:account.id,category:categories.includes(rememberedCategory)?rememberedCategory:fallbackCategory,useOverlay:Boolean(QUICK_PHOTO_WORKFLOW_0957.defaultUseOverlay),useTechnicianOverlay:false,includeReport:Boolean(QUICK_PHOTO_WORKFLOW_0957.defaultIncludeReport)};
...
function quickPhotoReviewMarkup0950(){
  ...
  const optionRows=[
    ...
    QUICK_PHOTO_WORKFLOW_0957.showOverlayToggle?`<label class="checkRow"><input type="checkbox" id="quickPhotoOverlayToggle0950" ${quickPhotoDraft0950.useOverlay?"checked":""}> <span>Show Photo Overlay</span></label>`:"",
    moduleEnabled0955("core.photoOverlay")?`<label class="checkRow technicianPhotoToggle1034"><input type="checkbox" id="quickPhotoTechnicianOverlay1034" ${quickPhotoDraft0950.useTechnicianOverlay?"checked":""}> <span>Add Technician Overlay</span></label>`:"",
...
function quickPhotoShowReview0950(){
  ...
  document.getElementById("quickPhotoTechnicianOverlay1034")?.addEventListener("change",event=>{quickPhotoDraft0950.useTechnicianOverlay=event.target.checked;quickPhotoRenderPreview0950();});
...
async function quickPhotoRenderPreview0950(){
  ...
  const token=quickPhotoDraft0950.dataUrl+quickPhotoDraft0950.accountId+quickPhotoDraft0950.category+String(quickPhotoDraft0950.useOverlay)+String(quickPhotoDraft0950.useTechnicianOverlay);
  const hasOverlay=quickPhotoDraft0950.useOverlay||quickPhotoDraft0950.useTechnicianOverlay;
  canvas.dataset.renderToken=token;if(status){status.hidden=false;status.textContent=hasOverlay?`Building ${recordLower} overlay…`:"Preparing photo…";}
  try{
    const composed=hasOverlay?await renderOverlayComposite0890(quickPhotoDraft0950.dataUrl,overlayCleanSetting510(data.settings.overlay||{}),account||{},1200,{category:quickPhotoDraft0950.category,showMain:quickPhotoDraft0950.useOverlay,showTechnician:quickPhotoDraft0950.useTechnicianOverlay}):await loadImage512(quickPhotoDraft0950.dataUrl);
...
/* Build 1.03.25 — Apple VisionKit capture restored; existing scan reading and PDF export retained. */
function docIsScan0800(d={}){
  return d.isScannedDocument===true && Array.isArray(d.scanPages) && d.scanPages.length>0;
}
function nativeScanDefaultTitle10325(){
  return `Scanned Document — ${new Date().toLocaleDateString([], {year:"numeric",month:"short",day:"numeric"})}`;
}
async function startNativeDocumentScan10325(account=site(),options={}){
  if(!account){toast("Open an account before scanning.","error");return;}
  if(!nativeDocumentScannerAvailable10325()){toast("Apple Document Scanner is available in the native iPhone app.","error");return;}
  const button=document.getElementById("scanDocumentBtn10325");setButtonBusy0781(button,true,"Opening Apple Scanner…");
  try{
    const result=await nativeDocumentScanRequest10325();
    setButtonBusy0781(button,false);
    showNativeScanReview10325(account,result.pages||[],result.source||"Apple VisionKit Document Camera",options);
  }catch(error){
    setButtonBusy0781(button,false);
    if(!error?.cancelled)toast(error?.message||"Document scanning failed.","error");
  }
}
function showNativeScanReview10325(account,pages,source,options={}){
  const validPages=(pages||[]).filter(page=>String(page?.imageData||"").startsWith("data:image/jpeg;base64,"));
  if(!validPages.length){toast("No scanned pages were returned.","error");return;}
  const types=["Panel Manual","Permit","Inspection Form","Monitoring Account","Contract","Map / Drawing","Code Reference","Other"];
  const overlay=document.createElement("div");overlay.className="nativeScanReviewOverlay10325";
  overlay.innerHTML=`<section class="nativeScanReviewSheet10325" role="dialog" aria-modal="true" aria-labelledby="nativeScanReviewTitle10325">
    <header><div><span>APPLE DOCUMENT SCANNER</span><h2 id="nativeScanReviewTitle10325">Save ${validPages.length}-Page Scan</h2><p>${esc(account.name||"Account")} · Pages remain in capture order</p></div><button class="ghost" id="closeNativeScan10325" aria-label="Discard scanned pages">×</button></header>
    <div class="nativeScanPages10325">${validPages.map((page,index)=>`<article><img src="${esc(page.imageData)}" alt="Scanned page ${index+1}"><span>Page ${index+1}</span></article>`).join("")}</div>
    <div class="nativeScanFields10325"><label><span>Document title</span><input id="nativeScanName10325" value="${esc(nativeScanDefaultTitle10325())}" autocomplete="off"></label><div><label><span>Document type</span><select id="nativeScanType10325">${types.map(type=>`<option value="${esc(type)}">${esc(type)}</option>`).join("")}</select></label><label><span>Date / revision</span><input id="nativeScanDate10325" value="${esc(new Date().toLocaleDateString())}"></label></div><label><span>Notes <small>optional</small></span><textarea id="nativeScanNotes10325" rows="3" placeholder="Panel, drawing set, inspection date, or other field detail…"></textarea></label></div>
    <div class="nativeScanStorage10325"><b>Stored offline with this account</b><span>Apple corrected the page edges and perspective. FireVault will protect the pages in on-device media storage and keep PDF download and sharing available.</span></div>
    <footer><button class="ghost" id="discardNativeScan10325">Discard</button><button class="primary" id="saveNativeScan10325">Save Scan</button></footer>
  </section>`;
  document.body.appendChild(overlay);document.body.classList.add("nativeScanReviewOpen10325");
  const close=()=>{overlay.remove();document.body.classList.remove("nativeScanReviewOpen10325");};
  overlay.addEventListener("click",event=>{if(event.target===overlay&&confirm("Discard these scanned pages?"))close();});
  document.getElementById("closeNativeScan10325").onclick=()=>{if(confirm("Discard these scanned pages?"))close();};
  document.getElementById("discardNativeScan10325").onclick=()=>{if(confirm("Discard these scanned pages?"))close();};
  document.getElementById("saveNativeScan10325").onclick=async()=>{
    const title=String(document.getElementById("nativeScanName10325")?.value||"").trim();
    if(!title){toast("Enter a document title before saving.","error");document.getElementById("nativeScanName10325")?.focus();return;}
    const saveButton=document.getElementById("saveNativeScan10325");setButtonBusy0781(saveButton,true,"Saving scan…");
    const now=new Date().toISOString(),target=fileStorageTarget0794("document");
    const record={id:uid(),type:val("nativeScanType10325")||"Panel Manual",title,date:val("nativeScanDate10325"),notes:raw("nativeScanNotes10325"),ref:"",url:"",createdAt:now,updatedAt:now,isScannedDocument:true,scanCreatedAt:now,scanSource:source||"Apple VisionKit Document Camera",scanPageCount:validPages.length,scanPages:validPages.map((page,index)=>({id:uid(),imageData:page.imageData,width:Number(page.width)||0,height:Number(page.height)||0,mediaMime:page.mediaMime||"image/jpeg",mediaStorage:"indexeddb",capturedAt:now,source:"Apple VisionKit",pageNumber:index+1})),storageTargetId:`${target.provider||"local"}:document`,storageProvider:target.provider||"local",storageFolder:target.folder||"FireVault/Documents",storageStatus:(target.provider||"local")==="local"?"local":"pending",includeInCustomerReport:false};
    account.docs=Array.isArray(account.docs)?account.docs:[];account.docs.unshift(record);
    try{
      const stored=await stageVaultMedia(data);if(stored.failed)throw new Error("One or more scanned pages could not be protected in device storage.");
      save();close();toast(`${validPages.length}-page scan saved to ${account.name||"account"}.`,"success");route(options.returnRoute==="siteDetail"?"siteDetail":"siteDocs");
    }catch(error){
      account.docs=account.docs.filter(item=>item.id!==record.id);pruneOrphanedMedia(data).catch(()=>{});setButtonBusy0781(saveButton,false);toast(error?.message||"The scanned document could not be saved.","error");
    }
  };
}
...
function contactsList(){
  ...
  html(`<div class="screen contactsScreen"><div class="row fieldListHeader10316"><button class="back ghost" id="backBtn">←</button><div><h1>Contacts & Access</h1><p>${esc(s.name||"Site")}</p></div><button class="primary" id="addContactBtn" aria-label="Add contact">＋</button></div>
...
function contactForm(){
  ...
  html(`<div class="screen fieldForm10315 contactFieldForm10315"><div class="row fieldFormHeader10315 fieldFormTop10315"><button class="back ghost" id="backBtn">←</button><h1>${mode?"Edit":"Add"} Contact</h1></div><div class="form grow fieldFormBody10315">
    <div class="card contactFormCard fieldFormCard10315"><div class="compactField"><div><label>Type</label><select id="contactType">${types.map(x=>`<option value="${esc(x)}" ${((c.type||"Customer")===x)?"selected":""}>${esc(x)}</option>`).join("")}</select></div><div><label>Role / Title</label><input id="contactRole" value="${esc(c.role||"")}" placeholder="Manager, owner, security"></div></div>
    ...
    <div class="fieldFormActions10315 ${mode?"hasDelete10315":""}"><button class="primary" id="saveContactBtn">Save Contact</button>${mode?`<button class="danger" id="deleteContactBtn">Delete Contact</button>`:""}</div>
  </div></div>`);
...
function equipmentList(){
  ...
  html(`<div class="screen equipmentScreen"><div class="row fieldListHeader10316"><button class="back ghost" id="backBtn">←</button><div><h1>Equipment Vault</h1><p>${esc(s.name||"Site")}</p></div><button class="primary" id="addEquipmentBtn" aria-label="Add equipment">＋</button></div>
...
function equipmentForm(){
  ...
  html(`<div class="screen fieldForm10315 equipmentFieldForm10315"><div class="row fieldFormHeader10315 fieldFormTop10315"><button class="back ghost" id="backBtn">←</button><h1>${mode?"Edit":"Add"} Equipment</h1></div><div class="form grow fieldFormBody10315">
    <div class="card equipmentFormCard fieldFormCard10315"><div class="compactField"><div><label>Type</label><select id="eqType">${types.map(x=>`<option value="${esc(x)}" ${e.type===x?"selected":""}>${esc(x)}</option>`).join("")}</select></div><div><label>Status</label><select id="eqStatus">${statuses.map(x=>`<option value="${esc(x)}" ${((e.status||"Active")===x)?"selected":""}>${esc(x)}</option>`).join("")}</select></div></div>
...
function siteForm(){
  ...
  const guidedNewAccount10317=!editing&&(firstAccountGuide10317||(!isDemoMode()&&!(data.sites||[]).length));
  const s=editing ? site() : {};
  if(editing && !s){route("sites");return;}
  if(!editing){accountAddressConfirmed10318=false;accountAddressSignature10318="";accountAddressResults10318=[];firstAccountDetected10317=null;}
  ...
  const createLocationMarkup10318=locationFields.length?`<section class="card accountFormCard0760 accountAddressWorkspace10318" data-record-group="location"><div class="accountFormSectionTitle0760"><span>2</span><div><strong>Find and confirm the location</strong><small>Search a business or street address, use current GPS, or enter it manually.</small></div></div>
        <div class="accountAddressFinder10318">
          <label for="accountAddressSearch10318">Business or street address</label>
          <div class="accountAddressSearchRow10318"><input id="accountAddressSearch10318" type="search" placeholder="Business, street, city, state, or ZIP" autocomplete="off" autocapitalize="words" spellcheck="false"><button type="button" class="primary" id="accountAddressSearchBtn10318">Search</button></div>
          <div class="accountAddressFinderActions10318">${fieldOn("gps")?`<button type="button" class="ghost" id="firstAccountGps10317">⌖ Use Current Location</button>`:""}<button type="button" class="ghost" id="firstAccountManual10317">✎ Enter Manually</button></div>
          <p id="firstAccountAssistStatus10317" aria-live="polite">Address lookup starts only after you tap Search or Use Current Location.</p>
        </div>
        <div class="accountAddressResults10318" id="accountAddressResults10318" hidden></div>
        <div class="accountAddressStatus10318" id="accountAddressStatus10318" data-state="review"><i>!</i><div><strong>Confirmation needed</strong><span id="accountAddressStatusCopy10318">Choose a lookup result or enter the address below, then confirm it before creating the account.</span></div></div>
        <div class="accountManualAddressHead10318"><span>CONFIRMED OR MANUAL ADDRESS</span><button type="button" class="ghost" id="clearAccountAddress10318">Clear</button></div>
        ${fieldOn("street")?`<label>${esc(recordFieldById("street")?.label||"Street Address")}${requiredBadge("street")}</label><input id="street" value="${esc(s.street||"")}" placeholder="Street number and name" autocomplete="street-address">`:""}
        ${(fieldOn("city")||fieldOn("state"))?`<div class="compactField">${fieldOn("city")?`<div><label>${esc(recordFieldById("city")?.label||"City")}${requiredBadge("city")}</label><input id="city" value="${esc(s.city||"")}" autocomplete="address-level2"></div>`:""}${fieldOn("state")?`<div><label>${esc(recordFieldById("state")?.label||"State")}${requiredBadge("state")}</label><input id="state" value="${esc(s.state||"")}" maxlength="2" autocapitalize="characters" autocomplete="address-level1"></div>`:""}</div>`:""}
        ${fieldOn("zip")?`<div class="accountZipField10318"><label>${esc(recordFieldById("zip")?.label||"ZIP")}${requiredBadge("zip")}</label><input id="zip" value="${esc(s.zip||"")}" inputmode="numeric" autocomplete="postal-code"></div>`:""}
        ${fieldOn("gps")?`<details class="accountLocationAdvanced10318"><summary><span>Advanced location data</span><em>GPS + Plus Code</em></summary><div class="accountGpsFields0760"><div><label>Latitude</label><input id="gpsLat" inputmode="decimal" value=""></div><div><label>Longitude</label><input id="gpsLng" inputmode="decimal" value=""></div><input id="gpsAccuracy" type="hidden" value=""><input id="gpsCapturedAt" type="hidden" value=""></div><div class="accountPlusPreview0794" id="accountPlusPreview0794"></div></details>`:""}
      </section>`:"";
  const systemFieldsInner10318=`${(fieldOn("panelManufacturer")||fieldOn("panelModel"))?`<div class="compactField">${fieldOn("panelManufacturer")?`<div><label>${esc(recordFieldById("panelManufacturer")?.label||"Panel Make")}${requiredBadge("panelManufacturer")}</label><input id="pm" value="${esc(s.panelManufacturer||"")}" placeholder="Notifier, EST, Siemens…"></div>`:""}${fieldOn("panelModel")?`<div><label>${esc(recordFieldById("panelModel")?.label||"Panel Model")}${requiredBadge("panelModel")}</label><input id="model" value="${esc(s.panelModel||"")}"></div>`:""}</div>`:""}${fieldOn("notes")?`<label>${esc(recordFieldById("notes")?.label||"Site Notes")}${requiredBadge("notes")}</label><textarea id="notes" placeholder="Access details, panel location, recurring issues…">${esc(s.notes||"")}</textarea>`:""}`;
  const systemMarkup=systemFields.length?(editing?`<section class="card accountFormCard0760" data-record-group="fireSystem"><div class="accountFormSectionTitle0760"><span>${locationFields.length?3:2}</span><div><strong>${esc(fireProfileVisible?(RECORD_SCHEMA.groups.find(group=>group.id==="fireSystem")?.label||"Fire Alarm System"):"Record Notes")}</strong><small>${fireProfileVisible?`Optional panel information helps technicians identify the ${recordLower} quickly.`:`Keep reusable notes connected to this ${recordLower}.`}</small></div></div>
        ${systemFieldsInner10318}</section>`:`<details class="card accountOptionalCard10318" data-record-group="fireSystem"><summary><span>3</span><div><strong>${esc(fireProfileVisible?"Fire alarm details":"Additional notes")}</strong><small>${fireProfileVisible?"Panel information and site notes can be added now or later.":`Keep reusable notes connected to this ${recordLower}.`}</small></div><em>Optional</em></summary><div class="accountOptionalBody10318">${systemFieldsInner10318}</div></details>`):"";
  html(`<div class="screen accountFormScreen0760${editing?"":" accountFormCreate10318"}">
    <section class="accountFormHeader0760 accountFormTop0760"><button class="back ghost" id="backBtn" aria-label="Cancel and go back">←</button><div><span>${editing?`${esc(recordSingular.toUpperCase())} MAINTENANCE`:`ADD ${esc(recordSingular.toUpperCase())}`}</span><h1>${esc(editing?appLabel("editRecord"):"New Account")}</h1><p>${editing?`Update this ${esc(recordLower)} without changing its history.`:"Name it, confirm the location, and save. Everything else can wait."}</p>${!editing?`<nav class="accountCreateSteps10318" aria-label="Account setup steps"><button type="button" data-account-step10318="identity"><i>1</i>Basics</button><button type="button" data-account-step10318="location"><i>2</i>Location</button><button type="button" data-account-step10318="fireSystem"><i>3</i>Details</button></nav>`:""}</div></section>
    <div class="form grow accountForm0760">
      <div class="accountFormError0760" id="accountFormError0760" hidden></div>
      ${!editing?`<section class="accountCreateHero10318"><span>＋</span><div><small>FAST ACCOUNT SETUP</small><h2>Build a field-ready account</h2><p>Only the name and confirmed location are essential. Add system details now or from Account Detail later.</p></div><strong id="accountCreateReadyCount10318">0/2 ready</strong><div class="accountCreateChecklist10318"><p id="accountCreateNameCheck10318" data-ready="false"><i>✓</i><span>Account name needed</span></p><p id="accountCreateAddressCheck10318" data-ready="false"><i>✓</i><span>Address needed</span></p><p id="accountCreateIdCheck10318"><i>•</i><span>Account ID can be added now or later</span></p></div></section>`:""}
      <section class="card accountFormCard0760${editing?"":" accountIdentityCard10318"}" data-record-group="identity"><div class="accountFormSectionTitle0760"><span>1</span><div><strong>${esc(editing?(RECORD_SCHEMA.groups.find(group=>group.id==="identity")?.label||`${recordSingular} Identity`):"Name the account")}</strong><small>${editing?`Name${fieldOn("externalAccountId")?` and ${esc(recordIdLabel)}`:""} are used throughout ${esc(APP_PROFILE.name)}.`:"Use the customer, company, campus, or building name technicians will recognize."}</small></div></div>
      ...
      ${editing?editLocationMarkup10318:createLocationMarkup10318}
      ${systemMarkup}
      <div class="accountFormActions0760${editing?"":" accountCreateActions10318"}">${!editing?`<span id="accountCreateFooterStatus10318">Complete the name and confirm the address</span>`:""}<button class="ghost" id="cancelAccount0760">Cancel</button><button class="primary" id="saveBtn">${editing?"Save Changes":`Create ${esc(recordSingular)}`}</button></div>
  ...
  document.getElementById("firstAccountGps10317")?.addEventListener("click",captureFirstAccountAddress10317);
  document.getElementById("accountAddressSearchBtn10318")?.addEventListener("click",searchAccountAddress10318);
  document.getElementById("accountAddressSearch10318")?.addEventListener("keydown",event=>{if(event.key==="Enter"){event.preventDefault();searchAccountAddress10318();}});
  document.getElementById("firstAccountManual10317")?.addEventListener("click",()=>{setFirstAccountAssistStatus10317("Manual entry selected. Enter the address below; FireVault will ask you to confirm it before creating the account.","ready");document.getElementById("street")?.focus();});
  document.getElementById("clearAccountAddress10318")?.addEventListener("click",()=>{["street","city","state","zip","gpsLat","gpsLng","gpsAccuracy","gpsCapturedAt"].forEach(id=>{const input=document.getElementById(id);if(input){input.value="";input.dispatchEvent(new Event("input",{bubbles:true}));}});accountAddressConfirmed10318=false;accountAddressSignature10318="";firstAccountDetected10317=null;setAccountAddressStatus10318("review","Address cleared. Search again, use current location, or enter it manually.");setFirstAccountAssistStatus10317("Address cleared.","ready");updateAccountPlusPreview0794();document.getElementById("accountAddressSearch10318")?.focus();});
  document.querySelectorAll("[data-account-step10318]").forEach(button=>button.addEventListener("click",()=>document.querySelector(`[data-record-group="${button.dataset.accountStep10318}"]`)?.scrollIntoView({behavior:"smooth",block:"start"})));
  ["street","city","state","zip"].forEach(id=>document.getElementById(id)?.addEventListener("input",invalidateAccountAddress10318));
  ["name","externalAccountId0760","sitePhone0760"].forEach(id=>document.getElementById(id)?.addEventListener("input",updateAccountCreateSummary10318));
  ...
  if(!editing)updateAccountCreateSummary10318();
  ...
  const persistAccount10318=()=>{
    const accountName=val("name"),obj={name:accountName};
    ...
    if(fieldOn("gps")){
      const gpsLatRaw=val("gpsLat").trim(),gpsLngRaw=val("gpsLng").trim(),gpsLat=Number(gpsLatRaw),gpsLng=Number(gpsLngRaw);
      if(gpsLatRaw&&gpsLngRaw&&Number.isFinite(gpsLat)&&Number.isFinite(gpsLng)){
        const lookupSource=String(firstAccountDetected10317?.source||"");
        obj.gps={lat:gpsLat,lng:gpsLng,accuracy:Number(val("gpsAccuracy"))||0,capturedAt:val("gpsCapturedAt")||new Date().toISOString(),source:lookupSource.includes("address search")?"Confirmed address search":firstAccountDetected10317?"Browser GPS + confirmed address":"Browser GPS"};
        obj.plusCode=encodePlusCode071(gpsLat,gpsLng,plusCodeSettings0794().accountLength);
      }else{obj.gps=null;obj.plusCode="";}
    }
    if(firstAccountDetected10317){obj.geocodeSource=firstAccountDetected10317.source;obj.geocodeMatchedAddress=firstAccountDetected10317.displayAddress;obj.geocodedAt=firstAccountDetected10317.lookedUpAt;}
    if(!editing)obj.addressVerification={status:"confirmed",method:firstAccountDetected10317?String(firstAccountDetected10317.source||"address lookup"):"manual confirmation",confirmedAt:new Date().toISOString(),addressSignature:accountAddressSignatureFromForm10318()};
    ...
    save();completeFirstRun10317();firstAccountGuide10317=false;accountAddressConfirmed10318=false;accountAddressSignature10318="";firstAccountDetected10317=null;toast(editing?`${recordSingular} updated.`:guidedNewAccount10317?`First ${recordLower} created. Add contacts, photos, and exact locations from Account Detail.`:`${recordSingular} created with a confirmed address.`);route("siteDetail");
  };
  document.getElementById("saveBtn")?.addEventListener("click",()=>{
    showFormError("");
    const accountName=val("name");
    if(!editing&&!accountName){showFormError("Enter an account name before saving.");nameInput?.focus();return;}
    ...
    for(const field of activeRecordFields(APP_PROFILE).filter(item=>item.required)){
      const present=field.id==="gps"?(Boolean(val("gpsLat").trim())&&Boolean(val("gpsLng").trim())&&Number.isFinite(Number(val("gpsLat")))&&Number.isFinite(Number(val("gpsLng")))):Boolean(val(requiredInputIds[field.id]||field.id));
    ...
    const hasAddress=[val("street"),val("city"),val("state"),val("zip")].some(Boolean);
    if(!editing&&hasAddress&&!accountAddressConfirmed10318){setAccountAddressStatus10318("review","Confirm the completed address before creating the account.");document.querySelector('[data-record-group="location"]')?.scrollIntoView({behavior:"smooth",block:"start"});confirmManualAccountAddress10318(persistAccount10318);return;}
    persistAccount10318();
  });
...
function tasks(){
  ...
  html(`<div class="screen tasksScreen422"><div class="row taskTopRow fieldListHeader10316"><button class="back ghost" id="backBtn">←</button><div><h1>Task Center</h1><p>${selectedSiteId ? esc(site()?.name || "Site tasks") : "All site follow-ups"}</p></div><button class="primary" id="addBtn" aria-label="Add task">＋</button></div>
...
function taskForm(){
  ...
  html(`<div class="screen fieldForm10315 taskFieldForm10315"><div class="row fieldFormHeader10315 fieldFormTop10315"><button class="back ghost" id="backBtn">←</button><h1>${mode?"Edit":"Add"} Task</h1></div><div class="form grow fieldFormBody10315"><div class="card fieldFormCard10315"><label>Site</label><select id="sitePick">${data.sites.map(x=>`<option value="${x.id}" ${x.id===s.id?"selected":""}>${esc(x.name)}</option>`).join("")}</select><label>Title</label><input id="title" value="${esc(t.title||"")}"><div class="compactField"><div><label>Status</label><select id="status"><option ${t.status!=="Done"?"selected":""}>Open</option><option ${t.status==="Done"?"selected":""}>Done</option></select></div><div><label>Due</label><input id="due" type="date" value="${esc(t.due||"")}"></div></div><label>Notes</label><textarea id="notes">${esc(t.notes||"")}</textarea></div><div class="fieldFormActions10315 ${mode?"hasDelete10315":""}"><button class="primary" id="saveBtn">Save Task</button>${mode?`<button class="danger" id="delBtn">Delete Task</button>`:""}</div></div></div>`);
...
function deficiencies(){
  ...
  html(`<div class="screen deficiencyScreen439"><div class="row defTopRow fieldListHeader10316"><button class="back ghost" id="backBtn">←</button><div><h1>Deficiency Center</h1><p>${selectedSiteId ? esc(site()?.name || "Site deficiencies") : "Open issues across all sites"}</p></div><button class="primary" id="addBtn" aria-label="Add deficiency">＋</button></div>
...
function deficiencyForm(){
  ...
  html(`<div class="screen deficiencyForm525 fieldForm10315 deficiencyFieldForm10315"><div class="row fieldFormHeader10315 fieldFormTop10315"><button class="back ghost" id="backBtn">←</button><h1>${mode?"Edit":"Add"} Deficiency</h1></div><div class="form grow fieldFormBody10315"><div class="card deficiencyFormCard525 fieldFormCard10315"><label>Site</label><select id="sitePick">${data.sites.map(x=>`<option value="${x.id}" ${x.id===s.id?"selected":""}>${esc(x.name)}</option>`).join("")}</select><label>Title</label><input id="title" value="${esc(d.title||"")}" placeholder="Smoke detector trouble, NAC open, battery date, ground fault..."><div class="compactField"><div><label>Priority</label><select id="priority">${["Normal","High","Critical"].map(x=>`<option ${d.priority===x?"selected":""}>${x}</option>`).join("")}</select></div><div><label>Status</label><select id="status"><option ${d.status!=="Closed"?"selected":""}>Open</option><option ${d.status==="Closed"?"selected":""}>Closed</option></select></div></div><label>Notes</label><textarea id="notes" placeholder="Problem found, device location, circuit, parts needed, customer impact...">${esc(d.notes||"")}</textarea>${!mode?`<label class="checkRow fieldFormCheck10315"><input type="checkbox" id="makeTask" checked> Create matching follow-up task</label>`:""}</div>
    <div class="card defPhotoPanel525 fieldFormSupportCard10315"><div class="row"><div><h2>Deficiency Photos</h2><p>${linkedPhotos.length?`${linkedPhotos.length} photo${linkedPhotos.length===1?"":"s"} linked to this deficiency.`:"Attach proof photos, before/after photos, device photos, or wiring photos directly to this deficiency."}</p></div>${mode?`<button class="primary smallBtn" id="addDefPhoto525">＋ Add Photo</button>`:""}</div>${mode?deficiencyPhotoStrip525(s,d,6):`<p class="fieldNote">Save this deficiency first, or use Save + Add Photo below.</p>`}</div>
    <div class="fieldFormActions10315 deficiencyFormActions10315 ${mode?"hasDelete10315":""}"><button class="primary" id="saveBtn">Save Deficiency</button><button class="ghost" id="saveAddPhotoBtn525">Save + Add Photo</button>${mode?`<button class="danger" id="delBtn">Delete Deficiency</button>`:""}</div></div></div>`);
...
function rowPlusCode065(row){
  const pair=rowCoordinatePair065(row)||row?.existingCoordinates;
  return pair?encodePlusCode071(pair.lat,pair.lng,plusCodeSettings0794().accountLength):"";
}
...
function customerManagedFields065(row,existing=null){
  ...
  const codePair=pair||siteCoordinatePair065(existing);
  if(codePair) fields.plusCode=encodePlusCode071(codePair.lat,codePair.lng,plusCodeSettings0794().accountLength);
  if(pair){
...
function activateRealVaultForCustomerImport065(){
  if(!isDemoMode()) return true;
  const approved=confirm("Customer CSV imports create real customer records. Exit Demo Mode and prepare this CSV in your real FireVault vault? Fictional demo accounts will not be imported.");
  if(!approved){toast("CSV import canceled. Demo data was not changed.");return false;}
  setDemoMode(false);
  if(isDemoMode()){
    toast("Demo Mode could not be exited because browser storage is unavailable.","error");
    return false;
  }
  data=loadData({allowEmptyReal:true});
  // Persist the newly selected real workspace before FileReader begins. On a
  // first install there is no real vault yet; without this checkpoint, any
  // interruption or reload would correctly fall back to Demo Mode again.
  saveData(data);
  applyTheme();
  toast("Demo Mode exited. Review the CSV before importing it into the real vault.","success");
  return true;
}
function loadCustomerCsv065(file){
  if(!file) return false;
  if(!activateRealVaultForCustomerImport065()) return false;
...
function customerImportPanel065(){
  ...
  return `<div class="settingsStack settingsStack540 customerImportStack065">
    ${settingsSection540("Customer database","Customer CSV Import","Import customer and monitoring-account information without replacing visits, photos, notes, tasks, or deficiencies.",`
      ...
      ${isDemoMode()?`<div class="settingsInfo540 warning"><strong>Real-vault import</strong><span>Choosing a Customer CSV will ask to exit Demo Mode before the file is analyzed. CSV records are never saved into the temporary demo workspace.</span></div>`:""}
      ...
      ${last?`<div class="customerLastImport065"><strong>Last import</strong><span>${esc(new Date(last.at).toLocaleString())} · ${esc(last.fileName||"Customer CSV")}</span><em>${Number(last.added||0)} added · ${Number(last.updated||0)} updated · ${Number(last.plusCodes||0)} Plus Codes</em></div>`:""}
    `,"blue")}
    ...
    ${summary?settingsSection540("Address coordinates","Calculate Latitude and Longitude",`${summary.coordinatesReady} of ${summary.total} records already have usable coordinates.`,`
      ...
      <p class="fieldNote">Coordinates already present in a CSV are used directly. Each usable coordinate pair also receives an offline Google Plus Code at the saved account precision. Existing manually captured site GPS is preserved. Failed matches stay visible for correction or manual GPS capture.</p>
    `,"cyan"):""}
    ...
    `,"green"):""}
    ${state.lastResult?settingsSection540("Import complete","Customer records saved","The local database, Plus Codes, and synchronization queue were updated.",`<div class="customerImportResult065"><div><strong>${state.lastResult.added}</strong><span>Added</span></div><div><strong>${state.lastResult.updated}</strong><span>Updated</span></div><div><strong>${state.lastResult.plusCodes||0}</strong><span>Plus Codes</span></div><div><strong>${state.lastResult.skipped}</strong><span>Skipped</span></div></div><button class="ghost" id="openImportedSites065">Open Customer Database</button>`,"violet"):""}
...
function performCustomerImport065(){
  if(isDemoMode()){toast("Exit Demo Mode before importing real customer records.","error");return;}
  ...
  try{
    ...
    const persistedIds=new Set((persisted.sites||[]).map(site=>canonicalAccountId0731(site.externalAccountId)).filter(Boolean));
    const missing=selected.map(row=>canonicalAccountId0731(row.accountId)).filter(id=>id&&!persistedIds.has(id));
    if(isDemoMode()||missing.length) throw new Error("The imported customer records could not be verified in the real vault.");
    data=persisted;
  }catch(err){
    console.error("Customer CSV import was not persisted",err);
    data=loadData();
    reanalyzeCustomerImport065({lastResult:null,error:"The CSV was read, but the customer records could not be saved to the real vault. Your source CSV was not changed."});
    settings();toast("Customer import was not saved. Keep the CSV and try again.","error");return;
  }
  reanalyzeCustomerImport065({lastResult:stats,error:""});
  settings(); toast(`${stats.added} added and ${stats.updated} updated with ${stats.plusCodes} Google Plus Codes.`);
}
function wireCustomerImport065(){
  const file=document.getElementById("customerCsvFile065"); if(file) file.onchange=e=>{if(!loadCustomerCsv065(e.target.files?.[0]))e.target.value="";};
...
function importedAccountCard065(s={}){
  ...
  const items=[["Account ID",s.externalAccountId],["Site Phone",s.sitePhone],["Device Type",s.deviceType],["Device Phone",s.devicePhone],["Site ID 1",s.siteId1],["Site ID 2",s.siteId2],["Language",s.siteLanguage],["Source Group",s.sourceGroupNumber],["Latitude",gps?.lat?.toFixed(6)],["Longitude",gps?.lng?.toFixed(6)],["Google Plus Code",sitePlusCode071(s)],["Coordinate Source",s.geocodeSource||s.gps?.source]].filter(x=>x[1]!==undefined&&x[1]!==null&&x[1]!=="");
...
const SETTINGS_GROUPS_067 = [
  ...
  {key:"photos",icon:"▧",title:"Photo Overlay",tabs:["overlay"]},
...
const SETTINGS_DASHBOARD_0860 = [
  {key:"profile",title:"Profile",icon:"user",tabs:["tech"]},
  {key:"photos",title:"Photo Overlay",icon:"camera",tabs:["overlay"]},
...
const SETTINGS_GROUPED_LIST_0873 = [
  {label:"Profile",items:[
    {key:"profile",subtitle:"Technician identity and profile photo"}
  ]},
  {label:"Photos",items:[
    {key:"photos",subtitle:"Photo stamp, fields, branding, and technician overlay"}
  ]},
...
function settingsTabStatus0880(id){
  ...
  if(id==="gps") return gps.enabled===false ? ["Off","Location tools disabled"] : [nativeAppleMapsAvailable10323()?"Apple Maps":gps.mapProvider==="google"?"Google Maps":"Apple Maps",gps.highAccuracy===false?"Standard accuracy":"High accuracy"];
...
function settings(){
  ...
  const saveable=!['privacy','cloudFiles','microsoftStorage','customerImport','categories','backup','webdav','updates','demo','about','architecture','overlay'].includes(settingsTab);
...
function overlayActiveFieldsMarkup0944(){
  ...
  return overlayFieldState0944.map((item,index)=>{
    ...
    return `<details class="overlayActiveField0944 overlayMainField1012 overlayFieldDisclosure1035 ${isTechInfo?"overlayTechInfoField0949":""}" draggable="true" data-overlay-field-index="${index}">
      <summary class="overlayFieldTop1012">
        ...
        <span class="overlayFieldState1035">${isTechInfo?"Independent":index===0?"First line":item.breakBefore?"New line":"Same line"} · ${align[0].toUpperCase()+align.slice(1)}</span>
        <b class="overlayFieldChevron1035" aria-hidden="true">›</b>
      </summary>
      <div class="overlayFieldBottom1012">
        <button type="button" class="overlayBreakButton0944 ${item.breakBefore?"active":""}" data-overlay-break-field="${index}" ${(index===0||isTechInfo)?"disabled":""} aria-label="${isTechInfo?"Tech Info uses its independent Technician Overlay line arrangement":`${item.breakBefore?"Place on same line":"Start a new line before"} ${esc(meta.label)}`}">${isTechInfo?"Uses Technician Overlay lines":index===0?"First field starts the first line":item.breakBefore?"Starts a new line":"Keep on same line"}</button>
...
function photoOverlayTagline1034(category=""){
  const label=String(category||"").trim();
  return !label||label.toLowerCase()==="other" ? "FIREVAULT FIELD NOTES" : `FIREVAULT FIELD NOTES - ${label.toUpperCase()}`;
}
function technicianOverlayRuntimeLines1034(){
  const tech=data.settings?.technician||{};
  const values={
    "{technician}":String(tech.name||"").trim(),
    "{company}":String(tech.company||"").trim(),
    "{phone}":String(formatPhone0758(tech.phone)||tech.phone||"").trim(),
    "{email}":String(tech.email||"").trim(),
    "{license}":String(tech.license||"").trim()
  };
  const lines=[];
  technicianOverlayNormalize0946(data.settings?.technicianOverlay?.fields).forEach(item=>{
    const value=values[item.tag]||"";if(!value)return;
    if(!lines.length||item.breakBefore)lines.push([]);
    lines[lines.length-1].push(value);
  });
  return lines.map(line=>line.join(" • ")).filter(Boolean);
}
function overlayRenderedLines0945(set,siteData,options={}){
  ...
  layout.forEach((item,index)=>{
    ...
    if(item.tag===OVERLAY_TECH_INFO_TAG_0949){
      if(options.omitTechnician)return;
...
async function drawOverlayStamp0890(ctx,w,h,set,siteData,options={}){
  ...
  const logicalLines=overlayRenderedLines0945(set,siteData,options);
  const tagline=set.showTagline ? photoOverlayTagline1034(options.category) : "";
  ...
  const layout=()=>{
    ...
    const tagFontSize=Math.max(11,Math.round(fontSize*.62));
    ctx.font=`800 ${tagFontSize}px Arial, sans-serif`;
    const taglineWidth=tagline?ctx.measureText(tagline).width:0;
    maxRenderedWidth=Math.max(measured,taglineWidth);
    stampW=set.backgroundStyle==="card"
      ? Math.min(w-pad*2,Math.max(Math.round(w*.46),maxRenderedWidth+logoSize+pad*4))
      : w-pad*2;
...
function drawTechnicianOverlay1034(ctx,w,h,set={}){
  const sourceLines=technicianOverlayRuntimeLines1034();
  if(!sourceLines.length)return;
  const pad=Math.max(14,Math.round(w*.017));
  const inner=Math.max(12,Math.round(w*.014));
  const fontSize=Math.max(14,Math.min(34,Math.round(w*.021)));
  const lineHeight=Math.round(fontSize*1.28);
  const maxTextWidth=Math.max(150,Math.round(w*.52));
  ctx.save();ctx.font=`800 ${fontSize}px Arial, sans-serif`;
  const lines=sourceLines.flatMap(line=>wrapCanvasText512(ctx,line,maxTextWidth));
  const labelSize=Math.max(10,Math.round(fontSize*.56));
  const labelHeight=Math.round(labelSize*1.45);
  const measured=Math.max(...lines.map(line=>ctx.measureText(line).width),ctx.measureText("TECHNICIAN").width);
  const cardW=Math.min(w-pad*2,Math.max(Math.round(w*.29),Math.min(maxTextWidth,measured)+inner*2));
  const cardH=inner*2+labelHeight+lines.length*lineHeight;
  const align=technicianOverlaySavedAlignment0948();
  const x=align==="left"?pad:align==="center"?Math.round((w-cardW)/2):w-cardW-pad;
  const y=set.alignment==="bottom"?pad:Math.max(pad,h-cardH-pad);
  const accent=hexToRgb512(set.accentColor||"#ef4444");
  roundRect512(ctx,x,y,cardW,cardH,Math.max(14,Math.round(w*.011)));
  ctx.fillStyle="rgba(4,10,18,.88)";ctx.fill();
  ctx.strokeStyle=`rgba(${accent.r},${accent.g},${accent.b},.92)`;ctx.lineWidth=Math.max(2,Math.round(w*.0025));ctx.stroke();
  const textX=align==="left"?x+inner:align==="center"?x+cardW/2:x+cardW-inner;
  ctx.textAlign=align;ctx.textBaseline="top";ctx.font=`900 ${labelSize}px Arial, sans-serif`;ctx.fillStyle=`rgba(${accent.r},${accent.g},${accent.b},1)`;ctx.fillText("TECHNICIAN",textX,y+inner);
  ctx.font=`800 ${fontSize}px Arial, sans-serif`;ctx.fillStyle="#fff";
  let ty=y+inner+labelHeight;lines.forEach(line=>{ctx.fillText(line,textX,ty);ty+=lineHeight;});
  ctx.restore();
}
async function renderOverlayComposite0890(source,set,siteData,maxW=1800,options={}){
  ...
  const showTechnician=options.showTechnician===true;
  if(options.showMain!==false)await drawOverlayStamp0890(ctx,w,h,set,siteData,{category:options.category||"",omitTechnician:showTechnician});
  if(showTechnician)drawTechnicianOverlay1034(ctx,w,h,set);
  return canvas;
}
function overlayPreviewMarkup510(){
  return `<section class="overlayPreviewCard0890 overlayPreviewSticky0942 overlayPreviewWide0943 overlayPreview1012">
    <header class="overlayWorkspaceHead1012">
      <div><span>LIVE RESULT</span><strong>Photo Preview</strong><small>Exactly what downloads with the photo</small></div>
    ...
    <div class="overlayPreviewFoot1035"><span><i></i> Live preview</span><small>Deficiency category sample</small></div>
  </section>`;
}
function overlayDisclosureSummary1035(step,title,note,status=""){
  return `<summary class="overlayDisclosureSummary1035"><span>${esc(step)}</span><div><strong>${esc(title)}</strong><small>${esc(note)}</small></div>${status?`<em>${esc(status)}</em>`:""}<b aria-hidden="true">›</b></summary>`;
}
function overlaySettingsPanel510(o){
  ...
  const layoutSummary=`${set.alignment[0].toUpperCase()+set.alignment.slice(1)} · ${set.backgroundStyle==="bar"?"Full bar":set.backgroundStyle==="card"?"Card":"Minimal"} · ${set.fontSize}`;
  const brandSummary=`${set.showLogo?(set.logoMode==="custom"?"Custom logo":"FireVault logo"):"Logo off"} · ${set.showTagline?"Tagline on":"Tagline off"}`;
  return `<div class="overlayStudio0890 overlayStudio0942 overlayStudio0943 overlayStudio1012 overlayStudio1035">
    ${overlayPreviewMarkup510()}
    <div class="overlayControlsColumn0942 overlayControlsColumn0943 overlayControlsColumn1012 overlayControlsColumn1035">
      <section class="overlayQuickCard0890 overlayQuickCard1012 overlayQuickCard1035">
        <div class="overlayCardHead0890"><div><span>START HERE</span><h2>Choose a look</h2><p>Pick a preset. Everything saves automatically.</p></div><button type="button" class="ghost overlayReset1012" id="ovReset0890">Reset</button></div>
        ...
        <p class="overlayTaglineNote1034">Tagline follows the selected photo category. Other shows FIREVAULT FIELD NOTES only.</p>
      </section>
      <details class="overlayDisclosure1035 overlayContent0890 overlayFieldBuilder0944 overlayFieldBuilder1012">
        ${overlayDisclosureSummary1035("1","Photo information","Choose and arrange fields",`${overlayFieldState0944.length} fields`)}
        <div class="overlayDisclosureBody1035"><input type="hidden" id="ovTemplate" value="${esc(set.template)}">
          <div class="overlayBuilderGroup0944"><div class="overlayBuilderTitle0944"><div><strong>Fields on photo</strong><small>Tap a field to adjust its line, alignment, or order.</small></div><button type="button" class="overlayQuickAlign0945" id="overlayTechnicianInfo0946">${fvIcon073("visit","overlayQuickInfoIcon1012")}<strong>Tech Info</strong><small>Use separate overlay</small></button></div><div class="overlayActiveFields0944" id="overlayActiveFields0944">${overlayActiveFieldsMarkup0944()}</div></div>
          <div class="overlayBuilderGroup0944"><div class="overlayBuilderTitle0944"><div><strong>Add a field</strong><small>Added fields are checked.</small></div></div><div class="overlayFieldGrid510" id="overlayAvailableFields0944">${overlayTagButtons510()}</div></div>
        </div>
      </details>
      <details class="overlayDisclosure1035 overlayLayout0890 overlayLayout1012">
        ${overlayDisclosureSummary1035("2","Layout","Position, style, size, and opacity",layoutSummary)}
        <div class="overlayDisclosureBody1035 overlayLayoutBody1035"><div class="overlayControlBlock0890"><label>Position</label>${overlayChoiceButtons0890("ovAlign",set.alignment,[["top","Top",fvIcon073("up","overlayChoiceIcon1012")],["middle","Center",fvIcon073("nearby","overlayChoiceIcon1012")],["bottom","Bottom",fvIcon073("down","overlayChoiceIcon1012")]])}</div>
      ...
      <details class="overlayDisclosure1035 overlayBrand0890 overlayBrand1012">
        ${overlayDisclosureSummary1035("3","Branding","Logo, tagline, and colors",brandSummary)}
        <div class="overlayDisclosureBody1035"><div class="overlayColorGrid0890">
            ${fieldBlock("Accent",`<input id="ovAccent" type="color" value="${esc(set.accentColor)}">`)}
            ${fieldBlock("Text",`<input id="ovText" type="color" value="${esc(set.textColor)}">`)}
            ${fieldBlock("Logo",`<select id="ovLogoMode"><option value="firevault" ${set.logoMode==="firevault"?"selected":""}>FireVault logo</option><option value="custom" ${set.logoMode==="custom"?"selected":""}>Custom logo</option></select>`)}
          </div><div class="overlayLogoCard511"><div class="overlayLogoPreview511">${set.showLogo ? (overlayLogoSrc510(set)?`<img src="${esc(overlayLogoSrc510(set))}" alt="Current overlay logo preview">`:`<span>No logo image</span>`) : `<span>Logo hidden</span>`}</div><div class="overlayLogoMeta511"><strong>Overlay logo</strong><p id="ovLogoStatus">${esc(overlayLogoStatus510(set))}</p><label class="overlayUploadLabel511"><span>Upload custom logo</span><input id="ovCustomLogo" type="file" accept="image/*"></label><div class="overlayLogoActions511"><button type="button" class="ghost" id="ovUseFireVault">Use FireVault</button><button type="button" class="ghost" id="ovClearCustomLogo">Clear custom</button></div></div></div>
        </div>
      </details>
    </div>
  </div>`;
...
async function updateOverlayPreview510(){
  ...
  try{
    const composed=await renderOverlayComposite0890("assets/overlay-sample-fire-alarm-issue.png",set,overlaySampleSite510(),900,{category:"Deficiency"});
...
function wireOverlayDisclosures1035(){
  const sections=[...document.querySelectorAll(".overlayDisclosure1035")].filter(section=>!section.classList.contains("overlayFieldDisclosure1035"));
...
const FIREVAULT_MANUAL_058 = [
  {id:"start",title:"Getting Started",icon:"▶",status:"Current",summary:"Install FireVault, understand local storage, and learn the main navigation.",topics:[
    ...
    ["Your first visit","A new empty FireVault opens with fictional demo accounts and three short guide cards. Explore Search, Nearby Accounts, and Account Detail freely; the demo workspace stays separate from your real customer vault."],
    ["Create your first real account","Tap Add Account when you are ready and choose Yes — Use Real Vault to leave Demo Mode. Enter a business or street address and tap Search, use Current Location, or enter the location manually. Review the match, confirm the address, add the account name, and create the account. Changes to street, city, state, or ZIP require another confirmation."],
  ...
  {id:"sites",title:"Customer Database",icon:"▦",status:"Current",summary:"Create, find, edit, and organize customer accounts.",topics:[
    ["Create a site","Open Sites and tap Add Account. Enter the recognizable account name, then search a business or street address, use current GPS, or enter it manually. Confirm the location before creating the account. Fire alarm details and site notes are optional during setup and remain available later."],
    ["Address lookup","Search runs only after you tap Search and returns up to five matches. Tap a match to review its structured address, available commercial name, and coordinates. Confirm Address fills the form; Back to Results and Enter Manually leave the account unchanged."],
    ["Address confirmation","If you change street, city, state, or ZIP after a lookup, FireVault marks the address for review again. A manually entered or corrected address must pass the final Confirm & Create check."],
    ...
    ["Customer CSV Import","Open Settings → Customer Import and choose a compatible CSV file. If Demo Mode is active, FireVault asks to exit it before analyzing the file. Coordinates supplied by the CSV or calculated from each address automatically generate a full offline Google Plus Code at the saved account precision. Review the coordinates and Plus Codes, then import; success appears only after the selected Account IDs are verified in the real local vault."],
  ...
  {id:"photos",title:"Photos & Photo Overlay",icon:"▧",status:"Current",summary:"Capture, label, organize, and preserve site photographs.",topics:[
    ...
    ["Scan a document on iPhone","In the installed native iPhone app, open the correct account and tap the large Scan Document button on Account Detail. Apple’s scanner finds the page edges, corrects perspective, and lets you capture, review, retake, or add more pages. Tap Done, enter the document title and type, then tap Save Scan. FireVault returns to the same account. A secondary Scan Document button remains under Documents / Photos; the browser PWA intentionally hides both native-only controls."],
    ["Review and share a scan","Saved scans stay with the account in capture order. Tap a scan to preview its pages, then use the existing PDF controls to download or share it. Scans also appear in Account Activity."],
    ...
    ["Overlay settings","Settings → Photo Overlay keeps the live result and presets visible first. Tap Photo Information, Layout, Branding, or Technician Overlay only when you need those controls. Tap an individual photo field to adjust its line, alignment, order, or removal. The Technician Overlay remains independent from the main overlay settings."],
  ...
  {id:"nearby",title:"GPS & Nearby Accounts",icon:"⌖",status:"Current",summary:"Use saved coordinates to find, select, and navigate to nearby customer accounts.",topics:[
    ...
    ["Exact locations","Account Detail → Locations stores parking, entrances, panels, risers, FDCs, and other exact points with GPS, Plus Codes, access notes, and route actions."],
    ["On-Site Guide","When a foreground Nearby scan confirms arrival with usable GPS accuracy, FireVault can offer a focused property map. The guide orders parking, entrances, and equipment and is always available manually from Account Detail → Locations."],
    ["Shared-address accounts","When multiple accounts share the same site GPS, confirm the account name and Account ID before opening On-Site Guide."],
    ["Location Privacy","FireVault uses phone location only while the app is open and a location-based action is requested. On-Site Guide does not add background monitoring."]
  ]},
...
function manualScreenshotGuide1030(){
  const cards=[
    ...
    ["assets/help-photo-overlay.png","Set up both photo overlays","Go directly to Settings → Photo Overlay. The normal photo stamp and Technician Overlay save independently."]
  ];
...
function manualHome058(){
  ...
  return `<div class="academyHome067">
    ...
    ${pinnedLearning059()}
    <p class="academyReviewNote067">Manual revision 1.03.28 · Reviewed July 2026. Documentation must be checked with every release.</p>
  </div>`;
}
...
function manualSimplePage058(type){
 ...
 pages.quick[3][0][1]="Confirm the green build badge shows 1.03.28 before entering production information.";
 pages.new=["🆕","What’s New in 1.03.28","Account Details now follows the selected Option 4 design: one calm, tabless workspace focused on field records.",[
  ["Tabless navigation","Overview, Map & Locations, Equipment, Files & Scans, Notes, and Account Info use a single accordion with only one section open."],
  ["Field-first tools","Scan, Call, and Route remain in one compact row instead of several oversized action panels."],
  ["Focused modules","Tasks and Deficiencies are disabled throughout the active app profile while their stored records remain untouched."],
  ["More working space","The redundant global brand header is removed from Account Details so account content starts higher on the screen."],
  ["Core workflow","Notes, files, Apple document scans, equipment, maps, GPS, photos, and account information remain available."]
 ]];
 if(Array.isArray(pages.revisions?.[3]?.[0]))pages.revisions[3][0].unshift(["1.03.28","Rebuilds Account Details as a tabless accordion focused on Overview, Maps, Equipment, Files, Scans, and Notes; Tasks and Deficiencies are disabled without deleting stored records."],["1.03.27","Compacts the Account Detail command header so working information appears sooner."],["1.03.26","Moves Scan Document directly onto Account Detail, returns saved scans to the same account, collapses secondary account tabs and document-management controls, and adds the native FireVault app icon."],["1.03.25","Restores account document scanning through Apple VisionKit with automatic edge correction, multi-page capture, compact save review, IndexedDB storage, and retained PDF tools."]);
...
const ACADEMY_TUTORIALS_059=[
 ...
 {id:"first-site",icon:"▦",title:"Create Your First Customer",time:"4 min",level:"Beginner",summary:"Move safely from Demo Mode into a confirmed first real account.",steps:["Explore the fictional demo accounts, then tap Add Account and choose Yes — Use Real Vault when you are ready.","Enter the business or street address and tap Search, or choose Use Current Location while at the site.","Select a result, review the commercial name, structured address, and GPS, then tap Confirm Address. Use manual entry if no match is correct.","Enter the account name and optional Account ID or phone. Expand Fire alarm details only when you want to add panel information now.","Tap Create Account. If the address was entered or changed manually, review it and tap Confirm & Create, then verify Account Detail."]},
 {id:"visit",icon:"✓",title:"Document a Site Visit",time:"6 min",level:"Beginner",summary:"Record work performed and leave a useful history for the next technician.",steps:["Open the correct customer account.","Review the account Overview.","Confirm Map & Locations and update Equipment when needed.","Add Notes, photos, files, or document scans.","Review the account before leaving the site."]},
 {id:"notes",icon:"✎",title:"Notes & Field Records",time:"5 min",level:"Beginner",summary:"Create clear account documentation without unnecessary workflow layers.",steps:["Use Notes for observations, service history, and follow-up context.","Add the exact device or equipment involved.","Attach a photo or document scan when it improves the record.","Save exact entrance, parking, panel, or equipment locations under Map & Locations.","Review the account record before leaving the site."]},
 ...
 {id:"scanner",icon:"▤",title:"Scan a Site Document",time:"3 min",level:"Beginner",summary:"Use Apple’s document camera to attach a clean multi-page scan to an account.",steps:["In the installed iPhone app, open the correct account.","Tap the large Scan Document button and hold the phone over the first page. Apple finds and corrects the page edges.","Review or retake the page, then capture any additional pages and tap Done.","Confirm the page order, enter a title and document type, and add optional notes.","Tap Save Scan. FireVault returns to the account; open Photos / Files later to preview, download, or share the PDF."]},
...
const ACADEMY_TOUR_059=[
 ...
 ["Bottom Navigation","Use the four persistent buttons—Nearby, Search, Photo, and Settings—to move between field workspaces without losing stored data. The dock remains available after Account Directory searches."],
...
function switchDemoMode0738(enabled,reset=false){
  ...
  if(!enabled){
    // A deliberate first-user exit must create the empty real workspace before
    // reload. Otherwise startup sees no real vault and returns to Demo Mode.
    data=loadData({allowEmptyReal:true});
    saveData(data);
  }
...
function technicianOverlaySettingsPanel1030(){
  const fields=technicianOverlayNormalize0946(data.settings?.technicianOverlay?.fields);
  return `<details class="card technicianOverlayIndependent1030 overlayDisclosure1035 technicianDisclosure1035">
    <summary class="overlayDisclosureSummary1035"><span>T</span><div><strong>Technician Overlay</strong><small>Optional, separately saved photo layer</small></div><em>${fields.length} fields · ${technicianOverlaySavedAlignment0948()}</em><b aria-hidden="true">›</b></summary>
    <div class="technicianOverlayTemplate0946 overlayFieldBuilder0944 overlayDisclosureBody1035">
      <div class="technicianOverlayCompactHead1035"><p>Used only when Add Technician Overlay is selected while taking or importing a photo.</p><em class="overlayAutoSave0944" id="technicianOverlayAutoSave0946" data-state="saved">Saved</em></div>
...
function settingsPanel(){
  ...
  if(settingsTab==="gps") return `<div class="settingsStack settingsStack540">
    ${settingsSection540("Navigation","Map Preferences","Set the map service, GPS accuracy, and distance used for nearby-account detection.",`<div class="settingsGrid settingsGrid540">${fieldBlock("Default map",`<select id="gpsMapProvider">${nativeAppleMapsAvailable10323()?`<option value="apple" selected>Apple Maps — Native</option>`:`<option value="apple" ${gps.mapProvider!=="google"?"selected":""}>Apple Maps</option><option value="google" ${gps.mapProvider==="google"?"selected":""}>Google Maps</option>`}</select>`)}${fieldBlock("GPS accuracy",`<select id="gpsHighAccuracy"><option value="true" ${gps.highAccuracy!==false?"selected":""}>High accuracy</option><option value="false" ${gps.highAccuracy===false?"selected":""}>Standard</option></select>`)}${fieldBlock("Nearby radius",`<input id="gpsNearbyRadius" inputmode="decimal" value="${esc(gps.nearbyRadiusMiles||1)}">`,`Distance in miles used by Nearby Accounts`)}</div>`,"green",saveButton())}
    ${settingsSection540("Availability","GPS Tools","Choose where location controls and saved coordinates are available.",`<div class="settingsList settingsToggleList540">${checkBlock("gpsEnabled","Show GPS capture buttons on site pages",gps.enabled!==false)}${checkBlock("gpsReports","Include GPS coordinates in reports",gps.includeInReports!==false)}${checkBlock("gpsAddressAssist","Enable address lookup when adding accounts",gps.addressAssist!==false)}</div><div class="settingsInfo540"><strong>Location permission required</strong><span>${nativeAppleMapsAvailable10323()?"The native iPhone app uses Apple Maps and Core Location for GPS, address search, business matching, map previews, and routing.":"The browser version uses HTTPS location access with OpenStreetMap address and map fallback. Search and GPS assistance run only after a tap and always require address confirmation."}</span></div>`,"teal")}
  </div>`;
...
function saveSettings(){
  ...
  if(settingsTab==="gps") s.gps={...(s.gps||{}),enabled:checked("gpsEnabled"),mapProvider:val("gpsMapProvider")||"apple",highAccuracy:val("gpsHighAccuracy")!=="false",includeInReports:checked("gpsReports"),nearbyRadiusMiles:Number(val("gpsNearbyRadius"))||1,addressAssist:checked("gpsAddressAssist")};
  if(settingsTab==="homeLayout") {
    const cards={};
...
function showChangelog(){
  const notes = [
    "Build 1.03.28 rebuilds Account Details as a tabless accordion focused on Overview, Maps, Equipment, Files, Scans, and Notes.",
    "Tasks and Deficiencies are disabled in the active app profile; any existing stored records remain preserved.",
    "Scan, Call, and Route share one compact essentials row, and the redundant global header is removed from Account Details.",
    "Saving from the primary scanner returns directly to the same account instead of opening the document-management workspace.",
    "Account Detail keeps Overview, Photos / Files, and Notes visible while Details, Locations, and Equipment stay collapsed under More.",
    "Document search, filters, and sorting stay collapsed until Find or organize saved items is opened.",
    "The native Xcode app now installs with the FireVault flame icon.",
    "Build 1.03.25 restores Apple-powered document scanning inside each account in the native iPhone app.",
    "Apple VisionKit provides automatic document edges, perspective correction, retakes, and multi-page capture.",
    "A compact Save Scan review keeps the captured order and records title, type, date, and optional notes.",
    "Scanned pages remain in FireVault’s protected on-device media store and retain preview, PDF download, share, timeline, and delete tools.",
    "Build 1.03.24 repairs the bottom dock so its four visible buttons always receive four matching columns.",
    "Nearby, Search, Photo, and Settings now use larger icons, readable single-line labels, and iPhone safe-area spacing.",
    "Account Directory search restores the dock and its dynamic layout after typing, dictation, paste, clearing, or keyboard dismissal.",
    "Build 1.03.23 adds native Apple Maps search, Core Location GPS, reverse geocoding, map previews, and routes with browser fallback.",
    "Build 1.03.21 removes all quick-action buttons from Account Directory rows, including the top result.",
    "Each directory row remains one compact tap or keyboard target that opens the exact account.",
    "Bottom navigation remains visible through directory search and is restored when search loses focus.",
    "The redundant GPS & Navigation panel is removed from the bottom of Account Detail’s Details tab.",
    "The account address, Route action, location editing, Plus Code, and exact-location tools remain available.",
    "Account Detail tabs now use individual shaded surfaces and a stronger active treatment.",
    "Build 1.03.20 polishes Nearby Accounts as a clearer map-and-list workspace without adding features.",
    "Build 1.03.18 completely rebuilt Add Account as a focused three-step workspace with confirmed address search.",
    "Build 1.03.17 added the protected first-run guide and Demo Mode transition retained by this release.",
    "Build 1.03.16 polishes Contact, Equipment, Task, and Deficiency lists without changing their workflows.",
    "Those list screens retain their compact headers, readable cards, 44 pixel filters, and phone action grids.",
    "Build 1.03.15 standardizes Contact, Equipment, Task, and Deficiency editors without changing their workflows.",
    "All four forms retain their consistent headers, readable fields, Unsaved states, and sticky actions.",
    "Build 1.03.14 polishes Add Account and Edit Account without changing fields, validation, or saved data.",
    "Its Unsaved state, labels, Required badges, GPS capture, and Save / Cancel actions remain protected.",
    "Build 1.03.13 cleans up Quick Photo review and Take Photo / Upload Photo without changing photo behavior.",
    "All nine imported-photo categories fit in a compact three-column phone selector, and the four photo actions use a two-by-two grid.",
    "Build 1.03.12 improves active-screen readability and touch targets without changing field workflows.",
    "Account Directory names, addresses, metadata, issue badges, filters, and quick actions are easier to read.",
    "Call, Route, Add Note, Photo, and Favorite remain single-line actions with 46 to 48 pixel touch targets.",
    "Account Detail phone labels and the smallest compact Photo Overlay controls receive focused readability polish.",
    "Build 1.03.11 traces and verifies the complete runtime and offline asset graph before packaging.",
    "The release suite resolves HTML references, every JavaScript import, CSS asset URLs, manifest icons and shortcuts, branding media, Help screenshots, and Photo Overlay sample media.",
    "All 30 runtime files must exist in the 32-entry offline shell and all versioned references must use the current build token.",
    "Manifest icons are validated as real PNG files with their exact declared dimensions.",
    "Build 1.03.10 publishes FireVault directly through GitHub Pages without an unnecessary Jekyll build.",
    "The package includes a root-level .nojekyll marker and current, exact extraction instructions.",
    "A deployment contract rejects nested release folders, missing root files, Jekyll/Ruby build files, and stale 1.03 asset references.",
    "Only the newest Pages workflow run for the newest commit needs to succeed; earlier failed runs do not need to be restarted.",
    "This deployment release adds no records, settings, storage changes, or field features.",
    "Build 1.03.9 locks a realistic field account into the upgrade-preservation release suite.",
    "Account notes, contacts, tasks, deficiencies, GPS, account and entrance Plus Codes, and preferred locations survive save and reload checks.",
    "Photo metadata, media references, customer captions, Photo Overlay, Technician Overlay, and technician profile settings survive the same cycle.",
    "Rolling backup, prior-revision recovery, stable device identity, real-vault Demo Mode separation, and service-worker storage isolation are verified.",
    "This stability release adds no records, settings, storage changes, or field features.",
    "Build 1.03.8 adds a bounded fresh-first navigation bootstrap for newly deployed pages.",
    "Fast online navigation returns the newest HTML within a 1.8-second freshness window.",
    "Slow or unavailable connections return the complete cached app without blocking field use.",
    "A slow response continues refreshing the cache in the background after fallback.",
    "Executable service-worker simulations cover fast, slow, offline, and first-install navigation.",
    "This stability release adds no records, settings, storage changes, or field features.",
    "Build 1.03.7 pins the canonical Update Ready geometry after every other application style.",
    "Safe-area-aware sizing keeps the card centered, bounded, and scrollable on iPhone and iPad.",
    "A dedicated PWA upgrade contract verifies build alignment, deferred activation, cache cleanup, navigation refresh, and all offline-shell files.",
    "Photo Overlay checks now cover Quick Photo, imported photos, categories, the normal overlay, and independent Technician Overlay composition.",
    "This stability release adds no records, settings, storage changes, or field features.",
    "Build 1.03.6 repairs the iPhone Update Ready screen that could expand into a blank dark panel and cover its controls.",
    "Retired compact-banner geometry no longer overrides the current centered dialog.",
    "Install Update, Release Notes, Later, Try Again, and Reload App remain visible and touchable.",
    "Tapping the empty blurred backdrop safely dismisses the prompt before installation starts.",
    "The existing eight-second recovery watchdog still restores controls if service-worker activation stalls.",
    "Build 1.03.5 redesigns Photo Overlay Settings as a compact, progressive workspace.",
    "The exact live preview and Compact, Standard, and Detailed presets remain immediately visible.",
    "Photo Information, Layout, Branding, and Technician Overlay now open from concise summary rows.",
    "Each active field expands independently for line placement, alignment, ordering, and removal.",
    "Desktop layouts place preview and controls side by side, while phones use one clean vertical flow.",
    "All overlay rendering, presets, auto-save, category taglines, and Technician Overlay behavior remain unchanged.",
    "Build 1.03.4 changes the photo tagline to FIREVAULT FIELD NOTES followed by the selected category.",
    "When Other is selected, the tagline stays FIREVAULT FIELD NOTES without a category suffix.",
    "Quick Photo and Take Photo / Upload Photo now include an independent Add Technician Overlay option.",
    "The Technician Overlay choice is saved per photo and does not change the normal Photo Overlay settings.",
    "Photo Overlay remains directly available on the main Settings page whenever Photos are enabled.",
    "Build 1.03.3 redesigns Update Ready as a large centered dialog over a blurred background.",
    "The dialog confirms that local FireVault data stays protected and provides Install Update, Release Notes, and Later controls.",
    "If activation fails or stalls, Try Again and Reload App remain available instead of locking the popup.",
    "Build 1.03.2 generates a Google Plus Code for each GPS-ready address processed through Customer CSV Import.",
    "Codes are calculated offline from coordinates supplied by the CSV, calculated from the address, or already stored on an updated account.",
    "The CSV preview shows the Plus Code beside latitude and longitude before import.",
    "The import summary reports how many Plus Codes were persisted in the real vault.",
    "Photo Overlay is restored as its own Settings section with visible Back and Done navigation.",
    "All normal and Technician Overlay controls remain available and independently auto-save.",
    "Build 1.03.1 real-vault CSV persistence and Update Ready lockup recovery remain unchanged.",
    ...
    "Shared-location accounts require a visible account name and Account ID choice, and the arrival card appears only once per site session.",
    "Account Detail → Locations now provides permanent manual On-Site Guide access without adding background location monitoring.",
    "Build 1.01.5 redesigns Update Ready as a clear field-safe sheet with full-size Later and Install Update actions.",
    "The sheet prevents repeat taps, reports installation progress, refreshes duplicate prompts, and provides retry guidance when needed.",
    "The existing service-worker update mechanism and all locally stored data remain unchanged.",
    "Build 1.01.4 keeps the selected Account Detail tab fully visible after every tab change on narrow phones.",
    "Clipped tabs are revealed automatically while phone swiping and the six-column iPad layout remain unchanged.",
    "Account tab content, order, actions, workflows, and stored data remain unchanged.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 1.03.28 simplifies Account Detail so the account’s working information is visible sooner, without changing existing account data.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
...
window.fireVaultOpenReleaseNotes=showChangelog;
...
function bootFireVault518(){
  try{
    ...
    render();
    scheduleFirstRunOnboarding10317();
    ...
    if((view||"home")==="home" && (isDemoMode() || nativeAppleMapsAvailable10323() || navigator.geolocation) && data.settings.gps?.enabled!==false && !autoGpsAlready069){
