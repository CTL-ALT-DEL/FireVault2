import { BUILD, KEY, loadData, saveData, ensureSite, fullAddress, esc, uid, downloadBlob, syncSummary, syncQueue, syncConflicts, syncActivity, createSyncPackage, importSyncPackage, resolveSyncConflict, notePackageExport, deviceIdentity, recordSyncActivity, autoBackupInfo, latestAutoBackup, restoreAutoBackup, isDemoMode, setDemoMode, resetDemoData, securityFoundationSummary, securityAudit, recycleBinInfo, restoreRecycleRecord, purgeRecycleBin, recordSecurityEvent, validateVaultIntegrity } from "./storage.js?v=1.03.27";
import { backendAdapterSummary, runBackendAdapterDiagnostics, backendAdapterManifest, PROVIDER_CONTRACT_VERSION, FILE_STORAGE_CATALOG, fileStoragePlanSummary, cloudFileStorageManifest, MICROSOFT_STORAGE_TYPES, microsoftStorageAccounts, saveMicrosoftStorageAccounts, createMicrosoftStorageAccount, microsoftStorageAccountById, microsoftAppRegistration, saveMicrosoftAppRegistration, microsoftStorageSummary, microsoftStorageManifest } from "./providers.js?v=1.03.27";
import { encodePlusCode, isValidFullPlusCode, normalizePlusCode, plusCodePrecisionLabel } from "./open-location-code.js?v=1.03.27";
import { hydrateVaultMedia, stageVaultMedia, prepareVaultWithMedia, mediaStorageSummary, requestPersistentMediaStorage, pruneOrphanedMedia, flushMediaWrites } from "./media-store.js?v=1.03.27";
import { APP_PROFILE, APP_PROFILE_SCHEMA_VERSION, appTerm, appLabel, appNavigationLabel, appProfileExport } from "./app-profile.js?v=1.03.27";
import { MODULE_REGISTRY, MODULE_REGISTRY_VERSION, MODULE_CLASSIFICATIONS, FUTURE_APP_COLUMNS, moduleRegistrySummary, moduleMatrixRows, moduleRegistryExport } from "./module-registry.js?v=1.03.27";
import { MODULE_BINDINGS_VERSION, NAV_MODULE_REQUIREMENTS as NAV_MODULE_REQUIREMENTS_0955, ROUTE_MODULE_REQUIREMENTS as ROUTE_MODULE_REQUIREMENTS_0955, SETTINGS_MODULE_REQUIREMENTS as SETTINGS_MODULE_REQUIREMENTS_0955, ACCOUNT_TAB_BINDINGS, requirementsMet as moduleRequirementsMetFromBindings0955, moduleBindingsExport } from "./module-bindings.js?v=1.03.27";
import { RECORD_SCHEMA, RECORD_SCHEMA_VERSION, recordFieldById, activeRecordFields, recordFieldEnabled, recordFieldRequired, activeDetailSections, recordDetailSectionEnabled, recordPhotoCategories, recordSchemaSummary, recordSchemaExport } from "./record-schema.js?v=1.03.27";
import { WORKFLOW_SCHEMA, WORKFLOW_SCHEMA_VERSION, activeWorkflowActions, workflowActionEnabled, quickPhotoWorkflow, workflowSchemaSummary, workflowSchemaExport } from "./workflow-schema.js?v=1.03.27";
import { THEME_PROFILE_SCHEMA_VERSION, resolveThemeProfile, applyThemeProfile, themeBrandAsset, themeWordmarkMarkup, themeProfileSummary, themeProfileExport } from "./theme-profile.js?v=1.03.27";
import { CONTENT_PACK_SCHEMA_VERSION, CONTENT_SOURCE_TYPES, CONTENT_SOURCES, CONTENT_PACKS, resolveContentPackProfile, activeContentSources, activeContentPacks, contentPackLibraryFolders, contentPackSummary, contentPackRegistryExport } from "./content-pack-registry.js?v=1.03.27";
import { SYNC_STORAGE_PROFILE_SCHEMA_VERSION, STORAGE_PROVIDER_TYPES, STORAGE_PROVIDERS, STORAGE_ROLES, resolveSyncStorageProfile, activeStorageProviders, storageProviderEnabled, storageRoleProviders, syncStorageSettingsTabEnabled, syncStorageSummary, syncStorageProfileExport } from "./sync-storage-profile.js?v=1.03.27";
import { APP_FORGE_BLUEPRINT_SCHEMA_VERSION, appForgeReadinessSummary, validateAppForgeProfile, appForgeBlueprintExport } from "./app-forge-blueprint.js?v=1.03.27";
import { APP_FORGE_RECIPE_SCHEMA_VERSION, appForgeRecipes, appForgeRecipeSummary, appForgeRecipeBlueprintExport, appForgeRecipeCatalogExport } from "./app-forge-recipes.js?v=1.03.27";
import { APP_FORGE_FACTORY_SCHEMA_VERSION, createAppForgeGenerationRequest, appForgeFactoryManifest, appForgeFactorySummary, appForgeFactorySchemaExport } from "./app-forge-factory.js?v=1.03.27";
import { APP_FORGE_GENERATOR_SCHEMA_VERSION, appForgeGeneratorPlan, appForgeGeneratorSummary, generateAppForgePwaPackage, appForgeGeneratorSchemaExport } from "./app-forge-generator.js?v=1.03.27";
...
function siteDetail(){
  ...
  const tabByKey10326=new Map(tabs.map(item=>[item[0],item]));
  const primaryTabKeys10326=new Set(["overview","docs","notes"]);
  const primaryTabs10326=["overview","docs","notes"].flatMap(key=>tabByKey10326.has(key)?[tabByKey10326.get(key)]:[]);
  const moreTabs10326=["details","locations","equipment",...tabs.map(([key])=>key).filter(key=>!primaryTabKeys10326.has(key)&&!["details","locations","equipment"].includes(key))].flatMap(key=>tabByKey10326.has(key)?[tabByKey10326.get(key)]:[]);
  const moreTabActive10326=moreTabs10326.some(([key])=>key===accountDetailTab0735);
  const primaryTabCount10326=Math.max(1,primaryTabs10326.length+(moreTabs10326.length?1:0));
  html(`<div class="screen accountDetail0871 accountDetail0952 accountDetail1011">
    <section class="accountDetailHeader0952 accountCompactHeader10327 category-${category}">
      ...
      ${nativeDocumentScannerAvailable10325()&&moduleEnabled0955("core.files")?`<button class="accountScanPrimary10326" id="accountScanDocument10326" aria-label="Scan a document for ${esc(s.name||"this account")}">${fvIcon073("scan","accountScanIcon10326")}<span><strong>Scan Document</strong><small>Apple camera · automatic edges · multi-page</small></span><b aria-hidden="true">›</b></button>`:""}
      ...
      <nav class="accountTabs0871 accountTabs0952 accountPrimaryTabs10326" style="--account-primary-tab-count:${primaryTabCount10326}" aria-label="${esc(appLabel("recordSections"))}">${primaryTabs10326.map(([key,label])=>`<button class="${accountDetailTab0735===key?"active":""}" data-account-tab0735="${key}" aria-current="${accountDetailTab0735===key?"page":"false"}">${fvIcon073(tabIcons1011[key]||"home","accountTabIcon1011")}<span>${label}</span></button>`).join("")}${moreTabs10326.length?`<button class="accountMoreTab10326 ${moreTabActive10326?"active":""}" id="accountMoreTab10326" aria-expanded="false" aria-controls="accountMoreMenu10326" aria-current="${moreTabActive10326?"page":"false"}">${fvIcon073("list","accountTabIcon1011")}<span>More</span></button>`:""}</nav>
      ${moreTabs10326.length?`<div class="accountMoreMenu10326" id="accountMoreMenu10326" hidden>${moreTabs10326.map(([key,label])=>`<button class="${accountDetailTab0735===key?"active":""}" data-account-tab0735="${key}" aria-current="${accountDetailTab0735===key?"page":"false"}">${fvIcon073(tabIcons1011[key]||"list","accountTabIcon1011")}<span>${label}</span></button>`).join("")}</div>`:""}
    </section>
  ...
  document.getElementById("accountMoreTab10326")?.addEventListener("click",event=>{const button=event.currentTarget,menu=document.getElementById("accountMoreMenu10326"),opening=menu?.hidden!==false;if(menu)menu.hidden=!opening;button.setAttribute("aria-expanded",opening?"true":"false");button.classList.toggle("menuOpen10326",opening);});
...
function manualHome058(){
  ...
  return `<div class="academyHome067">
    ...
    ${pinnedLearning059()}
    <p class="academyReviewNote067">Manual revision 1.03.27 · Reviewed July 2026. Documentation must be checked with every release.</p>
  </div>`;
}
...
function manualSimplePage058(type){
 ...
 pages.quick[3][0][1]="Confirm the green build badge shows 1.03.27 before entering production information.";
 pages.new=["🆕","What’s New in 1.03.27","Account Detail now uses a compact command header so account information appears sooner without hiding important tools.",[
  ["Two-tap scanning","Open an account, then tap the large Scan Document button. There is no need to find the Documents / Photos workspace first."],
  ["Return to the account","After Save Scan, FireVault returns directly to the same Account Detail screen."],
  ["Collapsed secondary tabs","Overview, Photos / Files, and Notes stay visible. Details, Locations, and Equipment are available under one More control."],
  ["Cleaner document vault","Search, filters, and sorting stay collapsed until Find or organize saved items is opened."],
  ["Native phone icon","The Xcode app now installs with the FireVault flame icon instead of a generic placeholder."]
 ]];
 if(Array.isArray(pages.revisions?.[3]?.[0]))pages.revisions[3][0].unshift(["1.03.26","Moves Scan Document directly onto Account Detail, returns saved scans to the same account, collapses secondary account tabs and document-management controls, and adds the native FireVault app icon."],["1.03.25","Restores account document scanning through Apple VisionKit with automatic edge correction, multi-page capture, compact save review, IndexedDB storage, and retained PDF tools."]);
...
function showChangelog(){
  const notes = [
    "Build 1.03.27 compacts the Account Detail identity, scan, action, and section controls into one cleaner command header.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 1.03.27 simplifies Account Detail so the account’s working information is visible sooner, without changing existing account data.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
