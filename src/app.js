import { BUILD, KEY, loadData, saveData, ensureSite, fullAddress, esc, uid, downloadBlob, syncSummary, syncQueue, syncConflicts, syncActivity, createSyncPackage, importSyncPackage, resolveSyncConflict, notePackageExport, deviceIdentity, recordSyncActivity, autoBackupInfo, latestAutoBackup, restoreAutoBackup, isDemoMode, setDemoMode, resetDemoData, securityFoundationSummary, securityAudit, recycleBinInfo, restoreRecycleRecord, purgeRecycleBin, recordSecurityEvent, validateVaultIntegrity } from "./storage.js?v=0.96.1";
import { backendAdapterSummary, runBackendAdapterDiagnostics, backendAdapterManifest, PROVIDER_CONTRACT_VERSION, FILE_STORAGE_CATALOG, fileStoragePlanSummary, cloudFileStorageManifest, MICROSOFT_STORAGE_TYPES, microsoftStorageAccounts, saveMicrosoftStorageAccounts, createMicrosoftStorageAccount, microsoftStorageAccountById, microsoftAppRegistration, saveMicrosoftAppRegistration, microsoftStorageSummary, microsoftStorageManifest } from "./providers.js?v=0.96.1";
import { encodePlusCode, isValidFullPlusCode, normalizePlusCode, plusCodePrecisionLabel } from "./open-location-code.js?v=0.96.1";
import { hydrateVaultMedia, stageVaultMedia, prepareVaultWithMedia, mediaStorageSummary, requestPersistentMediaStorage, pruneOrphanedMedia, flushMediaWrites } from "./media-store.js?v=0.96.1";
import { APP_PROFILE, APP_PROFILE_SCHEMA_VERSION, appTerm, appLabel, appNavigationLabel, appProfileExport } from "./app-profile.js?v=0.96.1";
import { MODULE_REGISTRY, MODULE_REGISTRY_VERSION, MODULE_CLASSIFICATIONS, FUTURE_APP_COLUMNS, moduleRegistrySummary, moduleMatrixRows, moduleRegistryExport } from "./module-registry.js?v=0.96.1";
import { MODULE_BINDINGS_VERSION, NAV_MODULE_REQUIREMENTS as NAV_MODULE_REQUIREMENTS_0955, ROUTE_MODULE_REQUIREMENTS as ROUTE_MODULE_REQUIREMENTS_0955, SETTINGS_MODULE_REQUIREMENTS as SETTINGS_MODULE_REQUIREMENTS_0955, ACCOUNT_TAB_BINDINGS, requirementsMet as moduleRequirementsMetFromBindings0955, moduleBindingsExport } from "./module-bindings.js?v=0.96.1";
import { RECORD_SCHEMA, RECORD_SCHEMA_VERSION, recordFieldById, activeRecordFields, recordFieldEnabled, recordFieldRequired, activeDetailSections, recordDetailSectionEnabled, recordPhotoCategories, recordSchemaSummary, recordSchemaExport } from "./record-schema.js?v=0.96.1";
import { WORKFLOW_SCHEMA, WORKFLOW_SCHEMA_VERSION, activeWorkflowActions, workflowActionEnabled, quickPhotoWorkflow, workflowSchemaSummary, workflowSchemaExport } from "./workflow-schema.js?v=0.96.1";
import { THEME_PROFILE_SCHEMA_VERSION, resolveThemeProfile, applyThemeProfile, themeBrandAsset, themeWordmarkMarkup, themeProfileSummary, themeProfileExport } from "./theme-profile.js?v=0.96.1";
import { CONTENT_PACK_SCHEMA_VERSION, CONTENT_SOURCE_TYPES, CONTENT_SOURCES, CONTENT_PACKS, resolveContentPackProfile, activeContentSources, activeContentPacks, contentPackLibraryFolders, contentPackSummary, contentPackRegistryExport } from "./content-pack-registry.js?v=0.96.1";
import { SYNC_STORAGE_PROFILE_SCHEMA_VERSION, STORAGE_PROVIDER_TYPES, STORAGE_PROVIDERS, STORAGE_ROLES, resolveSyncStorageProfile, activeStorageProviders, storageProviderEnabled, storageRoleProviders, syncStorageSettingsTabEnabled, syncStorageSummary, syncStorageProfileExport } from "./sync-storage-profile.js?v=0.96.1";
import { ARCHITECTURE_AUDIT_SCHEMA_VERSION, LOCATION_GUIDE_TEST_PROFILE, runArchitectureValidationSuite, architectureAuditText, architectureAuditExport } from "./architecture-validator.js?v=0.96.1";
...
let ARCHITECTURE_AUDIT_0961=runArchitectureValidationSuite(APP_PROFILE,{build:BUILD,storageKey:KEY});
window.__FIREVAULT_ARCHITECTURE_AUDIT=ARCHITECTURE_AUDIT_0961;
function refreshArchitectureAudit0961(){ARCHITECTURE_AUDIT_0961=runArchitectureValidationSuite(APP_PROFILE,{build:BUILD,storageKey:KEY});window.__FIREVAULT_ARCHITECTURE_AUDIT=ARCHITECTURE_AUDIT_0961;return ARCHITECTURE_AUDIT_0961;}
...
const SETTINGS_GROUPS_067 = [
  ...
  {key:"photos",icon:"▧",title:"Photo Overlays",tabs:["overlay"]},
...
const SETTINGS_GROUPED_LIST_0873 = [
  {label:"Profile",items:[
    {key:"profile",subtitle:"Technician identity and overlay template"}
  ]},
...
function settings(){
  ...
  html(`<div class="screen settingsSimpleDetail0850 settingsDetail0860 settingsStable573 settingsPolish0880 settingsTab-${settingsTab} tone-${settingsTone0880(dashboardItem.key)} ${compactOverlayDetail?"settingsOverlayDetail0943":""}" data-settings-tab="${settingsTab}">
...
function overlayTagButtons510(){
  ...
  return OVERLAY_TAGS_510.map(([tag,label,note])=>`<button type="button" class="overlayFieldChip510 ${active.has(tag)?"isAdded0944":""}" data-overlay-add-field="${esc(tag)}" title="${esc(note)}" ${active.has(tag)?"disabled":""}><span>${active.has(tag)?"✓":"＋"}</span><strong>${esc(label)}</strong></button>`).join("");
}
function overlayActiveFieldsMarkup0944(){
  ...
  return overlayFieldState0944.map((item,index)=>{
    ...
    return `<div class="overlayActiveField0944 ${isTechInfo?"overlayTechInfoField0949":""}" draggable="true" data-overlay-field-index="${index}">
      <span class="overlayDragHandle0944" aria-hidden="true">☰</span>
      ...
      <div class="overlayFieldAlign0945" role="group" aria-label="Align ${esc(meta.label)}">
        <button type="button" class="${align==="left"?"active":""}" data-overlay-align-field="${index}" data-overlay-field-align="left" aria-pressed="${align==="left"}">L</button>
        <button type="button" class="${align==="center"?"active":""}" data-overlay-align-field="${index}" data-overlay-field-align="center" aria-pressed="${align==="center"}">C</button>
        <button type="button" class="${align==="right"?"active":""}" data-overlay-align-field="${index}" data-overlay-field-align="right" aria-pressed="${align==="right"}">R</button>
      </div>
      <button type="button" class="overlayBreakButton0944 ${item.breakBefore?"active":""}" data-overlay-break-field="${index}" ${(index===0||isTechInfo)?"disabled":""} aria-label="${isTechInfo?"Tech Info uses its Profile template line arrangement":`${item.breakBefore?"Place on same line":"Start a new line before"} ${esc(meta.label)}`}" title="${isTechInfo?"Line arrangement is set in Profile":item.breakBefore?"New line":"Same line"}">${isTechInfo?"▦":item.breakBefore?"↵":"•"}</button>
      <div class="overlayFieldActions0944">
        <button type="button" data-overlay-move-field="${index}" data-overlay-direction="up" ${index===0?"disabled":""} aria-label="Move ${esc(meta.label)} up">↑</button>
        <button type="button" data-overlay-move-field="${index}" data-overlay-direction="down" ${index===overlayFieldState0944.length-1?"disabled":""} aria-label="Move ${esc(meta.label)} down">↓</button>
        <button type="button" class="danger" data-overlay-remove-field="${index}" aria-label="Remove ${esc(meta.label)}">×</button>
      </div>
    </div>`;
  }).join("");
}
...
function scheduleOverlayAutoSave0944(delay=320){
  ...
  overlayAutoSaveTimer0944=setTimeout(()=>{
    ...
    save();
    overlayAutoSaveStatus0944("Saved automatically","saved");
  },delay);
}
...
function overlayPreviewMarkup510(){
  return `<section class="overlayPreviewCard0890 overlayPreviewSticky0942 overlayPreviewWide0943">
    <button type="button" class="overlayFloatingBack0943" id="settingsBackBtn" aria-label="Back to Settings">←</button>
    <div class="overlayPreviewIdentity0943"><span>LIVE</span><strong>Field Photo</strong><em>Exact export preview</em></div>
...
function overlaySettingsPanel510(o){
  ...
  return `<div class="overlayStudio0890 overlayStudio0942 overlayStudio0943">
    ${overlayPreviewMarkup510()}
    <div class="overlayControlsColumn0942 overlayControlsColumn0943">
      <section class="overlaySection0890 overlayContent0890 overlayFieldBuilder0944">
        <div class="overlayCardHead0890"><div><span>CONTENT</span><h2>Photo data fields</h2><p>Tap to add. Drag or use the arrows to rearrange.</p></div><em class="overlayAutoSave0944" id="overlayAutoSaveStatus0944" data-state="saved">Saved automatically</em></div>
        <input type="hidden" id="ovTemplate" value="${esc(set.template)}">
        <div class="overlayBuilderGroup0944"><div class="overlayBuilderTitle0944"><div><strong>Fields shown on photo</strong><small>Reorder fields, start a new line, or change alignment. Tech Info uses your Profile template as one field.</small></div><button type="button" class="overlayQuickAlign0945" id="overlayTechnicianInfo0946"><span>＋</span><strong>TECH INFO</strong><small>Use Profile template</small></button></div><div class="overlayActiveFields0944" id="overlayActiveFields0944">${overlayActiveFieldsMarkup0944()}</div></div>
        <div class="overlayBuilderGroup0944"><div class="overlayBuilderTitle0944"><strong>Add a field</strong><small>Fields already in use are checked. Technician profile details appear as one Tech Info field.</small></div><div class="overlayFieldGrid510" id="overlayAvailableFields0944">${overlayTagButtons510()}</div></div>
      </section>
...
function architectureSummaryText0953(){
  ...
  return [
    ...
    `Architecture audit: ${ARCHITECTURE_AUDIT_SCHEMA_VERSION} (${ARCHITECTURE_AUDIT_0961.status})`,
    ...
    `Architecture validation and hidden alternate-profile proof: ${ARCHITECTURE_AUDIT_0961.status}`,
...
function architectureAuditCheckMarkup0961(check){
  const icon=check.status==="pass"?"✓":check.status==="warning"?"!":"×";
  return `<article class="architectureAuditCheck0961 status-${esc(check.status)}"><span>${icon}</span><div><strong>${esc(check.title)}</strong><small>${esc(check.category)}</small><p>${esc(check.detail)}</p></div></article>`;
}
function architectureAuditProfileMarkup0961(audit,label){
  const exceptions=audit.checks.filter(check=>check.status!=="pass");
  return `<section class="architectureAuditProfile0961"><div class="architectureAuditProfileHead0961"><div><span>${esc(label)}</span><strong>${esc(audit.profile.name)}</strong><small>${esc(audit.profile.id)} · ${audit.totals.total} checks</small></div><em class="status-${audit.status.toLowerCase()}">${esc(audit.status.replaceAll("_"," "))}</em></div><div class="architectureAuditMiniMetrics0961"><span><b>${audit.totals.pass}</b> Passed</span><span><b>${audit.totals.warning}</b> Warnings</span><span><b>${audit.totals.fail}</b> Failed</span></div>${exceptions.length?`<div class="architectureAuditExceptions0961">${exceptions.map(architectureAuditCheckMarkup0961).join("")}</div>`:`<p class="architectureAuditClear0961">All checks passed for this profile.</p>`}<details class="architectureAuditDetails0961"><summary>View all ${audit.totals.total} checks</summary><div>${audit.checks.map(architectureAuditCheckMarkup0961).join("")}</div></details></section>`;
}
function architectureAuditSection0961(){
  const suite=ARCHITECTURE_AUDIT_0961;
  const status=suite.status.toLowerCase();
  return settingsSection540("Automated regression protection","Architecture Validation",`${suite.totals.pass} of ${suite.totals.total} checks passed across FireVault and the hidden alternate-app proof profile.`, `
    <div class="architectureAuditHero0961 status-${status}"><div><span>BUILD ${esc(BUILD)} VALIDATION</span><strong>${esc(suite.status.replaceAll("_"," "))}</strong><p>Profile references, module dependencies, routes, Account Detail tabs, workflows, themes, content packs, storage roles, and FireVault regressions are checked together.</p></div><div class="architectureAuditScore0961"><b>${suite.totals.pass}</b><span>of ${suite.totals.total}<br>passed</span></div></div>
    <div class="architectureAuditMetrics0961"><div><strong>${suite.primary.totals.pass}</strong><span>FireVault passes</span></div><div><strong>${suite.proof.totals.pass}</strong><span>Proof passes</span></div><div><strong>${suite.totals.warning}</strong><span>Warnings</span></div><div><strong>${suite.totals.fail}</strong><span>Failures</span></div></div>
    <div class="architectureAuditProfiles0961">${architectureAuditProfileMarkup0961(suite.primary,"ACTIVE APP PROFILE")}${architectureAuditProfileMarkup0961(suite.proof,"HIDDEN TRANSFORMATION PROOF")}</div>
    <div class="architectureProofNote0961"><strong>Alternate-profile proof</strong><span>${esc(LOCATION_GUIDE_TEST_PROFILE.name)} is validation-only and never replaces FireVault. It proves that Account becomes Location, fire-only fields disappear, a travel content pack resolves, and local-only storage still works.</span></div>
    <div class="architectureActions0953"><button class="primary" id="runArchitectureAudit0961">Run Audit Again</button><button class="ghost" id="downloadArchitectureAudit0961">Download Audit JSON</button><button class="ghost" id="downloadArchitectureAuditText0961">Download Audit Text</button></div>
  `,"green");
}
    ...
    ${settingsSection540("Reusable foundation","App Profile",`${APP_PROFILE.name} remains optimized for ${APP_PROFILE.audience.toLowerCase()}, while shared behavior is defined outside the fire-specific layer.`,`
      <div class="architectureProfile0953">
        ...
        <div class="architectureProfileGrid0953">
          ...
          <div><strong>Architecture audit</strong><span>${esc(ARCHITECTURE_AUDIT_0961.status.replaceAll("_"," "))} · v${ARCHITECTURE_AUDIT_SCHEMA_VERSION}</span></div>
        ...
        <p>The profile supplies terminology, enabled modules, record fields, detail sections, photo categories, actions, capture behavior, branding, visual tokens, data sources, content packs, storage providers, backup rules, and collaboration policy to the shared workflows. FireVault keeps every current fire-alarm module, field, and technician action enabled.</p>
      </div>
    `,"red")}
...
function wireArchitecture0953(){
  ...
  document.getElementById("runArchitectureAudit0961")?.addEventListener("click",()=>{refreshArchitectureAudit0961();toast(ARCHITECTURE_AUDIT_0961.passed?"Architecture audit passed.":"Architecture audit found failures.",ARCHITECTURE_AUDIT_0961.passed?"success":"error");settings();});
  document.getElementById("downloadArchitectureAudit0961")?.addEventListener("click",()=>{const suite=refreshArchitectureAudit0961();downloadBlob(`firevault-architecture-audit-build-${BUILD}.json`,JSON.stringify(architectureAuditExport(suite),null,2),"application/json");toast("Architecture audit downloaded.","success");});
  document.getElementById("downloadArchitectureAuditText0961")?.addEventListener("click",()=>{const suite=refreshArchitectureAudit0961();downloadBlob(`firevault-architecture-audit-build-${BUILD}.txt`,architectureAuditText(suite),"text/plain");toast("Architecture audit text downloaded.","success");});
...
function settingsPanel(){
  ...
  return `<div class="settingsStack settingsStack540">
    ${settingsSection540("Fire alarm field system",`About FireVault`,`A fire-alarm field application built on a reusable Field Vault core.`,`<div class="aboutBrand540">${fireVaultBrand575()}<span>Field Vault System</span></div><p class="aboutCopy540">FireVault is optimized for fire alarm technicians while its shared record, search, GPS, photo, file, security, and storage foundations remain reusable for future vertical apps.</p>`,"red")}
...
function showChangelog(){
  const notes = [
    "Build 0.96.1 adds automated architecture validation across module dependencies, routes, Account Detail tabs, record fields, workflows, themes, content packs, and storage roles.",
    "A hidden Location Guide proof profile confirms that FireVault-specific modules and fields can be removed while shared Search, Nearby, Notes, Photos, Files, location points, content packs, and local storage continue to resolve.",
    "Build 0.96.0 adds a configurable Sync & Storage Profile for local backends, approved providers, backup policy, package exchange, conflict handling, and credential safeguards.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.96.1 adds automated architecture and regression validation while preserving the current fire-alarm technician workflow.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
