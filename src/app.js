...
function accountDirectoryRow0759(s,addressCount=1){
  ...
  const supporting=panel || contact?.name || "Panel and primary contact not entered";
...
const SETTINGS_DASHBOARD_0860 = [
  ...
  {key:"field",title:"Field",icon:"briefcase",tabs:["plusCodes"]},
  {key:"reports",title:"Reports",icon:"document",tabs:["reports","email"]},
  {key:"data",title:"Data & Backup",icon:"database",tabs:["cloudFiles","microsoftStorage","sync","customerImport","categories","backup","updates"]},
  {key:"maps",title:"Maps & GPS",icon:"pin",tabs:["gps"]},
...
function settings(){
  ...
  if(mode!=="settingsDetail" && mode!=="settingsGroup0860"){
    html(`<div class="screen settingsDashboard0860 settingsStable573">
      <header class="settingsDashboardHeader0860"><div class="settingsBrand0860"><span class="settingsBrandMark0860">◆</span><b><i>FIRE</i>VAULT</b></div><button class="ghost" id="settingsHomeBtn572">Done</button></header>
      <div class="settingsDashboardBody0860"><h1>Settings</h1><div class="settingsTileGrid0860">${SETTINGS_DASHBOARD_0860.map(settingsDashboardTile0860).join("")}</div></div>
    </div>`);
    document.getElementById("settingsHomeBtn572")?.addEventListener("click",leaveSettingsHome572);
    document.querySelectorAll("[data-settings-group0860]").forEach(b=>b.onclick=()=>{
      const item=SETTINGS_DASHBOARD_0860.find(x=>x.key===b.dataset.settingsGroup0860);
      if(!item) return;
      settingsGroup067=item.key;if(item.tabs.length===1){settingsTab=item.tabs[0];mode="settingsDetail";render();}
