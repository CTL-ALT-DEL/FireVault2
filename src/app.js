...
function home(){
  ...
  html(`<div class="screen">
    ...
    <div class="card grow"><h2>Build ${BUILD}</h2><p>Bottom menu placement has been micro-adjusted upward while keeping the emergency loading fix intact.</p><p>Nearby Sites, GPS tools, Settings readability, and task/service modules remain preserved.</p></div>
  </div>`);
...
function showChangelog(){
  const notes = [
    "Micro-lifted the bottom menu icons so they no longer sit quite as low on iPhone home-screen mode.",
    "Kept the darker bottom safe-area coverage that prevents the white strip from showing under the menu.",
    "Preserved the emergency Loading FireVault fix and boot watchdog from 0.42.4.",
    "Kept Settings readability, GPS visibility, Nearby Sites, Task Center, Visit Log, service follow-ups, haptics, GPS capture, and map actions."
  ];
  const overlay=document.createElement("div");
  overlay.className="releaseOverlay";
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    <div class="releaseHead"><div><strong>FireVault</strong><span>Build ${BUILD}</span></div><button class="ghost iconBtn" id="closeRelease" aria-label="Close release notes">×</button></div>
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">bottom menu micro-lift and stability preservation.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
