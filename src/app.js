import { BUILD, KEY, loadData, saveData, ensureSite, fullAddress, esc, uid, downloadBlob, syncSummary, syncQueue, syncConflicts, syncActivity, createSyncPackage, importSyncPackage, resolveSyncConflict, notePackageExport, deviceIdentity, recordSyncActivity, autoBackupInfo, latestAutoBackup, restoreAutoBackup, isDemoMode, setDemoMode, resetDemoData, securityFoundationSummary, securityAudit, recycleBinInfo, restoreRecycleRecord, purgeRecycleBin, recordSecurityEvent, validateVaultIntegrity } from "./storage.js?v=0.98.0";
import { backendAdapterSummary, runBackendAdapterDiagnostics, backendAdapterManifest, PROVIDER_CONTRACT_VERSION, FILE_STORAGE_CATALOG, fileStoragePlanSummary, cloudFileStorageManifest, MICROSOFT_STORAGE_TYPES, microsoftStorageAccounts, saveMicrosoftStorageAccounts, createMicrosoftStorageAccount, microsoftStorageAccountById, microsoftAppRegistration, saveMicrosoftAppRegistration, microsoftStorageSummary, microsoftStorageManifest } from "./providers.js?v=0.98.0";
import { encodePlusCode, isValidFullPlusCode, normalizePlusCode, plusCodePrecisionLabel } from "./open-location-code.js?v=0.98.0";
import { hydrateVaultMedia, stageVaultMedia, prepareVaultWithMedia, mediaStorageSummary, requestPersistentMediaStorage, pruneOrphanedMedia, flushMediaWrites } from "./media-store.js?v=0.98.0";
import { APP_PROFILE, APP_PROFILE_SCHEMA_VERSION, appTerm, appLabel, appNavigationLabel, appProfileExport } from "./app-profile.js?v=0.98.0";
import { MODULE_REGISTRY, MODULE_REGISTRY_VERSION, MODULE_CLASSIFICATIONS, FUTURE_APP_COLUMNS, moduleRegistrySummary, moduleMatrixRows, moduleRegistryExport } from "./module-registry.js?v=0.98.0";
import { MODULE_BINDINGS_VERSION, NAV_MODULE_REQUIREMENTS as NAV_MODULE_REQUIREMENTS_0955, ROUTE_MODULE_REQUIREMENTS as ROUTE_MODULE_REQUIREMENTS_0955, SETTINGS_MODULE_REQUIREMENTS as SETTINGS_MODULE_REQUIREMENTS_0955, ACCOUNT_TAB_BINDINGS, requirementsMet as moduleRequirementsMetFromBindings0955, moduleBindingsExport } from "./module-bindings.js?v=0.98.0";
import { RECORD_SCHEMA, RECORD_SCHEMA_VERSION, recordFieldById, activeRecordFields, recordFieldEnabled, recordFieldRequired, activeDetailSections, recordDetailSectionEnabled, recordPhotoCategories, recordSchemaSummary, recordSchemaExport } from "./record-schema.js?v=0.98.0";
import { WORKFLOW_SCHEMA, WORKFLOW_SCHEMA_VERSION, activeWorkflowActions, workflowActionEnabled, quickPhotoWorkflow, workflowSchemaSummary, workflowSchemaExport } from "./workflow-schema.js?v=0.98.0";
import { THEME_PROFILE_SCHEMA_VERSION, resolveThemeProfile, applyThemeProfile, themeBrandAsset, themeWordmarkMarkup, themeProfileSummary, themeProfileExport } from "./theme-profile.js?v=0.98.0";
import { CONTENT_PACK_SCHEMA_VERSION, CONTENT_SOURCE_TYPES, CONTENT_SOURCES, CONTENT_PACKS, resolveContentPackProfile, activeContentSources, activeContentPacks, contentPackLibraryFolders, contentPackSummary, contentPackRegistryExport } from "./content-pack-registry.js?v=0.98.0";
import { SYNC_STORAGE_PROFILE_SCHEMA_VERSION, STORAGE_PROVIDER_TYPES, STORAGE_PROVIDERS, STORAGE_ROLES, resolveSyncStorageProfile, activeStorageProviders, storageProviderEnabled, storageRoleProviders, syncStorageSettingsTabEnabled, syncStorageSummary, syncStorageProfileExport } from "./sync-storage-profile.js?v=0.98.0";
import { APP_FORGE_BLUEPRINT_SCHEMA_VERSION, appForgeReadinessSummary, validateAppForgeProfile, appForgeBlueprintExport } from "./app-forge-blueprint.js?v=0.98.0";
import { APP_FORGE_RECIPE_SCHEMA_VERSION, appForgeRecipes, appForgeRecipeSummary, appForgeRecipeBlueprintExport, appForgeRecipeCatalogExport } from "./app-forge-recipes.js?v=0.98.0";
  ...
  return `<div class="settingsStack settingsStack540 architectureStack0953">
    ${settingsSection540("Reusable foundation","App Profile",`${APP_PROFILE.name} remains optimized for ${APP_PROFILE.audience.toLowerCase()}, while shared behavior is defined outside the fire-specific layer.`,`
      <div class="architectureProfile0953">
        ...
        <p>The profile supplies terminology, enabled modules, record fields, detail sections, photo categories, actions, capture behavior, branding, visual tokens, data sources, content packs, storage providers, backup rules, and collaboration policy to the shared workflows. The AppForge Blueprint validates and packages those contracts as one portable product definition, while Product Recipes prove that the same foundation can describe distinct vertical apps. FireVault keeps every current fire-alarm module, field, and technician action enabled.</p>
      </div>
    `,"red")}
...
function showChangelog(){
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.98.0 proves the shared core can define several distinct app products while FireVault remains the active fire-alarm technician experience.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
