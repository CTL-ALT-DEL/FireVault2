...
function customerPhotoCaptionChecklist531(s={}){
  const {photos, selected}=reportPhotoSummary527(s);
  const lines=[
    `FireVault Customer Photo Caption Checklist`,
    `Site: ${s.name||"Unnamed Site"}`,
    `Selected: ${selected.length} of ${photos.length} saved photo${photos.length===1?"":"s"}`,
    ""
  ];
  if(!selected.length){
    lines.push("No customer report photos are selected yet.");
    return lines.join("
");
  }
  selected.forEach((d,i)=>{
    const caption=String(d.customerCaption||"").trim();
    lines.push(`${i+1}. ${reportPhotoLabel526(d)}`);
    lines.push(`   Status: ${caption?"READY":"NEEDS CAPTION"}`);
    if(caption) lines.push(`   Caption: ${caption.replaceAll("
","
   ")}`);
    else lines.push(`   Suggested: ${defaultCustomerCaption530(d)}`);
    if(d.linkedDeficiencyTitle) lines.push(`   Related deficiency: ${d.linkedDeficiencyTitle}`);
  });
  lines.push("", `Generated by FireVault Build ${BUILD}`);
  return lines.join("
...
async function copyCaptionChecklist531(){
  const s=site(); if(!s) return;
  try{ await navigator.clipboard.writeText(customerPhotoCaptionChecklist531(s)); toast("Caption checklist copied."); }
  catch{ toast("Clipboard unavailable."); }
}
...
function report(){
  ...
  const copyCaptionChecklist531=document.getElementById("reportCopyCaptionChecklist531"); if(copyCaptionChecklist531) copyCaptionChecklist531.onclick=copyCaptionChecklist531;
