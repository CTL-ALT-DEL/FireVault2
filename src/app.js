...
function showChangelog(){
  const notes = [
    "Build 0.94.9 shows Tech Info as one Photo Overlay field, hides individual technician profile fields in the overlay editor, and keeps Tech Info adjustable like any other overlay field.",
  ...
  overlay.innerHTML=`<div class="releaseSheet" role="dialog" aria-modal="true" aria-label="FireVault release notes">
    ...
    <div class="releaseBody"><h2>Release Notes</h2><p class="releaseIntro">FireVault 0.94.9 shows Tech Info as a single adjustable overlay field and hides the individual technician profile fields from the Photo Overlay editor.</p><ul>${notes.map(n=>`<li>${esc(n)}</li>`).join("")}</ul></div>
  </div>`;
  document.body.appendChild(overlay);
  const close=()=>overlay.remove();
