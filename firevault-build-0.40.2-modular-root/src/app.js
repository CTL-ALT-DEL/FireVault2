...
function home(){
  ...
  html(`<div class="screen">
    ...
    <div class="card grow"><h2>Build 0.40.2</h2><p>Settings now use a cleaner two-column desktop layout, a horizontal iPhone tab rail, clearer section cards, better checkbox rows, and a sticky save button.</p><p>The selected #8 Flame Icon remains the official app logo across the header, favicon, and PWA home screen.</p></div>
  </div>`);
...
function settings(){
  const tabs=[
    ["tech","Tech","ID card"],["reports","Reports","Defaults"],["email","Email","Templates"],["overlay","Photos","Overlay"],
    ["themes","Themes","Look"],["advanced","Advanced","Services"],["backup","Backup","Data"],["about","About","Build"]
  ];
  const active=tabs.find(t=>t[0]===settingsTab)||tabs[0];
  html(`<div class="screen settingsScreen">
    <div class="card settingsHero">
      <div class="settingsHeroTop">
        <img class="settingsLogo" src="assets/icon-192.png" alt="FireVault">
        <div><h1>Settings</h1><p>${active[1]} • ${active[2]}</p></div>
        <button class="ghost miniBtn" id="diagBtn">Diagnostics</button>
      </div>
    </div>
    <div class="settingsLayout grow">
      <div class="settingsRail">${tabs.map(t=>`<button class="tabBtn ${settingsTab===t[0]?"active":""}" data-tab="${t[0]}"><strong>${t[1]}</strong><span>${t[2]}</span></button>`).join("")}</div>
      <div class="settingsContent">${settingsPanel()}</div>
    </div>
  </div>`);
...
function settingsPanel(){
  ...
  if(settingsTab==="tech") return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>Technician Profile</h2><p>Used on reports, email tags, and future photo stamps.</p></div></div><div class="settingsGrid">${fieldBlock("Name",`<input id="techName" value="${esc(tech.name)}">`)}${fieldBlock("Company",`<input id="techCompany" value="${esc(tech.company)}">`)}${fieldBlock("Phone",`<input id="techPhone" value="${esc(tech.phone)}">`)}${fieldBlock("Email",`<input id="techEmail" value="${esc(tech.email)}">`)}${fieldBlock("License / ID",`<input id="techLicense" value="${esc(tech.license)}">`)}</div></div><button class="primary saveBar" id="saveSettings">Save Technician</button></div>`;
  if(settingsTab==="reports") return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>Report Defaults</h2><p>Controls what is included when a site report is generated.</p></div></div><div class="settingsGrid">${fieldBlock("Report Title",`<input id="reportTitle" value="${esc(r.title)}">`)}${fieldBlock("Format",`<select id="reportFormat"><option ${r.format==="detailed"?"selected":""}>detailed</option><option ${r.format==="compact"?"selected":""}>compact</option></select>`)}</div><div class="settingsList">${checkBlock("repTech","Include technician profile",r.includeTechnician)}${checkBlock("repTasks","Include open and completed tasks",r.includeTasks)}${checkBlock("repDef","Include deficiencies",r.includeDeficiencies)}</div></div><button class="primary saveBar" id="saveSettings">Save Reports</button></div>`;
  if(settingsTab==="email") return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>Email Defaults</h2><p>Default recipient, subject line, and signature for reports.</p></div></div><div class="settingsGrid">${fieldBlock("Default To",`<input id="emailTo" value="${esc(email.defaultTo)}">`)}${fieldBlock("CC",`<input id="emailCc" value="${esc(email.cc)}">`)}</div>${fieldBlock("Default Subject",`<input id="emailSubject" value="${esc(email.defaultSubject)}">`)}${fieldBlock("Signature Tag",`<textarea id="emailSig">${esc(email.signature)}</textarea>`,`Tags: {site_name}, {date}, {technician}, {company}, {phone}, {email}`)}</div><button class="primary saveBar" id="saveSettings">Save Email</button></div>`;
  if(settingsTab==="overlay") return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>Photo Overlay</h2><p>Preview and format the field photo stamp.</p></div></div><div class="previewPanel"><div class="previewOverlay ${o.alignment==="top"?"top":""}">FIREVAULT • Site • Date • Time</div></div><div class="settingsGrid">${fieldBlock("Alignment",`<select id="ovAlign"><option ${o.alignment==="bottom"?"selected":""}>bottom</option><option ${o.alignment==="top"?"selected":""}>top</option></select>`)}${fieldBlock("Font Size",`<select id="ovSize"><option ${o.fontSize==="small"?"selected":""}>small</option><option ${o.fontSize==="medium"?"selected":""}>medium</option><option ${o.fontSize==="large"?"selected":""}>large</option></select>`)}${fieldBlock("Accent Color",`<input id="ovAccent" type="color" value="${esc(o.accentColor)}">`)}</div><div class="settingsList">${checkBlock("ovLogo","Show FireVault logo on overlay",o.showLogo)}</div></div><button class="primary saveBar" id="saveSettings">Save Overlay</button></div>`;
  if(settingsTab==="themes") return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>Theme Engine</h2><p>Quick presets and live UI controls for the whole app.</p></div></div><div class="presetGrid">${Object.entries(themePresets).map(([key,p])=>`<button class="ghost presetBtn" data-preset="${key}"><span class="themeSwatch" style="background:${p.accentColor}"></span><span>${p.label}</span></button>`).join("")}</div><div class="settingsGrid">${fieldBlock("Theme",`<select id="themeName">${Object.entries(themePresets).map(([key,p])=>`<option value="${key}" ${t.name===key?"selected":""}>${p.label}</option>`).join("")}</select>`)}${fieldBlock("Accent Color",`<input id="themeAccent" type="color" value="${esc(t.accentColor||"#ef4444")}">`)}${fieldBlock("Buttons",`<select id="buttonStyle"><option value="rounded" ${t.buttonStyle!=="squared"?"selected":""}>rounded</option><option value="squared" ${t.buttonStyle==="squared"?"selected":""}>squared</option></select>`)}${fieldBlock("Cards",`<select id="cardStyle"><option value="glass" ${t.cardStyle!=="solid"?"selected":""}>glass</option><option value="solid" ${t.cardStyle==="solid"?"selected":""}>solid</option></select>`)}</div><div class="settingsList">${checkBlock("themeHighContrast","High contrast support",t.highContrast)}${checkBlock("themeLargeText","Larger text",t.largeText)}${checkBlock("themeCompact","Compact layout",t.compactLayout)}</div></div><button class="primary saveBar" id="saveSettings">Apply Theme</button></div>`;
  if(settingsTab==="advanced") return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>Advanced Features</h2><p><span class="featureStar">*</span> Requires an outside service, permission, API, or future backend.</p></div></div><div class="settingsList twoCol">${[["advAi","aiTechnician","AI Technician"],["advReverse","reverseAddressLookup","Reverse Address Lookup *"],["advCloud","cloudBackup","Cloud Backup *"],["advVoice","voiceTranscription","Voice Transcription *"],["advOcr","ocrReader","OCR Reader *"],["advEmail","emailGateway","Email Gateway *"],["advWeather","weather","Weather Context *"],["advTraffic","traffic","Traffic / Routing *"]].map(x=>checkBlock(x[0],x[2],a[x[1]])).join("")}</div></div><button class="primary saveBar" id="saveSettings">Save Advanced</button></div>`;
  if(settingsTab==="backup") return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>Import / Export</h2><p>Export before replacing files in GitHub Pages.</p></div></div><div class="settingsList"><button class="primary" id="exportBtn">Export Backup</button>${fieldBlock("Import Backup",`<input type="file" id="importFile" accept="application/json">`)}<button class="danger" id="resetBtn">Clear Local Data</button></div></div></div>`;
  return `<div class="settingsStack"><div class="card settingGroup"><div class="sectionHeader"><div><h2>About FireVault</h2><p>A modular field knowledge system for fire alarm technicians.</p></div></div><div class="aboutGrid"><div><strong>Build</strong><span>${BUILD}</span></div><div><strong>Storage key</strong><span>${KEY}</span></div><div><strong>Roadmap lane</strong><span>Modular foundation, settings polish, iPhone PWA, deeper service-call modules.</span></div></div></div></div>`;
}
